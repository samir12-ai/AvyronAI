...
const DEPTH_STATUS_CONFIG: Record<string, { color: string; icon: string; label: string }> = {
  ...
  SIGNAL_GROUNDING_FAILED: { color: '#f59e0b', icon: 'information-circle', label: 'LOW CONFIDENCE' },
  LOW_CONFIDENCE: { color: '#f59e0b', icon: 'information-circle', label: 'LOW CONFIDENCE' },
...
function DepthGateStatusPanel({ campaignId, isDark }: { campaignId: string; isDark: boolean }) {
        ...
        for (const section of data.sections) {
          if (DEPTH_GATE_ENGINES.includes(section.engineId)) {
            ...
            if (section.engineId === 'positioning') {
              ...
              else if (output?.status === 'LOW_CONFIDENCE') status = 'LOW_CONFIDENCE';
            ...
            } else if (output?.status === 'SIGNAL_GROUNDING_FAILED' || output?.status === 'LOW_CONFIDENCE') {
  ...
  const anyFailed = Object.values(gateData).some((g: any) => g.status === 'DEPTH_FAILED' || g.status === 'DEPTH_BLOCKED' || g.status === 'SIGNAL_REQUIRED' || g.status === 'SIGNAL_BLOCKED');
