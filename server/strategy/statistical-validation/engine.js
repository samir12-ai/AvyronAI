import { ENGINE_VERSION, STATUS, BOUNDARY_HARD_PATTERNS, BOUNDARY_SOFT_PATTERNS, LAYER_WEIGHTS, NARRATIVE_CLAIM_PATTERNS, ASSUMPTION_INDICATORS, EVIDENCE_DENSITY_THRESHOLDS, CLAIM_CONFIDENCE_THRESHOLDS, VALIDATION_STATES, SIGNAL_GROUNDING_THRESHOLD, SIGNAL_EQUIVALENCE_MAP, SIGNAL_MAPPING_CONFIDENCE_THRESHOLD, PROOF_TYPE_CLASSIFIERS, PERSUASION_DRIVER_CLASSIFIERS, AWARENESS_TRIGGER_CLASSIFIERS, AWARENESS_ENTRY_CLASSIFIERS, CANONICAL_SIGNAL_REGISTRY, } from "./constants";
...
function classifyProofSubType(proofText) {
    const lower = proofText.toLowerCase();
    for (const [subType, pattern] of Object.entries(PROOF_TYPE_CLASSIFIERS)) {
        if (pattern.test(lower))
            return `offer_proof_${subType}`;
    }
    return "offer_proof";
}
function classifyPersuasionDriverSubType(driverText) {
    const lower = driverText.toLowerCase();
    for (const [subType, pattern] of Object.entries(PERSUASION_DRIVER_CLASSIFIERS)) {
        if (pattern.test(lower))
            return `persuasion_driver_${subType}`;
    }
    return "persuasion_driver";
}
function classifyAwarenessTriggerSubType(triggerText) {
    const lower = triggerText.toLowerCase();
    for (const [subType, pattern] of Object.entries(AWARENESS_TRIGGER_CLASSIFIERS)) {
        if (pattern.test(lower))
            return `awareness_trigger_${subType}`;
    }
    return "awareness_trigger";
}
function classifyAwarenessEntrySubType(entryText) {
    const lower = entryText.toLowerCase();
    for (const [subType, pattern] of Object.entries(AWARENESS_ENTRY_CLASSIFIERS)) {
        if (pattern.test(lower))
            return `awareness_entry_${subType}`;
    }
    return "awareness_entry";
}
function computeKeywordMappingConfidence(claimText, sourceTag) {
    const registryEntries = Object.values(CANONICAL_SIGNAL_REGISTRY).filter(r => r.clusterId === sourceTag || r.parentCategory === sourceTag);
...
function matchClaimToSignalCluster(claimText, clusters, sourceTag) {
    const lower = claimText.toLowerCase();
    let bestSupporting = null;
    let bestSupportingScore = 0;
    let bestContradicting = null;
    ...
    const priorityClusters = equivalence ? new Set(equivalence.canonicalClusters) : new Set();
...
function assessDataReliability(mi, audience, offer, funnel, awareness, persuasion) {
    const advisories = [];
    ...
    if (persuasion.persuasionMode)
        sourceDiversity.add("persuasion_mode");
    const signalDiversity = clamp(sourceDiversity.size / 6, 0, 1);
    if (sourceDiversity.size < 3)
        advisories.push(`Low signal diversity: only ${sourceDiversity.size}/7 data sources populated`);
    ...
    if (narrativeStability < 0.5)
        advisories.push("Low narrative stability: persuasion or awareness data sparse");
    const competitorValidity = clamp(totalMISignals / 5, 0, 1);
    const marketMaturityConfidence = clamp((safeNumber(mi.overallConfidence, 0) + safeNumber(offer.offerStrengthScore, 0) + safeNumber(funnel.funnelStrengthScore, 0)) / 3, 0, 1);
    const overallReliability = clamp(signalDensity * 0.20 + signalDiversity * 0.20 + narrativeStability * 0.15 +
        competitorValidity * 0.15 + marketMaturityConfidence * 0.30, 0, 1);
...
function generateSignalTraceId(source, index) {
    const prefix = source.toUpperCase().replace(/_/g, "_");
    return `${prefix}_${String(index + 1).padStart(3, "0")}`;
}
function buildSignalPath(source, provenance, lineageMatch) {
...
function findLineageMatch(claimText, lineage) {
    if (lineage.length === 0)
        return null;
    const lower = claimText.toLowerCase();
...
function extractClaims(offer, persuasion, awareness, signalClusters, upstreamLineage = []) {
    const claims = [];
    const unmappedSignals = [];
    const lowConfidenceSignals = [];
    let claimIndex = 0;
    const hasLineage = upstreamLineage.length > 0;
    const addClaim = (claimText, source) => {
        if (!claimText)
            return;
...
function validateClaim(claim, source, signalProvenance, isHypothesis, signalTraceId, signalPath, mi, audience, offer, funnel, awareness, persuasion, hasLineageAnchor = false) {
    const lower = claim.toLowerCase();
    const supportingSignals = [];
    const contradictingSignals = [];
    let evidenceType = "assumption";
    ...
    const hasMIOrAudienceSupport = supportingSignals.some(s => !s.startsWith("Persuasion ") && !s.startsWith("Trust sequence:") &&
...
function layer_evidenceDensity(mi, audience, offer) {
    const findings = [];
    const warnings = [];
    let score = 0.5;
    ...
    if (totalSignals < 2)
        warnings.push("MI signals insufficient for competitive validation");
    if (proofCount === 0)
        warnings.push("No proof alignment found in offer — claims lack proof backing");
    return {
...
function layer_claimSignalAlignment(claimValidations) {
    const findings = [];
    const warnings = [];
...
function layer_narrativeVsSignal(claimValidations) {
    const findings = [];
    const warnings = [];
    const scorableClaims = claimValidations.filter(c => !c.isHypothesis);
    const hypothesisCount = claimValidations.filter(c => c.isHypothesis).length;
...
function layer_assumptionDetection(claimValidations, offer, persuasion) {
    const findings = [];
    const warnings = [];
...
function layer_crossEngineConsistency(offer, funnel, awareness, persuasion) {
    const findings = [];
    const warnings = [];
    let score = 0.6;
...
function layer_proofStrengthValidation(offer, persuasion, awareness) {
    const findings = [];
    const warnings = [];
    let score = 0.5;
    const proofCount = (offer.proofAlignment || []).length;
    ...
    const resolvedObjections = objectionProofLinks.filter((l) => l.proofAvailable);
...
function layer_confidenceCalibration(mi, reliability, claimValidations) {
    const findings = [];
    const warnings = [];
    let score = 0.5;
...
function buildConfidenceExplanation(score, evidenceStrength, state, wasNormalized, reliability, signalGroundingRatio, warnings) {
    const factors = [];
...
export async function runStatisticalValidationEngine(mi, audience, offer, funnel, awareness, persuasion, accountId, upstreamLineage = []) {
    ...
    const minConstrainedConfidence = Math.min(claimSupportScore, evidenceDensityScore, proofStrengthScore, crossEngineScore);
    let weightedScore = 0;
    ...
    if (claimConfidenceScore >= CLAIM_CONFIDENCE_THRESHOLDS.VALIDATED && evidenceStrength >= EVIDENCE_DENSITY_THRESHOLDS.MODERATE) {
        validationState = VALIDATION_STATES.VALIDATED;
    }
    else if (claimConfidenceScore >= CLAIM_CONFIDENCE_THRESHOLDS.PROVISIONAL) {
        validationState = VALIDATION_STATES.PROVISIONAL;
    }
    else if (claimConfidenceScore >= CLAIM_CONFIDENCE_THRESHOLDS.WEAK) {
        validationState = VALIDATION_STATES.WEAK;
    }
    else {
        validationState = VALIDATION_STATES.REJECTED;
    }
    if (signalBackedClaimRatio < SIGNAL_GROUNDING_THRESHOLD) {
        ...
        structuralWarnings.push(`GUARD: Signal-backed claims (${Math.round(signalBackedClaimRatio * 100)}%) below ${Math.round(SIGNAL_GROUNDING_THRESHOLD * 100)}% threshold — strategy is PROVISIONAL, execution engines blocked until additional signal grounding is achieved`);
    }
    ...
    const confidenceExplanation = buildConfidenceExplanation(claimConfidenceScore, evidenceStrength, validationState, confidenceNormalized, reliability, signalBackedClaimRatio, structuralWarnings);
    const layersPassed = layers.filter(l => l.passed).length;
    const strategyAcceptability = assessStrategyAcceptability(claimConfidenceScore, layersPassed, layers.length, validationState !== "rejected", structuralWarnings);
    return {
