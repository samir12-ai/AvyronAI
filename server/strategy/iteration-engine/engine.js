import { ENGINE_VERSION, MIN_PERFORMANCE_DATA_POINTS, MIN_CREATIVE_COUNT, MIN_FUNNEL_STAGES, CTR_FLOOR, ROAS_FLOOR, CONVERSION_RATE_FLOOR, MAX_CONCURRENT_TESTS, MIN_TEST_DURATION_DAYS, OPTIMIZATION_CONFIDENCE_THRESHOLD, BOUNDARY_BLOCKED_PATTERNS, STATUS, } from "./constants";
...
function sanitizeBoundary(text) {
    if (!text)
...
function assessDataReliability(performance, funnel, creative) {
    const advisories = [];
    let signalDensity = 0;
...
function layer1_performanceAnalysis(performance) {
    const findings = [];
    const warnings = [];
    let score = 0;
    if (!performance) {
...
function layer2_funnelAnalysis(funnel, performance = null) {
    const findings = [];
    const warnings = [];
    let score = 0;
...
function layer3_creativeAnalysis(creative, performance = null) {
    const findings = [];
    const warnings = [];
    let score = 0;
...
function layer4_guardLayer(hypotheses, failedFlags, reliability) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    ...
    for (const hyp of hypotheses) {
        const matchesFailure = recentFailures.some(f => f.strategyName.toLowerCase().includes(hyp.variable.toLowerCase()) ||
...
function generateHypotheses(performance, funnel, creative, persuasion) {
    const hypotheses = [];
    if (performance) {
    ...
    if (funnel) {
        const stages = funnel.stageConversions || [];
        const worstStage = stages.reduce((worst, s) => (!worst || s.conversionRate < worst.conversionRate) ? s : worst, null);
...
function generateOptimizationTargets(performance, funnel) {
    const targets = [];
    if (performance) {
...
function detectFailedStrategies(performance, creative) {
    const flags = [];
    if (performance) {
...
function generateIterationPlan(hypotheses, targets, reliability) {
    const steps = [];
    let stepNumber = 1;
...
function generateBenchmarkExplorationHypotheses() {
    return [
        {
            hypothesisId: generateId(),
...
function generateBenchmarkOptimizationTargets() {
    return [
        {
            targetArea: "Click-Through Rate",
            currentValue: 0,
...
function generateBenchmarkExplorationPlan() {
    return [
        {
            stepNumber: 1,
...
export async function runIterationEngine(performance, funnel, creative, persuasion, memoryContext) {
    ...
    const reliability = assessDataReliability(performance, effectiveFunnel || (performance ? {} : null), effectiveCreative || (performance ? {} : null));
    ...
    const layerResults = [perfLayer, funnelLayer, creativeLayer];
    ...
    if (memoryContext) {
        ...
        for (const line of lines) {
            if (line.includes("REINFORCE") || line.includes("Reinforce") || line.includes("reinforce"))
                ...
                reinforceTerms.push(line.toLowerCase());
            else if (mode === "avoid")
                avoidTerms.push(line.toLowerCase());
        }
        const memoryAdjLog = [];
    ...
    let rawConfidence = clamp(avgLayerScore * 0.4 +
    ...
    const acceptability = assessStrategyAcceptability(confidenceScore, layersPassed, totalLayers, guardLayer.passed, structuralWarnings);
    return {
        status,
        statusMessage,
