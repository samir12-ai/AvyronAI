...
function buildDesigner(args: {
  ...
  const judge = args.judgeFeedback ? `\n═══ PRIOR REJECTED ═══\n${args.judgeFeedback}\nRewrite with named interlock + concrete withdrawal triggers.\n` : "";
...
- whatWeWillNotPursue is empty — must show real tradeoffs
...
export async function designChannelOrchestration(args: Parameters<typeof buildDesigner>[0] & { accountId: string }): Promise<ChannelOrchestration | null> {
  ...
  console.log(`[ChannelOrchestration] STEP_1 | designing | primary=${args.primaryChannelName} | secondary=${args.secondaryChannelName} | mode=${args.channelMode}`);

  let raw = "";
  try {
    const r = await aiChat({ model: MODEL, messages: [{ role: "user", content: buildDesigner(args) }], temperature: 0.3, max_tokens: 1300, endpoint: "channel-orchestration", accountId: args.accountId });
  ...
  try {
    const jr = await aiChat({ model: MODEL, messages: [{ role: "user", content: buildJudge(JSON.stringify(o, null, 2)) }], temperature: 0.1, max_tokens: 400, endpoint: "channel-orchestration-judge", accountId: args.accountId });
  ...
  if (verdict === "REJECTED" && (reason || fix)) {
    ...
    try {
      const r2 = await aiChat({ model: MODEL, messages: [{ role: "user", content: buildDesigner({ ...args, judgeFeedback: fb }) }], temperature: 0.3, max_tokens: 1300, endpoint: "channel-orchestration-retry", accountId: args.accountId });
      const o2 = parseOrch(safeJson(r2.choices[0]?.message?.content?.trim() || ""), MODEL, 1, args.primaryChannelName, args.secondaryChannelName);
      if (o2) {
        o = o2;
        try {
          const jr2 = await aiChat({ model: MODEL, messages: [{ role: "user", content: buildJudge(JSON.stringify(o, null, 2)) }], temperature: 0.1, max_tokens: 400, endpoint: "channel-orchestration-judge-retry", accountId: args.accountId });
