import { ENGINE_VERSION, STATUS, CHANNEL_DEFINITIONS, AWARENESS_CHANNEL_MAP, PERSUASION_CHANNEL_COMPATIBILITY, PERSUASION_COMPATIBILITY_THRESHOLD, LAYER_WEIGHTS, BOUNDARY_HARD_PATTERNS, BOUNDARY_SOFT_PATTERNS, OBJECTIVE_FIT_MATRIX, CHANNEL_DIFFERENTIATION_PROFILES, ORGANIC_CHANNELS, PAID_CHANNELS, CHANNEL_FUNNEL_CAPABILITIES, FUNNEL_ROLE_THRESHOLDS, CONVERSION_INJECTION_PRIORITY, PERSUASION_CORRECTION_FLOOR, CHANNEL_SCALABILITY, CHANNEL_ROLE_REGISTRY, AWARENESS_STAGE_ALLOWED_ROLES, AWARENESS_BLOCKED_CHANNEL_TYPES, AWARENESS_INJECTION_PRIORITY, } from "./constants";
...
function assessDataReliability(audience, awareness, persuasion, offer, budget, validation) {
    const advisories = [];
    ...
    if (totalSignals < 3)
        advisories.push(`Low audience signal density: ${totalSignals} signals`);
    ...
    if (narrativeStability < 0.5)
        advisories.push("Weak upstream stability: awareness or persuasion data missing");
    ...
    const overallReliability = signalDensity * 0.25 +
    ...
    if (isWeak)
        advisories.push(`Data reliability WEAK (${overallReliability.toFixed(2)}) — confidence scores capped`);
    return {
        signalDensity,
        signalDiversity,
        narrativeStability,
...
function scoreAudienceDensity(audience, channelKey) {
    const findings = [];
    const warnings = [];
    const segmentCount = (audience.audienceSegments || []).length;
    const painCount = (audience.audiencePains || []).length;
    ...
    if (segmentCount === 0)
        warnings.push("No audience segments defined — channel targeting will be generic");
    if (painCount === 0)
        warnings.push("No audience pains identified — messaging alignment uncertain");
...
function enforceAwarenessConstraint(awareness, channelKey, channelDef) {
    const diagnostics = [];
    if (!awareness) {
...
function scoreAwarenessMapping(awareness, channelKey, channelDef) {
    const findings = [];
    const warnings = [];
    if (!awareness) {
        return {
...
function scorePersuasionCompatibility(persuasion, channelDef, channelKey) {
    const findings = [];
    const warnings = [];
    if (!persuasion) {
        return {
...
function getContentFormatBonus(channelType, persuasionMode) {
    ...
    if (["education_first", "trust_building"].includes(persuasionMode) && longFormChannels.includes(channelType))
        return 0.08;
    if (["proof_driven", "scarcity_urgency"].includes(persuasionMode) && visualChannels.includes(channelType))
        return 0.05;
    if (["proof_driven"].includes(persuasionMode) && intentChannels.includes(channelType))
        return 0.07;
    return 0;
}
function checkBudgetConstraint(budget, channelDef, channelKey) {
    const findings = [];
    const warnings = [];
...
function scoreCostEfficiency(channelDef, offer) {
    const findings = [];
    const warnings = [];
...
function assessRisk(validation, channelDef) {
    const findings = [];
    const warnings = [];
    let riskScore = 0.7;
    if (validation) {
...
function runGuardLayer(audience, persuasion, budget, validation, channelDef) {
    const findings = [];
    const warnings = [];
    let passed = true;
    const segmentCount = (audience.audienceSegments || []).length;
...
function determineBestFunnelRole(channelKey) {
    const cap = CHANNEL_FUNNEL_CAPABILITIES[channelKey];
    if (!cap)
        return { role: "awareness", score: 0.3 };
    const roles = [
...
function isPersuasionIncompatible(persuasionLayer) {
    return !persuasionLayer.passed || persuasionLayer.score < PERSUASION_COMPATIBILITY_THRESHOLD;
}
function attemptPersuasionCorrection(channelKey, persuasionLayer, persuasion) {
...
function runFunnelResolutionLayer(candidates, persuasion, awarenessStage = "problem_aware") {
    ...
    if (incompatibleChannels.length === 0) {
        ...
        const reconstructionLog = ["No persuasion incompatibilities detected — standard channel evaluation applied"];
    ...
    for (const role of ["awareness", "nurture", "conversion"]) {
...
function injectConversionChannel(funnelStages, candidates, persuasionMode, reconstructionLog, awarenessStage = "problem_aware") {
    ...
    for (const channelKey of injectionList) {
        const cap = CHANNEL_FUNNEL_CAPABILITIES[channelKey];
        if (!cap || cap.conversion < FUNNEL_ROLE_THRESHOLDS.conversion)
            continue;
        ...
        if (candidate && !candidate.guardLayer.passed)
            continue;
        if (candidate && !candidate.budgetLayer.passed)
            continue;
...
function runDecisionGate(persuasionLayer, guardLayer, budgetLayer, awarenessLayer, channelDef, channelKey) {
    const violations = [];
    const findings = [];
...
function computeDecisionGateScoring(funnelStages, candidates, budget) {
    ...
    let funnelIntegrityScore = clamp((hasAwareness ? 0.33 : 0) + (hasNurture ? 0.33 : 0) + (hasConversion ? 0.34 : 0), 0, 1);
    if (hasInjections) {
    ...
    let compositeGateScore = clamp(funnelIntegrityScore * 0.30 +
        persuasionAlignmentScore * 0.25 +
        budgetRealism * 0.25 +
        channelScalability * 0.20, 0, 1);
...
function getObjectiveFit(channelKey) {
    const fit = OBJECTIVE_FIT_MATRIX[channelKey];
    if (!fit)
        return { awarenessFit: 0.3, nurtureFit: 0.3, conversionFit: 0.3 };
    return { ...fit };
}
function getChannelDifferentiation(channelKey) {
...
function filterChannelsByMode(channelKeys, mode) {
...
function buildChannelCandidate(channelKey, layers, reliability, gateResult) {
    const def = CHANNEL_DEFINITIONS[channelKey];
    if (!def) {
        return {
...
export function runChannelSelectionEngine(audience, awareness, persuasion, offer, budget, validation, channelMode = "automatic", memoryContext) {
    const startTime = Date.now();
    const structuralWarnings = [];
    const memoryAdjustments = [];
    function applyMemoryAdjustments(candidateList) {
        if (!memoryContext)
            return;
        ...
        for (const line of lines) {
            if (line.includes("REINFORCE") || line.includes("Reinforce") || line.includes("reinforce"))
                mode = "reinforce";
            else if (line.includes("AVOID") || line.includes("Avoid") || line.includes("avoid"))
                mode = "avoid";
            else if (mode === "reinforce")
                reinforceLines.push(line.toLowerCase());
            else if (mode === "avoid")
                avoidLines.push(line.toLowerCase());
        }
        for (const c of candidateList) {
            if (c.candidate.rejectionReason)
                continue;
    ...
    const candidates = [];
    const preFunnelData = [];
    const awarenessBlockedChannels = [];
    ...
    for (const channelKey of channelKeys) {
        ...
        if (awarenessBlocked) {
            ...
            const rejectedCandidate = buildChannelCandidate(channelKey, [awarenessLayer], reliability, {
                outcome: "exploratory",
        ...
        const { layer: decisionGateLayer, gate: gateResult } = runDecisionGate(persuasionLayer, guardLayer, budgetLayer, awarenessLayer, def, channelKey);
        const coreLayers = [
            audienceLayer,
            awarenessLayer,
            persuasionLayer,
    ...
    if (awarenessBlockedChannels.length > 0) {
        structuralWarnings.push(`AWARENESS CONSTRAINT ENFORCEMENT: ${awarenessBlockedChannels.length} channel(s) blocked for "${awarenessStage}" audience stage: ${awarenessBlockedChannels.map(c => c.channelLabel).join(", ")}`);
    }
        ...
        for (const c of candidates) {
            if (rescuedChannels.has(c.key)) {
                ...
                if (assignment) {
                    ...
                    const postReconstructionViolations = [];
                    ...
                    const postGateOutcome = postReconstructionViolations.length === 0 ? "support_channel" : "exploratory";
...
function buildFallbackChannel(role) {
    return {
