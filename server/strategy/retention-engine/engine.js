...
import { ENGINE_VERSION, RETENTION_SCORE_WEIGHTS, GUARD_THRESHOLDS, BOUNDARY_BLOCKED_PATTERNS, STATUS, } from "./constants";
import { sanitizeBoundary, normalizeConfidence, assessDataReliability, detectGenericOutput, } from "../../engine-hardening";
...
function runGuardLayer(input) {
    const flags = [];
    const hasRawInputs = !!input.customerJourneyData?.rawInputs;
    const rawInputs = input.customerJourneyData?.rawInputs;
    const derivedMetrics = input.customerJourneyData?.derivedMetrics;
...
function buildRetentionPrompt(input) {
    const journey = input.customerJourneyData;
    ...
    const rawInputs = journey.rawInputs;
    const derivedMetrics = journey.derivedMetrics;
...
function buildFallbackResult(input, guardResult, startTime) {
...
export async function runRetentionEngine(input) {
    const startTime = Date.now();
    const warnings = [];
    const guardResult = runGuardLayer(input);
    // Short-circuit when there is no real data — skip the AI call entirely
    const hasAnySparseData = (input.customerJourneyData.touchpoints.length === 0 &&
    ...
    const hasRawInputs = !!input.customerJourneyData?.rawInputs;
    const rawInputs = input.customerJourneyData?.rawInputs;
    const rawSignalBoost = hasRawInputs ? ((rawInputs.totalCustomers > 0 ? 3 : 0) +
        (rawInputs.totalPurchases > 0 ? 2 : 0) +
        (rawInputs.returningCustomers > 0 ? 2 : 0) +
        (rawInputs.dataWindowDays > 0 ? 1 : 0)) : 0;
    const reliability = assessDataReliability(input.customerJourneyData.touchpoints.length + (hasRawInputs ? 2 : 0), input.purchaseMotivations.length + input.postPurchaseObjections.length + rawSignalBoost, input.offerStructure.coreOutcome !== null || (hasRawInputs && rawInputs.totalCustomers > 0), input.offerStructure.deliverables.length > 0 || (hasRawInputs && rawInputs.totalPurchases > 0), input.purchaseMotivations.length > 0 || hasRawInputs, guardResult.retentionMechanismPresence);
    ...
    let retentionLoops = [];
    let churnRiskFlags = [];
    let ltvExpansionPaths = [];
    let upsellTriggers = [];
    let finalBoundaryCheck = { clean: true, violations: [] };
    try {
        ...
        const response = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error(`RetentionEngine AI call timed out after ${AI_CALL_TIMEOUT_MS / 1000}s`)), AI_CALL_TIMEOUT_MS)),
        ]);
        ...
        if (parsed) {
            retentionLoops = Array.isArray(parsed.retentionLoops)
                ? parsed.retentionLoops.map((loop) => ({
            ...
            churnRiskFlags = Array.isArray(parsed.churnRiskFlags)
                ? parsed.churnRiskFlags.map((flag) => ({
            ...
            ltvExpansionPaths = Array.isArray(parsed.ltvExpansionPaths)
                ? parsed.ltvExpansionPaths.map((path) => ({
            ...
            upsellTriggers = Array.isArray(parsed.upsellTriggers)
                ? parsed.upsellTriggers.map((trigger, idx) => ({
    ...
    let rawConfidence = guardResult.valueDeliveryClarity * RETENTION_SCORE_WEIGHTS.valueDeliveryClarity +
    ...
    const acceptability = assessStrategyAcceptability(confidenceScore, layersPassed, 5, guardResult.passed, warnings);
    if (!finalBoundaryCheck.clean) {
        const fallback = buildFallbackResult(input, guardResult, startTime);
        return {
            ...fallback,
