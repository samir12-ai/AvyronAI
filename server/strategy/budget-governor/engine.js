import { ENGINE_VERSION, MIN_VALIDATION_CONFIDENCE_FOR_SCALE, MIN_OFFER_PROOF_FOR_SCALE, MIN_FUNNEL_STRENGTH_FOR_SCALE, CAC_DEVIATION_THRESHOLD, MAX_RISK_FOR_EXPANSION, TEST_BUDGET_FLOOR, TEST_BUDGET_CEILING, SCALE_BUDGET_FLOOR, SCALE_BUDGET_CEILING, KILL_THRESHOLDS, RISK_WEIGHTS, STATUS, PERFORMANCE_OVERRIDE_THRESHOLDS, } from "./constants";
import { assessStrategyAcceptability } from "../../shared/strategy-acceptability";
function clamp(v, min = 0, max = 1) {
    return Math.max(min, Math.min(max, v));
}
function reconcileValidationWithPerformance(baseValidationConfidence, perf) {
    const diagnostics = [];
    if (!perf) {
...
function computeRiskScore(input) {
    ...
    if (validationRisk > 0.5)
        riskFactors.push(`Low validation confidence (${(input.validationConfidence * 100).toFixed(0)}%)`);
    const offerRisk = 1 - clamp(input.offerStrength);
    if (offerRisk > 0.5)
        riskFactors.push(`Weak offer strength (${(input.offerStrength * 100).toFixed(0)}%)`);
    const funnelRisk = 1 - clamp(input.funnelStrengthScore);
    if (funnelRisk > 0.5)
        riskFactors.push(`Weak funnel strength (${(input.funnelStrengthScore * 100).toFixed(0)}%)`);
    const channelRiskScore = clamp(input.channelRisk);
    if (channelRiskScore > 0.5)
        riskFactors.push(`High channel risk (${(input.channelRisk * 100).toFixed(0)}%)`);
    ...
    if (cacDeviation > CAC_DEVIATION_THRESHOLD)
        riskFactors.push(`CAC assumptions deviate ${(cacDeviation * 100).toFixed(0)}% from historical`);
    const marketRisk = clamp(input.marketIntensity);
    if (marketRisk > 0.7)
        riskFactors.push(`High market intensity (${(input.marketIntensity * 100).toFixed(0)}%)`);
    const overallRisk = clamp(validationRisk * RISK_WEIGHTS.validationConfidence +
    ...
    if (input.offerCompleteness)
        mitigations.push("Offer is structurally complete");
    if (input.validationConfidence > 0.7)
        mitigations.push("Strong validation confidence provides safety margin");
    if (input.historicalROAS && input.historicalROAS > 2.0)
        mitigations.push("Historical ROAS above 2.0x reduces risk");
    if (input.funnelFrictionScore < 0.3)
        mitigations.push("Low funnel friction reduces drop-off risk");
    return { overallRisk, riskFactors, mitigations };
}
function runGuard(input, riskScore) {
    const violations = [];
    const warnings = [];
    const overrides = [];
...
function determineBudgetDecision(input, riskScore, guardResult) {
...
function computeTestBudgetRange(input, riskScore) {
    const confidenceFactor = clamp(input.validationConfidence, 0.2, 1.0);
    const riskAdjustment = 1 - (riskScore * 0.4);
    const recommended = Math.round(TEST_BUDGET_FLOOR + (TEST_BUDGET_CEILING - TEST_BUDGET_FLOOR) * confidenceFactor * riskAdjustment);
    return {
...
function computeScaleBudgetRange(input, riskScore) {
    if (riskScore > MAX_RISK_FOR_EXPANSION) {
        return {
            min: 0,
            max: 0,
    ...
    const recommended = Math.round(SCALE_BUDGET_FLOOR + (SCALE_BUDGET_CEILING - SCALE_BUDGET_FLOOR) * combinedFactor);
    return {
...
function computeExpansionPermission(input, riskScore, guardResult) {
    const blockers = [];
    const conditions = [];
    if (!guardResult.passed) {
...
function assessCACRealism(input) {
...
export function runBudgetGovernorEngine(input) {
    const startTime = Date.now();
    const structuralWarnings = [];
    try {
        const reconciliation = reconcileValidationWithPerformance(input.validationConfidence, input.campaignPerformance);
        if (reconciliation.reconciledConfidence !== input.validationConfidence) {
            structuralWarnings.push(`CONFIDENCE RECONCILIATION: base=${(input.validationConfidence * 100).toFixed(0)}% → reconciled=${(reconciliation.reconciledConfidence * 100).toFixed(0)}%` +
        ...
        const killReasons = [];
        if (killFlag) {
            if (reconciledInput.offerStrength < KILL_THRESHOLDS.minOfferStrength)
                killReasons.push("Offer strength below minimum viable threshold");
            if (reconciledInput.validationConfidence < KILL_THRESHOLDS.minValidationConfidence)
                killReasons.push("Validation confidence critically low");
            if (overallRisk > KILL_THRESHOLDS.maxRisk)
                killReasons.push("Overall risk exceeds maximum safe threshold");
        }
        let layersPassed = 0;
        const totalLayers = 6;
        if (guardResult.passed)
            layersPassed++;
        if (cacAssumptionCheck.realistic)
            layersPassed++;
        if (overallRisk < MAX_RISK_FOR_EXPANSION)
            layersPassed++;
        if (reconciledInput.offerCompleteness)
            layersPassed++;
        if (reconciledInput.validationConfidence >= 0.5)
            layersPassed++;
        if (reconciliation.overrideApplied || reconciledInput.validationConfidence >= MIN_VALIDATION_CONFIDENCE_FOR_SCALE)
            layersPassed++;
        const rawConfidence = clamp(reconciledInput.validationConfidence * 0.3 +
        ...
        const strategyAcceptability = assessStrategyAcceptability(confidenceScore, layersPassed, totalLayers, guardResult.passed, structuralWarnings);
