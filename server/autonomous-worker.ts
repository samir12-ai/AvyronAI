...
const MAX_DECISIONS_PER_HOUR = 9;
...
async function checkHourlyDecisionCap(accountId: string): Promise<{ allowed: boolean; count: number }> {
  ...
  return { allowed: recent.length < MAX_DECISIONS_PER_HOUR, count: recent.length };
}

async function checkCircuitBreaker(accountId: string): Promise<{ tripped: boolean; failures: number }> {
  ...
  return { tripped: failures >= CIRCUIT_BREAKER_THRESHOLD, failures };
}

async function checkIdleAccount(accountId: string): Promise<boolean> {
  const threshold = new Date(Date.now() - IDLE_SKIP_DAYS * 24 * 60 * 60 * 1000);
...
async function processAccount(accountId: string) {
  ...
  try {
    const isIdle = await checkIdleAccount(accountId);
    if (isIdle) {
      console.log(`[Worker] Account ${accountId} idle for ${IDLE_SKIP_DAYS}d, skipping`);
      await releaseLock(jobId, "completed");
      await logAudit(accountId, "JOB_SKIPPED", { details: { reason: "idle_account", threshold_days: IDLE_SKIP_DAYS } });
      return;
    }

    const circuitBreaker = await checkCircuitBreaker(accountId);
    if (circuitBreaker.tripped) {
      console.log(`[Worker] Circuit breaker tripped for ${accountId} (${circuitBreaker.failures} consecutive failures)`);
      await releaseLock(jobId, "completed");
      await logAudit(accountId, "JOB_SKIPPED", { details: { reason: "circuit_breaker", consecutive_failures: circuitBreaker.failures } });
      return;
    }

    const hourlyCheck = await checkHourlyDecisionCap(accountId);
    if (!hourlyCheck.allowed) {
      console.log(`[Worker] Hourly cap reached for ${accountId} (${hourlyCheck.count}/${MAX_DECISIONS_PER_HOUR})`);
      await releaseLock(jobId, "completed");
      await logAudit(accountId, "JOB_SKIPPED", { details: { reason: "hourly_cap", decisions_this_hour: hourlyCheck.count } });
      return;
    }
