...
export async function applyMemoryMutation(campaignId, accountId, entries, planId) {
    let written = 0;
    let updated = 0;
    let decayed = 0;
    for (const entry of entries) {
        ...
        const existing = await db
            .select()
            .from(strategyMemory)
            .where(and(eq(strategyMemory.accountId, accountId), eq(strategyMemory.campaignId, campaignId), eq(strategyMemory.strategyFingerprint, fingerprint)))
            .limit(1);
...
async function applyConfidenceDecay(campaignId, accountId) {
    const staleThreshold = new Date(Date.now() - DECAY_HALF_LIFE_DAYS * 24 * 60 * 60 * 1000);
    const staleRows = await db
        .select()
        .from(strategyMemory)
        .where(and(eq(strategyMemory.accountId, accountId), eq(strategyMemory.campaignId, campaignId), lt(strategyMemory.updatedAt, staleThreshold)));
    let decayed = 0;
    for (const row of staleRows) {
        if (!row.updatedAt || (row.score ?? 0) < DECAY_THRESHOLD)
            continue;
...
export async function recordWinnerMemory(campaignId, accountId, entry, planId) {
    await applyMemoryMutation(campaignId, accountId, [{ ...entry, isWinner: true, confidenceScore: 0.75 }], planId);
}
export async function recordAvoidMemory(campaignId, accountId, entry, planId) {
