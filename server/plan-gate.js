...
export function resolveArchetype(businessType) {
    const lower = (businessType || "").toLowerCase();
    if (lower.includes("agency") || lower.includes("service") || lower.includes("consulting")) {
        if (lower.includes("high") || lower.includes("premium") || lower.includes("luxury"))
            return ARCHETYPES.high_ticket;
        if (lower.includes("local"))
            return ARCHETYPES.local_service;
        return ARCHETYPES.agency;
    }
    if (lower.includes("ecommerce") || lower.includes("shop") || lower.includes("store") || lower.includes("retail"))
        return ARCHETYPES.ecommerce;
    if (lower.includes("restaurant") || lower.includes("food") || lower.includes("cafe") || lower.includes("bar"))
        return ARCHETYPES.restaurant;
    if (lower.includes("saas") || lower.includes("software") || lower.includes("tech") || lower.includes("app"))
        return ARCHETYPES.saas;
    if (lower.includes("local"))
        return ARCHETYPES.local_service;
    return ARCHETYPES.agency;
}
export async function checkPlanReadiness(campaignId, accountId) {
    ...
    if (!bizData?.coreOffer) {
        businessClarity -= 20;
        missingItems.push("Core offer/service");
    }
    if (!bizData?.targetAudienceSegment && !bizData?.targetAudienceAge) {
        businessClarity -= 20;
        missingItems.push("Target audience");
    }
    ...
    if (!campaign?.objective && !bizData?.funnelObjective) {
        goalSpecificity -= 40;
        missingItems.push("Primary goal/objective");
    }
    ...
    if (!bizData?.funnelObjective && !campaign?.objective) {
        funnelViability -= 25;
    }
    ...
    if (!bizData?.monthlyBudget) {
        ...
        if (budget < 100) {
            budgetFeasibility -= 20;
            missingItems.push("Budget too low for meaningful paid activity");
        }
    }
    ...
    const overallScore = Math.round((businessClarity * 0.25 + goalSpecificity * 0.25 + funnelViability * 0.15 + budgetFeasibility * 0.15 + executionReadiness * 0.1 + dataConfidence * 0.1));
    let gate = "PASS";
    if (overallScore < 40 || !bizData?.businessType)
        gate = "BLOCKED";
    else if (overallScore < 70 || assumptions.length > 0)
...
export function registerPlanGateRoutes(app) {
    app.post("/api/plan-gate/check", async (req, res) => {
        try {
    ...
    app.get("/api/plan-gate/:campaignId", async (req, res) => {
        try {
            const accountId = resolveAccountId(req);
