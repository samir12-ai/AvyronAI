import { ENGINE_VERSION, STATUS, BOUNDARY_HARD_PATTERNS, BOUNDARY_SOFT_PATTERNS, LAYER_WEIGHTS, GENERIC_PHRASES, } from "./constants";
...
export function assessDataReliability(mi, audience, positioning, differentiation, offer) {
    const advisories = [];
    ...
    if (sourceDiversity.size < 3)
        advisories.push(`Low signal diversity: only ${sourceDiversity.size}/8 data sources populated`);
    ...
    if (narrativeStability < 0.5)
        advisories.push("Weak narrative stability: positioning or differentiation incomplete");
    const miConfidence = safeNumber(mi.overallConfidence, 0);
    const competitorValidity = clamp(miConfidence, 0, 1);
    if (miConfidence < 0.4)
        advisories.push(`Low competitor data validity: MI confidence ${miConfidence.toFixed(2)}`);
    ...
    if (isWeak)
        advisories.push(`Data reliability is WEAK (${overallReliability.toFixed(2)}) — confidence will be capped`);
    return {
        signalDensity,
        signalDiversity,
        narrativeStability,
...
export function normalizeConfidence(rawScore, reliability) {
...
function detectMarketMaturity(mi, audience) {
    const maturity = safeNumber(audience.maturityIndex, 0.5);
    if (maturity < 0.3)
        return "emerging";
    if (maturity < 0.6)
...
function detectProblemIntensity(audience) {
...
export function layer1_marketEntryDetection(audience, mi, positioning, differentiation) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
...
export function layer2_awarenessReadinessMapping(audience, mi) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
...
export function layer3_attentionTriggerMapping(audience, mi, differentiation) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    const pains = audience.audiencePains || [];
    ...
    if (multiSource && typeof multiSource === "object") {
        for (const compData of Object.values(multiSource)) {
            websiteHeadlineCount += (compData?.website?.headlineExtractions || []).length;
            blogTopicCount += (compData?.blog?.topicClusters || []).length;
        }
    }
...
export function layer4_narrativeEntryAlignment(positioning, differentiation, offer, funnel, entryRoute) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
...
export function layer5_awarenessFunnelFit(funnel, entryRoute, readinessStage) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
...
export function layer6_trustReadinessGuard(audience, mi, entryRoute) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    const trustLevel = detectTrustLevel(audience, mi);
...
export function layer7_genericAwarenessDetector(entryDescription, routeName, triggerClass) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
...
export function layer8_awarenessStrengthScoring(layerResults, audience, positioning, funnel) {
    const findings = [];
    const warnings = [];
    const dimensions = {
...
function extractSelected(layerResult, fallback) {
...
function selectEntryRoute(layer1) {
    return extractSelected(layer1, "");
}
function selectReadinessStage(layer2) {
    return extractSelected(layer2, "problem_aware");
}
function selectTriggerClass(layer3) {
    return extractSelected(layer3, "");
}
function buildAlternativeRoute(primaryEntry, audience, mi) {
    const alternatives = {
...
function buildRejectedRoute(primaryEntry, audience, mi) {
    const trustLevel = detectTrustLevel(audience, mi);
    const readiness = detectBuyerReadiness(audience);
...
function buildTrustRequirement(entryRoute, trustLevel) {
    if (trustLevel < 0.3)
        return "high — proof and authority signals required before engagement";
    if (trustLevel < 0.5)
        return "moderate — some trust-building needed alongside entry mechanism";
    if (entryRoute === "authority_entry" || entryRoute === "proof_led_entry")
        return "moderate — entry already builds trust";
    return "low — audience has adequate baseline trust";
}
function buildFunnelCompatibility(entryRoute, funnel) {
...
export async function runAwarenessEngine(mi, audience, positioning, differentiation, offer, accountId, upstreamLineage = [], funnel = EMPTY_FUNNEL, analyticalEnrichment) {
    ...
    if (boundary.sanitized && boundary.warnings.length > 0) {
        ...
        primaryRoute.frictionNotes = primaryRoute.frictionNotes.map((n) => applySoftSanitization(n, BOUNDARY_SOFT_PATTERNS));
        ...
        alternativeRoute.frictionNotes = alternativeRoute.frictionNotes.map((n) => applySoftSanitization(n, BOUNDARY_SOFT_PATTERNS));
    }
    for (const lr of layerResults) {
    ...
    const celDepth = enforceEngineDepthCompliance("awareness", [
...
function emptyRoute(name, reason = null) {
    return {
        routeName: name,
        entryMechanismType: "",
