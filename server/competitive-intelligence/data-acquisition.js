...
import { scrapeInstagramProfile, scrapeCommentsForPosts, extractHandleFromUrl } from "./profile-scraper";
...
export function detectCTAIntent(caption) {
    const emptyResult = {
        ctaIntentScore: 0,
    ...
    for (const { pattern, label } of NARRATIVE_CTA_PATTERNS) {
        if (pattern.test(trimmed))
            narrativeMatches.push(label);
    }
    for (const { pattern, label } of TRUST_CTA_PATTERNS) {
        if (pattern.test(trimmed))
            trustMatches.push(label);
    }
    ...
    const weightedScore = explicitMatches.length * explicitWeight +
    ...
    if (explicitMatches.length > 0)
        explanationParts.push(`Explicit(${explicitMatches.join(",")})`);
    if (softMatches.length > 0)
        explanationParts.push(`Soft(${softMatches.join(",")})`);
    if (narrativeMatches.length > 0)
        explanationParts.push(`Narrative(${narrativeMatches.join(",")})`);
    if (trustMatches.length > 0)
        explanationParts.push(`Trust(${trustMatches.join(",")})`);
    const explanation = explanationParts.length > 0
        ? `Intent detected: ${explanationParts.join(" | ")}`
        : "No CTA intent patterns detected";
    const legacyCtaTypes = [];
    if (explicitMatches.length > 0)
        legacyCtaTypes.push(...explicitMatches);
    if (softMatches.length > 0)
        legacyCtaTypes.push(...softMatches);
    if (narrativeMatches.length > 0)
        legacyCtaTypes.push(...narrativeMatches);
    if (trustMatches.length > 0)
        legacyCtaTypes.push(...trustMatches);
    return {
        ctaIntentScore,
        ctaTypeDistribution,
...
export async function fetchCompetitorData(competitorId, accountId = "default", forceRefresh = false, proxyCtx, collectionMode = "FAST_PASS", fetchOptions) {
...
async function _executeFetch(competitorId, accountId, forceRefresh, proxyCtx, collectionMode = "FAST_PASS", fetchOptions) {
    ...
    if (!forceRefresh && collectionMode !== "DEEP_PASS" && normalizedHandle) {
        ...
        if (sharedLookup.found && !sharedLookup.isStale && sharedLookup.sharedProfileId) {
            const reuseResult = await reuseFromSharedPool(competitorId, accountId, sharedLookup.sharedProfileId, sharedLookup);
            if (reuseResult) {
    ...
    if (!forceRefresh && collectionMode !== "DEEP_PASS") {
        ...
        if (latestMetrics.length > 0 && latestMetrics[0].lastFetchAt) {
            ...
            if (elapsed < CACHE_REUSE_WINDOW_MS) {
                const livePostCount = await db.select({ count: sql `count(*)` }).from(ciCompetitorPosts)
                ...
                if (cachedPosts >= MIN_POSTS_THRESHOLD) {
                    const liveCommentCount = await db.select({ count: sql `count(*)` }).from(ciCompetitorComments)
    ...
    const scrapeMode = fetchOptions?.scrapeMode ?? "INITIAL";
    ...
    const cutoffDate = isIncremental
        ? (() => {
            ...
            if (watermark && watermark > windowCutoff)
                return watermark;
            return windowCutoff;
        })()
        : null;
    ...
    if (collectionMode === "DEEP_PASS") {
        const storedPostCount = await db.select({ count: sql `count(*)` }).from(ciCompetitorPosts)
            .where(and(eq(ciCompetitorPosts.competitorId, competitorId), eq(ciCompetitorPosts.accountId, accountId)));
        ...
        if (existingStoredPostTotal >= TARGET_POSTS_DEEP) {
            ...
            const existingCommentCount = await db.select({ count: sql `count(*)` }).from(ciCompetitorComments)
                .where(and(eq(ciCompetitorComments.competitorId, competitorId), eq(ciCompetitorComments.accountId, accountId)));
    ...
    const existingPostCount = await db.select({ count: sql `count(*)` }).from(ciCompetitorPosts)
        .where(and(eq(ciCompetitorPosts.competitorId, competitorId), eq(ciCompetitorPosts.accountId, accountId)));
    ...
    const existingPostIdsWithComments = new Set();
    if (existingPostIds.size > 0) {
        ...
        const postsWithComments = await db.select({
            ...
            .where(and(eq(ciCompetitorComments.competitorId, competitorId), eq(ciCompetitorComments.accountId, accountId), inArray(ciCompetitorComments.postId, postIdsArray)));
    ...
    if (realComments.length > 0) {
        ...
        for (const r of existingCRows) {
            if (r.commentId)
                existingCommentIds.add(r.commentId);
        }
    }
    let realCommentsInserted = 0;
    for (const rc of realComments) {
        if (existingCommentIds.has(rc.commentId))
            continue;
    ...
    if (postsWithTimestamps.length >= 4) {
        const sorted = postsWithTimestamps.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    ...
    const verifyPosts = await db.select({ count: sql `count(*)` }).from(ciCompetitorPosts)
        .where(and(eq(ciCompetitorPosts.competitorId, competitorId), eq(ciCompetitorPosts.accountId, accountId)));
    const verifyComments = await db.select({ count: sql `count(*)` }).from(ciCompetitorComments)
        .where(and(eq(ciCompetitorComments.competitorId, competitorId), eq(ciCompetitorComments.accountId, accountId)));
    ...
    if (coverageSufficient) {
        ...
        const missing = [];
        if (persistedPostCount < postsTarget)
            missing.push(`posts: ${persistedPostCount}/${postsTarget}`);
    ...
    const confirmedBatchPosts = postInserts.length > 0
        ? await db.select({ timestamp: ciCompetitorPosts.timestamp })
            .from(ciCompetitorPosts)
            .where(and(eq(ciCompetitorPosts.competitorId, competitorId), eq(ciCompetitorPosts.accountId, accountId), eq(ciCompetitorPosts.batchId, batchId)))
        : [];
    const confirmedTimestamps = confirmedBatchPosts
        .map(p => p.timestamp)
        .filter((t) => t instanceof Date);
    const newWatermark = confirmedTimestamps.length > 0
        ? new Date(Math.max(...confirmedTimestamps.map(t => t.getTime())))
        : null;
    console.log(`[DataAcq] SCRAPE_COMPLETE | competitor=${competitor.name}` +
...
function generateSyntheticCommentSamples(post) {
    const samples = [];
    ...
    const targetSamples = Math.min(MAX_COMMENTS_PER_POST, Math.max(MIN_SYNTHETIC_COMMENTS_PER_POST, samples.length, Math.ceil(reportedComments * 0.3)));
...
export async function enrichCompetitorWithComments(competitorId, accountId, options) {
    const [competitor] = await db.select().from(ciCompetitors)
        .where(and(eq(ciCompetitors.id, competitorId), eq(ciCompetitors.accountId, accountId)));
    if (!competitor) {
    ...
    const realCommentCount = await db.select({ count: sql `count(*)` }).from(ciCompetitorComments)
    ...
    const postsWithComments = await db.select({ postId: ciCompetitorComments.postId })
        .from(ciCompetitorComments)
        .where(and(eq(ciCompetitorComments.competitorId, competitorId), eq(ciCompetitorComments.accountId, accountId), inArray(ciCompetitorComments.postId, postIds)));
    for (const row of postsWithComments) {
        existingPostIdsWithComments.add(row.postId);
    }
    ...
    try {
        ...
        if (embeddedComments.length > 0) {
            ...
            for (const r of existingCRows) {
                if (r.commentId)
                    existingCIds.add(r.commentId);
            }
            for (const rc of cleanEmbedded) {
                if (existingCIds.has(rc.commentId))
                    continue;
                commentInserts.push({
                    competitorId,
                    accountId,
    ...
    for (const ci of commentInserts) {
        const pid = ci.postId;
        if (!postDistribution[pid])
            postDistribution[pid] = { real: 0, synthetic: 0 };
        if (ci.isSynthetic)
            postDistribution[pid].synthetic++;
        else
            postDistribution[pid].real++;
    }
...
export async function getCompetitorDataCoverage(competitorId, accountId = "default") {
    const [competitor] = await db.select().from(ciCompetitors)
        .where(and(eq(ciCompetitors.id, competitorId), eq(ciCompetitors.accountId, accountId)));
    const postsResult = await db.select({ count: sql `count(*)` }).from(ciCompetitorPosts)
        .where(and(eq(ciCompetitorPosts.competitorId, competitorId), eq(ciCompetitorPosts.accountId, accountId)));
    const totalCommentsResult = await db.select({ count: sql `count(*)` }).from(ciCompetitorComments)
        .where(and(eq(ciCompetitorComments.competitorId, competitorId), eq(ciCompetitorComments.accountId, accountId)));
    const realCommentsResult = await db.select({ count: sql `count(*)` }).from(ciCompetitorComments)
        .where(and(eq(ciCompetitorComments.competitorId, competitorId), eq(ciCompetitorComments.accountId, accountId), eq(ciCompetitorComments.isSynthetic, false)));
    const latestMetrics = await db.select().from(ciCompetitorMetricsSnapshot)
        .where(and(eq(ciCompetitorMetricsSnapshot.competitorId, competitorId), eq(ciCompetitorMetricsSnapshot.accountId, accountId)))
        .orderBy(desc(ciCompetitorMetricsSnapshot.createdAt))
...
export async function getStoredPostsForMIv3(competitorId, accountId = "default") {
...
export async function getStoredCommentsForMIv3(competitorId, accountId = "default") {
...
function computeCompetitorHash(profileLink) {
...
function isSyntheticCooldownActive(lastEnrichmentAt) {
    if (!lastEnrichmentAt)
        return false;
...
function detectHighSyntheticChurn(enrichmentCount, lastSyntheticEnrichmentAt, firstEnrichmentApproxAt) {
    if (enrichmentCount <= SYNTHETIC_CHURN_THRESHOLD)
        return false;
    if (!lastSyntheticEnrichmentAt)
        return false;
...
export async function cleanupExpiredSyntheticComments() {
    ...
    const totalSyntheticResult = await db.select({ count: sql `count(*)` }).from(ciCompetitorComments)
    ...
    const expiredRows = await db.select({
        ...
        .where(and(eq(ciCompetitorComments.isSynthetic, true), sql `${ciCompetitorComments.createdAt} < ${cutoffDate}`));
    ...
    const affectedCompetitors = new Map();
  ...
  `);
    const deleted = Number(deleteResult.rowCount || expiredRows.length);
    ...
    for (const [competitorId, accountId] of affectedCompetitors.entries()) {
        ...
        if (!comp || !comp.isActive)
            continue;
        const realCount = await db.select({ count: sql `count(*)` }).from(ciCompetitorComments)
            .where(and(eq(ciCompetitorComments.competitorId, competitorId), eq(ciCompetitorComments.accountId, accountId), eq(ciCompetitorComments.isSynthetic, false)));
        ...
        const remainingCount = await db.select({ count: sql `count(*)` }).from(ciCompetitorComments)
            .where(and(eq(ciCompetitorComments.competitorId, competitorId), eq(ciCompetitorComments.accountId, accountId)));
        const remaining = Number(remainingCount[0]?.count || 0);
        if (remaining >= MIN_COMMENTS_THRESHOLD) {
            continue;
        }
        const hasPosts = await db.select({ count: sql `count(*)` }).from(ciCompetitorPosts)
            .where(and(eq(ciCompetitorPosts.competitorId, competitorId), eq(ciCompetitorPosts.accountId, accountId)));
...
export async function fetchAllCompetitors(accountId = "default", campaignId) {
