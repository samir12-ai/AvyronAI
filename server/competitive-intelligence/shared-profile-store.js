import { db } from "../db";
import { ciSharedProfiles, ciCompetitors, ciCompetitorPosts, ciCompetitorComments, ciCompetitorMetricsSnapshot, } from "@shared/schema";
import { eq, and, desc, sql } from "drizzle-orm";
export const SHARED_REUSE_WINDOW_MS = 12 * 60 * 60 * 1000;
export async function lookupSharedProfile(platform, normalizedHandle, minPostsThreshold) {
    const [profile] = await db
        .select()
        .from(ciSharedProfiles)
        .where(and(eq(ciSharedProfiles.platform, platform), eq(ciSharedProfiles.normalizedHandle, normalizedHandle)))
        .limit(1);
...
export async function upsertSharedProfile(platform, normalizedHandle, data) {
...
export async function linkCompetitorToSharedProfile(competitorId, sharedProfileId) {
    await db
        .update(ciCompetitors)
        .set({ sharedProfileId, updatedAt: new Date() })
        .where(eq(ciCompetitors.id, competitorId));
}
export async function reuseFromSharedPool(targetCompetitorId, targetAccountId, sharedProfileId, sharedProfile) {
    ...
    const existingPostCount = await db
        ...
        .where(and(eq(ciCompetitorPosts.competitorId, targetCompetitorId), eq(ciCompetitorPosts.accountId, targetAccountId)));
    ...
    const sourcePosts = await db
        .select()
        .from(ciCompetitorPosts)
        .where(and(eq(ciCompetitorPosts.competitorId, sourceCompetitor.id), eq(ciCompetitorPosts.accountId, sourceCompetitor.accountId)));
    ...
    const sourceComments = await db
        ...
        .where(and(eq(ciCompetitorComments.competitorId, sourceCompetitor.id), eq(ciCompetitorComments.accountId, sourceCompetitor.accountId)));
    ...
    const latestSourceMetrics = await db
        ...
        .where(and(eq(ciCompetitorMetricsSnapshot.competitorId, sourceCompetitor.id), eq(ciCompetitorMetricsSnapshot.accountId, sourceCompetitor.accountId)))
    ...
    const verifyComments = await db
        .select({ count: sql `count(*)` })
        .from(ciCompetitorComments)
        .where(and(eq(ciCompetitorComments.competitorId, targetCompetitorId), eq(ciCompetitorComments.accountId, targetAccountId)));
    ...
    const hoursAgo = sharedProfile.lastScrapedAt
        ? Math.round((Date.now() - sharedProfile.lastScrapedAt.getTime()) / (60 * 60 * 1000) * 10) / 10
        : 0;
    console.log(`[SharedPool] SHARED_REUSE: copied ${finalPostCount} posts + ${finalCommentCount} comments from ${sourceCompetitor.id} → ${targetCompetitorId} (scraped ${hoursAgo}h ago)`);
    return {
        postsCollected: finalPostCount,
        commentsCollected: finalCommentCount,
        followers: sharedProfile.followers ?? null,
...
export async function getStaleSharedProfiles(maxAgeHours = 12) {
    const cutoffMs = Date.now() - maxAgeHours * 60 * 60 * 1000;
    const cutoff = new Date(cutoffMs);
    const staleProfiles = await db
        .select()
        .from(ciSharedProfiles)
        .where(sql `${ciSharedProfiles.lastScrapedAt} IS NOT NULL AND ${ciSharedProfiles.lastScrapedAt} < ${cutoff.toISOString()}`);
    const results = [];
    for (const profile of staleProfiles) {
        const linked = await db
            ...
            .from(ciCompetitors)
            .where(and(eq(ciCompetitors.sharedProfileId, profile.id), eq(ciCompetitors.isActive, true)));
        if (linked.length === 0)
            continue;
...
export async function fanOutSharedReuse(sharedProfileId, sourceCompetitorId, sourceAccountId) {
    const linked = await db
        .select({
        id: ciCompetitors.id,
        accountId: ciCompetitors.accountId,
        ...
        .from(ciCompetitors)
        .where(and(eq(ciCompetitors.sharedProfileId, sharedProfileId), eq(ciCompetitors.isActive, true)));
