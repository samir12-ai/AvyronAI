...
function getBucket(accountId: string, campaignId: string): TokenBucket {
...
export async function acquireToken(accountId: string, campaignId: string, stageName: string): Promise<void> {
...
export function getBucketState(accountId: string, campaignId: string): {
  tokens: number;
  maxBurst: number;
  totalConsumed: number;
  totalWaited: number;
...
export function resetBucket(accountId: string, campaignId: string): void {
  const key = `${accountId}:${campaignId}`;
  buckets.delete(key);
}
