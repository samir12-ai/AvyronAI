...
function getDominanceLevel(score) {
    if (score >= 80)
        return "DOMINANT";
    if (score >= 65)
        return "STRUCTURALLY_STRONG";
    if (score >= 50)
        return "EFFICIENT";
    if (score >= 30)
        return "EXPOSED";
...
function identifyStrengths(signal) {
    const strengths = [];
    const s = signal.signals;
    if (s.postingFrequencyTrend > 0.3)
        strengths.push("Increasing posting frequency");
    if (s.engagementVolatility < 0.2)
        strengths.push("Stable engagement");
    if (s.ctaIntensityShift > 0.2)
        strengths.push("Strengthening CTA presence");
    if (s.contentExperimentRate > 0.3)
        strengths.push("High content experimentation");
    if (s.sentimentDrift > 0.1)
        strengths.push("Improving audience sentiment");
    if (signal.signalCoverageScore > 0.8)
        strengths.push("Comprehensive market presence");
    return strengths;
}
function identifyWeaknesses(signal) {
    const weaknesses = [];
    const s = signal.signals;
    if (s.postingFrequencyTrend < -0.2)
        weaknesses.push("Declining posting frequency");
    if (s.engagementVolatility > 0.6)
        weaknesses.push("Unstable engagement");
    if (s.ctaIntensityShift < -0.1)
        weaknesses.push("Weakening CTA presence");
    if (s.offerLanguageChange > 0.3)
        weaknesses.push("Frequent offer changes (instability)");
    if (s.hashtagDriftScore > 0.6)
        weaknesses.push("High hashtag drift (unclear positioning)");
    if (s.sentimentDrift < -0.1)
        weaknesses.push("Declining audience sentiment");
    if (s.contentExperimentRate < 0.1)
        weaknesses.push("Low content innovation");
    return weaknesses;
}
function detectEngagementBiasRisk(engagementContribution, totalScore, goalMode) {
    if (totalScore === 0)
        return null;
...
export function computeDominanceForCompetitor(signal, allSignals, confidence, goalMode = "STRATEGY_MODE") {
    const s = signal.signals;
...
export function computeAllDominance(signals, confidence, goalMode = "STRATEGY_MODE") {
