...
export function verifySnapshotIntegrity(snapshot, expectedVersion, campaignId) {
    const failures = [];
    if (!snapshot) {
    ...
    const hasOutput = !!(snapshot.analysisOutput ||
        snapshot.output ||
        snapshot.signalData ||
...
export function getEngineReadinessState(snapshot, campaignId, expectedVersion = ENGINE_VERSION, freshnessThresholdDays = MI_CONFIDENCE.FRESHNESS_HARD_GATE_DAYS) {
    const diagnostics = {
        snapshotFound: false,
        snapshotId: null,
    ...
    if (!freshnessMetadata.schemaCompatible) {
        ...
        if (snapshot.id && snapshot.status !== "INCOMPATIBLE") {
            db.update(miSnapshots)
                ...
                .catch((err) => console.error(`[MI] SCHEMA_GUARD | failed to persist INCOMPATIBLE: ${err.message}`));
        }
        return {
            state: "BLOCKED",
            diagnostics,
...
export function logDiagnostics(engineName, diagnostics) {
...
export function checkMIDependencyForDownstream(miSnapshot, campaignId) {
...
export async function invalidateStaleSnapshots() {
