...
function parseContentMix(contentTypeRatio: string | null): Record<string, number> {
  const mix: Record<string, number> = {};
  if (!contentTypeRatio) return mix;
...
function contentMixVector(mix: Record<string, number>, allKeys: string[]): number[] {
  return allKeys.map(k => mix[k] || 0);
}

function computeContentMixSimilarity(competitors: CompetitorInput[]): { score: number; sufficient: boolean } {
  const mixes = competitors.map(c => parseContentMix(c.contentTypeRatio));
  const hasData = mixes.filter(m => Object.keys(m).length > 0);
  if (hasData.length < 2) return { score: 0, sufficient: false };
...
function computeOfferLanguageSimilarity(competitors: CompetitorInput[]): { score: number; sufficient: boolean } {
  const offerSets: Set<string>[] = [];
  ...
  if (withData.length < 2) return { score: 0, sufficient: false };

  let totalSim = 0;
  let pairCount = 0;
  for (let i = 0; i < offerSets.length; i++) {
    if (offerSets[i].size === 0) continue;
    for (let j = i + 1; j < offerSets.length; j++) {
      if (offerSets[j].size === 0) continue;
...
function computeHookStyleSimilarity(competitors: CompetitorInput[]): { score: number; sufficient: boolean } {
  const hookSets: Set<string>[] = [];
  for (const c of competitors) {
    if (c.hookStyles) {
  ...
  for (let i = 0; i < hookSets.length; i++) {
    if (hookSets[i].size === 0) continue;
    for (let j = i + 1; j < hookSets.length; j++) {
      if (hookSets[j].size === 0) continue;
...
function computeCTADistributionSimilarity(competitors: CompetitorInput[]): { score: number; sufficient: boolean } {
  const ctaVectors: number[][] = [];
  for (const c of competitors) {
    const posts = c.posts || [];
    if (posts.length === 0) {
      ctaVectors.push([]);
  ...
  for (let i = 0; i < ctaVectors.length; i++) {
    if (ctaVectors[i].length === 0) continue;
    for (let j = i + 1; j < ctaVectors.length; j++) {
      if (ctaVectors[j].length === 0) continue;
...
function buildEvidenceCoverageMap(competitors: CompetitorInput[]): CompetitorEvidenceCoverage[] {
  return competitors.map(c => {
    ...
    if (comments.length === 0) missing.push("comments");
    if (!c.contentTypeRatio) missing.push("contentTypeRatio");
    if (!c.hookStyles) missing.push("hookStyles");
    if (!c.ctaPatterns) missing.push("ctaPatterns");
    if (!c.engagementRatio) missing.push("engagementRatio");
    if (!c.messagingTone) missing.push("messagingTone");
...
export function computeSimilarityDiagnosis(
  ...
  const dimensions: Record<string, SimilarityDimension> = {
    contentMix,
    offerLanguage,
    hookStyle,
    ctaIntent,
  ...
  const overallSimilarityIndex = Math.round(
    (sufficientDimensions.reduce((s, d) => s + d.score, 0) / sufficientDimensions.length) * 1000
  ) / 1000;
