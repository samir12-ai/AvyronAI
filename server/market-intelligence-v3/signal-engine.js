import { MI_COST_LIMITS, MI_AUTHORITY_WEIGHT, MI_SAMPLE_BIAS, MI_LIFECYCLE } from "./constants";
...
function computePostingFrequencyTrend(posts) {
...
function computeEngagementVolatility(posts) {
    if (posts.length < 3)
        return 0;
...
function computeCTAIntensityShift(posts) {
    if (posts.length < 4)
        return 0;
    const sorted = [...posts].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
...
function computeOfferLanguageChange(posts) {
    if (posts.length < 4)
        return 0;
    const sorted = [...posts].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    const mid = Math.floor(sorted.length / 2);
...
function computeHashtagDriftScore(posts) {
    if (posts.length < 4)
        return 0;
    const sorted = [...posts].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    const mid = Math.floor(sorted.length / 2);
...
function computeBioModificationFrequency(competitor) {
    if (competitor.messagingTone && competitor.messagingTone !== "unknown")
        return 0.3;
    return 0;
}
function computeSentimentDrift(posts, comments) {
...
function computeReviewVelocityChange(competitor) {
    return 0;
}
function computeContentExperimentRate(posts) {
    if (posts.length < 4)
        return 0;
...
export function classifyEngagementQuality(comments, posts) {
...
export function detectAudienceIntentSignals(comments) {
    const signals = [];
    ...
    if (confusionCount >= 2)
        signals.push("audience_confusion");
    ...
    if (beginnerCount >= 2)
        signals.push("beginner_questions");
    ...
    if (comparisonCount >= 2)
        signals.push("method_comparison");
    ...
    if (trustCount >= 2)
        signals.push("trust_concerns");
    return signals;
}
function clamp01(v) {
    return Math.max(0, Math.min(1, v));
}
export function computeCompetitorAuthorityWeight(signals, sampleSize, signalCoverageScore) {
    ...
    const weight = engagementStability * MI_AUTHORITY_WEIGHT.ENGAGEMENT_COMPONENT +
...
export function classifyCompetitorLifecycle(signals, sampleSize, signalCoverageScore) {
    if (sampleSize < MI_LIFECYCLE.LOW_SIGNAL_SAMPLE_THRESHOLD) {
        return "LOW_SIGNAL";
    }
    if (signals.postingFrequencyTrend < MI_LIFECYCLE.DORMANT_FREQUENCY_THRESHOLD &&
        signalCoverageScore < MI_LIFECYCLE.DORMANT_COVERAGE_THRESHOLD) {
        return "DORMANT";
    }
    return "ACTIVE";
}
export function detectSampleBias(signalResults) {
    const flags = [];
    let biasScore = 0;
...
export function computeRealDataRatio(comments) {
...
export function extractSemanticSignals(posts) {
    const signals = [];
...
export function clusterSemanticSignals(allResults) {
...
export function computeCompetitorSignals(competitor) {
    ...
    if (!competitor.contentTypeRatio)
        missingFields.push("contentTypeRatio");
    if (!competitor.ctaPatterns)
        missingFields.push("ctaPatterns");
    if (!competitor.profileLink)
        missingFields.push("profileLink (bio/CTA detection)");
...
export function computeAllSignals(competitors) {
...
export function aggregateMissingFlags(signalResults) {
    const flags = new Set();
