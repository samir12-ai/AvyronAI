...
export function computeDataCompleteness(signalResults) {
...
export function computeFreshnessDecay(dataAgeDays) {
    if (dataAgeDays <= 1)
        return 1.0;
    if (dataAgeDays <= 3)
        return 0.95;
...
export function computeSampleStrength(signalResults) {
    if (signalResults.length === 0)
        return 0;
...
export function computeCrossCompetitorConsistency(signalResults) {
...
export function computeSignalStability(signalResults) {
    if (signalResults.length === 0)
        return 0;
...
export function computeConfidenceFactors(signalResults, dataAgeDays) {
    return {
...
export function getConfidenceLevel(confidence) {
    if (confidence >= MI_CONFIDENCE.STRONG_THRESHOLD)
        return "STRONG";
    if (confidence >= MI_CONFIDENCE.MODERATE_THRESHOLD)
        return "MODERATE";
    if (confidence >= MI_CONFIDENCE.LOW_THRESHOLD)
        return "LOW";
    if (confidence >= MI_CONFIDENCE.UNSTABLE_THRESHOLD)
        return "UNSTABLE";
    return "INSUFFICIENT";
}
export function evaluateSignalStabilityGuard(signalResults, contentPrimaryMode = false) {
...
export function computeConfidence(signalResults, dataAgeDays, realDataRatio = 1.0, contentPrimaryMode = false) {
    const factors = computeConfidenceFactors(signalResults, dataAgeDays);
    let overall = Math.round((factors.dataCompleteness * 0.25 +
        ...
        factors.signalStability * 0.15) * 1000) / 1000;
