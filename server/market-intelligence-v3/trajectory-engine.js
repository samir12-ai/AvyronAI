...
export function computeMarketHeatingIndex(signals, intents) {
    if (signals.length === 0)
        return 0;
    ...
    const aggressiveRatio = intents.filter(i => i.intentCategory === "AGGRESSIVE_SCALING" || i.intentCategory === "PRICE_WAR").length / Math.max(1, intents.length);
    return clamp01(avgFrequencyTrend * 0.3 + avgCtaShift * 0.3 + aggressiveRatio * 0.4);
}
export function computeNarrativeConvergenceScore(signals) {
    if (signals.length < 2)
        return 0;
...
export function computeOfferCompressionIndex(signals) {
    if (signals.length < 2)
        return 0;
...
export function computeAngleSaturationLevel(signals) {
    if (signals.length < 2)
        return 0;
...
export function computeRevivalPotential(signals, intents) {
    if (signals.length === 0)
        return 0;
...
export function computeMarketActivityLevel(signals) {
    if (signals.length === 0)
        return 0;
    ...
    for (const s of signalsToUse) {
        ...
        if (s.lifecycle === "DORMANT")
            lifecycleWeight = MI_LIFECYCLE.DORMANT_ACTIVITY_WEIGHT;
        else if (s.lifecycle === "LOW_SIGNAL")
            lifecycleWeight = MI_LIFECYCLE.LOW_SIGNAL_ACTIVITY_WEIGHT;
        if (s.lowSample)
            lifecycleWeight *= MI_LIFECYCLE.LOW_SAMPLE_ACTIVITY_WEIGHT;
...
export function computeDemandConfidence(engagementQuality, signals) {
    if (signals.length === 0)
        return 0;
...
export function computeCompetitionIntensity(signals, engagementQuality) {
    if (signals.length === 0)
        return 0;
    ...
    let intensity = clamp01(competitorDensity * 0.30 +
        contentVolume * 0.25 +
...
export function deriveCompetitionIntensityLevel(score) {
    if (score >= 0.70)
        return "HIGH";
    if (score >= 0.45)
        return "MODERATE";
...
export function computeMarketCompressionScore(narrativeConvergence, angleSaturation, marketHeating) {
    return clamp01(Math.round((narrativeConvergence * 0.45 + angleSaturation * 0.35 + marketHeating * 0.20) * 1000) / 1000);
}
export function computeTrajectory(signals, intents, engagementQuality) {
    const eq = engagementQuality || { highIntentCount: 0, lowValueCount: 0, engagementQualityRatio: 0 };
...
export function deriveTrajectoryDirection(trajectory, demandPressureScore) {
    ...
    if (marketHeatingIndex > 0.7 && offerCompressionIndex > 0.5)
        return "HEATING_COMPRESSED";
    if (marketHeatingIndex > 0.7)
        return "HEATING";
    if (marketHeatingIndex < 0.3 && angleSaturationLevel > 0.7) {
        if (highIntensity)
            return "STABLE_COMPETITIVE";
        if (dp >= MI_DEMAND_PRESSURE.SUPPRESSED_DEMAND_PRESSURE_FLOOR)
    ...
    if (offerCompressionIndex > 0.6)
        return "COMPRESSING";
    if (angleSaturationLevel > 0.7)
        return "SATURATED";
    return "STABLE";
}
export function deriveMarketState(trajectory, confidenceLevel, demandPressureScore) {
