...
export async function computeMarketBaseline(accountId, campaignId) {
    try {
...
export function computeDynamicThreshold(baseThreshold, ctx) {
    if (!ctx)
        return baseThreshold;
    let modifier = 0;
    if (ctx.postsAnalyzed < 12)
        modifier += 0.15;
    else if (ctx.postsAnalyzed < 50)
        modifier += 0.08;
    else if (ctx.postsAnalyzed >= 100)
        modifier -= 0.05;
    if (ctx.signalNoiseRatio < 0.3)
        modifier += 0.12;
    else if (ctx.signalNoiseRatio < 0.5)
        modifier += 0.06;
    else if (ctx.signalNoiseRatio >= 0.7)
        modifier -= 0.05;
    if (ctx.confidenceScore < 0.4)
        modifier += 0.10;
    else if (ctx.confidenceScore < 0.6)
        modifier += 0.05;
    else if (ctx.confidenceScore >= 0.8)
        modifier -= 0.05;
    if (ctx.competitorCoverage < 0.5)
        modifier += 0.10;
    else if (ctx.competitorCoverage >= 0.8)
        modifier -= 0.03;
    const threshold = baseThreshold + modifier;
    return Math.max(0.10, Math.min(0.60, threshold));
}
export function computeDeviation(observed, baseline, field, threshold) {
...
export function computeAllDeviations(trajectory, baseline, ctx) {
...
export function getDeviationForField(deviations, field) {
