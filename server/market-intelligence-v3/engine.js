...
import { extractNarrativeObjections, clusterObjections } from "./narrative-objection-extractor";
...
import { ENGINE_VERSION, INSTAGRAM_API_CEILING } from "./constants";
import { validateNoStrategyWrite } from "./isolation-guard";
...
function convertProblemStatementToObjection(snippet, competitorName) {
...
function isSnapshotAnalyticallyComplete(snapshot) {
...
export async function persistValidatedSnapshot(snapshotPayload, caller) {
    ...
    if (snapshotPayload.accountId && snapshotPayload.campaignId) {
        const existingBaseline = await db.select({ id: miSnapshots.id, dataStatus: miSnapshots.dataStatus, version: miSnapshots.version, snapshotSource: miSnapshots.snapshotSource })
            .from(miSnapshots)
            .where(and(eq(miSnapshots.accountId, snapshotPayload.accountId), eq(miSnapshots.campaignId, snapshotPayload.campaignId), inArray(miSnapshots.status, ["COMPLETE", "PARTIAL"])))
...
async function getCachedSnapshot(accountId, campaignId, competitorHash) {
    ...
    if (snapshots.length === 0)
        return { snapshot: null, invalidationReason: null };
    ...
    if (age > freshnessMs)
        return { snapshot: null, invalidationReason: "STALE" };
    return { snapshot, invalidationReason: null };
}
export function computeDataFreshnessDays(competitors) {
    if (competitors.length === 0)
        return 999;
    const now = Date.now();
    let newest = 0;
    ...
    if (days > 365) {
        ...
        for (const c of competitors) {
            for (const p of c.posts || []) {
                const t = new Date(p.timestamp).getTime();
                if (!isNaN(t) && t > recentThreshold) {
                    recentSignalExists = true;
                    break;
                }
            }
        ...
        if (recentSignalExists) {
            ...
            if (corrected > 0)
                return Math.round((now - corrected) / (1000 * 60 * 60 * 24));
        }
    }
    return days;
}
export function computeVolatilityIndex(signalResults) {
    if (signalResults.length === 0)
        return 0;
    const volatilities = signalResults.map((r) => {
        const s = r.signals;
        return (Math.abs(s.engagementVolatility || 0) * 0.3 +
            Math.abs(s.ctaIntensityShift || 0) * 0.25 +
            Math.abs(s.postingFrequencyTrend || 0) * 0.25 +
            Math.abs(s.contentExperimentRate || 0) * 0.2);
    });
    return Math.min(1, volatilities.reduce((s, v) => s + v, 0) / volatilities.length);
}
export function computeSignalNoiseRatio(signalResults, confidence) {
    if (signalResults.length === 0)
        return 0;
    const avgCoverage = signalResults.reduce((s, r) => s + (r.signalCoverageScore || 0), 0) / signalResults.length;
...
export function computeEvidenceCoverage(signalResults, totalCompetitors) {
    const postsAnalyzed = signalResults.reduce((s, r) => s + (r.sampleSize || 0), 0);
    const commentsAnalyzed = signalResults.reduce((s, r) => s + (r.commentCount || 0), 0);
    const competitorsWithSufficientData = signalResults.filter((r) => (r.sampleSize || 0) >= INSTAGRAM_API_CEILING).length;
    return { postsAnalyzed, commentsAnalyzed: commentsAnalyzed, competitorsWithSufficientData, totalCompetitors };
}
export function buildMarketDiagnosis(confidence, trajectory, dominantIntent) {
...
export function buildThreatSignals(confidence, trajectory, intents, deviations, baselineCalibrated, signalClusters) {
    const threats = [];
    if (confidence.guardDecision === "BLOCK") {
        threats.push("INSUFFICIENT DATA: Cannot assess threat landscape reliably");
        return threats;
    }
    const aggressiveCompetitors = intents.filter((i) => i.intentCategory === "AGGRESSIVE_SCALING" || i.intentCategory === "PRICE_WAR");
    if (aggressiveCompetitors.length > 0) {
        const names = aggressiveCompetitors.map((c) => c.competitorName).join(", ");
        threats.push(`${aggressiveCompetitors.length} competitor(s) showing aggressive scaling or price-war signals: ${names}`);
    }
    const positioningShifts = intents.filter((i) => i.intentCategory === "POSITIONING_SHIFT");
    ...
    const decliningCompetitors = intents.filter((i) => i.intentCategory === "DECLINING");
...
export function buildOpportunitySignals(confidence, trajectory, intents, deviations, baselineCalibrated, signalClusters) {
    const opportunities = [];
    if (confidence.guardDecision === "BLOCK") {
    ...
    const testingCompetitors = intents.filter((i) => i.intentCategory === "TESTING");
...
function sanitizeOpportunitySignals(signals) {
...
export function computeSnapshotDeltas(prevSnapshot, currSnapshot) {
    if (!prevSnapshot)
        return null;
    ...
    if (!competitorHashMatch)
        return null;
    const prevSignals = parseJsonSafe(prevSnapshot.signalData, []);
    const prevIntents = parseJsonSafe(prevSnapshot.intentData, []);
    const prevTrajectory = parseJsonSafe(prevSnapshot.trajectoryData, {});
    const prevDominance = parseJsonSafe(prevSnapshot.dominanceData, []);
    ...
    for (const curr of currSignals) {
        const prev = prevSignals.find((p) => p.competitorId === curr.competitorId);
        if (!prev)
            continue;
    ...
    for (const curr of currIntents) {
        const prev = prevIntents.find((p) => p.competitorId === curr.competitorId);
        if (!prev)
            continue;
        intentChanges.push({
            competitorId: curr.competitorId,
    ...
    const dominanceChanges = [];
    for (const curr of currDominance) {
        const prev = prevDominance.find((p) => p.competitorId === curr.competitorId);
        if (!prev)
            continue;
    ...
    const hasMeaningfulChanges = signalDeltas.length > 0 ||
...
export class MarketIntelligenceV3 {
    static async run(mode, accountId, campaignId, forceRefresh = false, goalMode = "STRATEGY_MODE") {
        ...
        if (existingLock) {
            ...
            try {
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Lock timeout: existing run for ${lockKey} exceeded ${LOCK_TIMEOUT_MS / 1000}s`)), LOCK_TIMEOUT_MS));
    ...
    static async _executeRun(mode, accountId, campaignId, forceRefresh, goalMode = "STRATEGY_MODE") {
        ...
        for (const comp of competitors) {
            const { sampledPosts, sampledComments } = applySampling(comp.posts || [], comp.comments || [], executionMode);
        ...
        for (const dna of contentDnaResults) {
            if (dna.hookArchetypes.includes("problem")) {
                ...
                for (const ev of problemEvidence) {
                    ...
                    if (snippet.length >= 10)
                        bridgeSnippets.push({ snippet, competitorName: dna.competitorName });
                }
            }
            if (dna.narrativeFrameworks.includes("problem_solution")) {
        ...
        const competitorNarratives = competitors.map(c => {
            const parts = [];
            if (c.messagingTone)
                parts.push(c.messagingTone);
            if (c.hookStyles)
                parts.push(c.hookStyles);
            if (c.ctaPatterns)
                parts.push(c.ctaPatterns);
        ...
        const deltaReport = computeSnapshotDeltas(previousSnapshot?.status === "COMPLETE" ? previousSnapshot : null, {
            competitorHash,
        ...
        const audiencePressureSignal = Math.round((demandPressure.components.intentSignalStrength * 0.4 +
            demandPressure.components.objectionDensity * 0.3 +
            demandPressure.components.purchaseIntentDensity * 0.3) * 1000) / 1000;
        const diagnostics = {
        ...
        const miLineage = [];
        opportunitySignals.forEach((sig, i) => {
            const text = typeof sig === "string" ? sig : (sig?.signal || sig?.description || sig?.message || "");
            if (text)
                miLineage.push(createSourceLineageEntry("market_intelligence", "market_opportunity", text, i));
        });
        threatSignals.forEach((sig, i) => {
            const text = typeof sig === "string" ? sig : (sig?.signal || sig?.description || sig?.message || "");
            if (text)
                miLineage.push(createSourceLineageEntry("market_intelligence", "market_threat", text, i));
        });
        const narObjections = narrativeObjectionMap?.objections || [];
        narObjections.forEach((obj, i) => {
            if (obj?.objection)
                miLineage.push(createSourceLineageEntry("market_intelligence", "narrative_objection", obj.objection, i));
        });
        ...
        const snapshotPayload = {
            ...
            status: qualityGate.gatePass ? "COMPLETE" : "PARTIAL",
            confirmedRuns,
        ...
        return {
            output,
            snapshotId: snapshot.id,
            snapshotStatus: snapshot.status === "COMPLETE" ? "COMPLETE" : "PARTIAL",
            executionMode,
            goalMode,
            telemetry,
            dominanceData: dominanceResults,
...
export function buildResultFromSnapshot(snapshot) {
    ...
    return {
        ...
        snapshotStatus: (snapshot.status === "COMPLETE" ? "COMPLETE" : "PARTIAL"),
        executionMode: snapshot.executionMode,
        goalMode: snapshot.goalMode || "STRATEGY_MODE",
        telemetry,
        dominanceData,
        trajectoryData,
        signalGuard: {
        ...
        snapshotSource: snapshot.snapshotSource || "FRESH_DATA",
        fetchExecuted: snapshot.fetchExecuted ?? true,
        timestamp: snapshot.createdAt?.toISOString?.() || new Date().toISOString(),
        dataStatus: snapshot.dataStatus || "COMPLETE",
    };
}
export function buildMarketSummary(marketState, direction, dominantIntent, confidence, intents, competitionIntensityLevel) {
    const parts = [];
    ...
    const intentSummary = intents.map((i) => `${i.competitorName}: ${i.intentCategory}`).join(", ");
    if (intentSummary)
        parts.push(`Intent breakdown: ${intentSummary}.`);
    return parts.join(" ");
}
