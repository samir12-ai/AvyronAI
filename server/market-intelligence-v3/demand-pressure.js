...
export function computeDemandPressure(comments, engagementQuality, audienceIntentSignals, commentCount) {
    ...
    if (comments.length === 0) {
        ...
        const score = clamp01(engagementDensity * (MI_DEMAND_PRESSURE.WEIGHT_ENGAGEMENT + MI_DEMAND_PRESSURE.WEIGHT_OBJECTION + MI_DEMAND_PRESSURE.WEIGHT_PURCHASE_INTENT) +
        ...
        let level = "LOW";
        if (roundedScore >= MI_DEMAND_PRESSURE.HIGH_THRESHOLD)
            level = "HIGH";
        else if (roundedScore >= MI_DEMAND_PRESSURE.MODERATE_THRESHOLD)
            level = "MODERATE";
    ...
    for (const text of texts) {
        if (PURCHASE_INTENT_PATTERNS.some(p => p.test(text)))
            purchaseIntentCount++;
        if (OBJECTION_PATTERNS.some(p => p.test(text)))
            objectionCount++;
        if (HIGH_INTENT_LANGUAGE.some(p => p.test(text)))
            highIntentLanguageCount++;
    }
    ...
    const score = clamp01(engagementDensity * MI_DEMAND_PRESSURE.WEIGHT_ENGAGEMENT +
        commentIntensity * MI_DEMAND_PRESSURE.WEIGHT_COMMENT_INTENSITY +
        intentSignalStrength * MI_DEMAND_PRESSURE.WEIGHT_INTENT_SIGNAL +
