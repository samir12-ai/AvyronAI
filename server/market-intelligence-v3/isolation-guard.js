...
export function validateEngineIsolation(caller) {
...
export function validateDomainOwnership(caller, domain) {
    if (caller !== ISOLATION_ALLOWED_CALLER) {
...
export function rejectBlockedEngine(engineName, endpoint) {
    const isBlocked = BLOCKED_ENGINES.includes(engineName);
    if (isBlocked) {
...
export function assertSnapshotReadOnly(caller, operation) {
...
export function assertNoPlanWrites() {
    logAudit("system", "CI_ENGINE_ISOLATION_VIOLATION", {
...
export function assertNoOrchestrator() {
    logAudit("system", "CI_ENGINE_ISOLATION_VIOLATION", {
...
export function assertNoAutopilot() {
    logAudit("system", "CI_ENGINE_ISOLATION_VIOLATION", {
...
export function assertNoComputeFromExternal(caller, computeFunction) {
    if (caller !== ISOLATION_ALLOWED_CALLER) {
...
export function assertNoStrategyWrites(operation) {
    ...
    throw new Error(`ISOLATION VIOLATION: Market Intelligence V3 must NOT generate or modify strategy outputs. Blocked domains: ${STRATEGY_WRITE_DOMAINS.join(", ")}. MI is a pure intelligence layer — it produces data, not strategies.`);
}
export function validateNoStrategyWrite(caller, targetDomain) {
    const isStrategyDomain = STRATEGY_WRITE_DOMAINS.includes(targetDomain.toUpperCase());
    if (isStrategyDomain && caller === ISOLATION_ALLOWED_CALLER) {
        ...
        throw new Error(`ISOLATION VIOLATION: MIv3 attempted to write to strategy domain '${targetDomain}'. MI must not generate positioning strategies, modify offer/pricing/funnel, or write audience/differentiation outputs.`);
    }
}
export function getProtectedDomains() {
    return MIV3_PROTECTED_DOMAINS;
}
export function getBlockedEngines() {
    return BLOCKED_ENGINES;
}
export function getStrategyWriteDomains() {
