...
export function clusterNarratives(narratives) {
...
export function detectEchoChamber(signals) {
    if (signals.length < 2) {
    ...
    const isEchoChamber = convergenceScore >= MI_NARRATIVE_CLUSTERING.ECHO_CHAMBER_CONVERGENCE_THRESHOLD &&
    ...
    if (convergenceScore >= MI_NARRATIVE_CLUSTERING.ECHO_CHAMBER_CONVERGENCE_THRESHOLD) {
        echoChamberRisk = Math.min(1, (convergenceScore - MI_NARRATIVE_CLUSTERING.ECHO_CHAMBER_CONVERGENCE_THRESHOLD) /
...
export function computeClusteredSaturation(narrativeSaturation) {
    const keys = Object.keys(narrativeSaturation);
    if (keys.length === 0)
        return narrativeSaturation;
