...
export async function logAssumptions(planId, campaignId, accountId, assumptions) {
...
export async function getAssumptionsForPlan(planId) {
    const rows = await db.select().from(planAssumptions)
        .where(eq(planAssumptions.planId, planId));
    return rows.map(r => ({
        ...r,
        affectedModules: r.affectedModules ? (() => { try {
            return JSON.parse(r.affectedModules);
        }
        catch {
            return [];
        } })() : [],
...
function detectImplicitAssumptions(planData, bizData) {
    const assumptions = [];
...
export function registerConflictResolverRoutes(app) {
    app.get("/api/plan-assumptions/:planId", async (req, res) => {
        try {
    ...
    app.post("/api/plan-assumptions/detect", async (req, res) => {
        try {
            const { planId, campaignId } = req.body;
            if (!planId || !campaignId)
                return res.status(400).json({ error: "planId and campaignId required" });
