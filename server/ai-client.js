...
export class AICallError extends Error {
    constructor(message, code = "AI_CALL_FAILED") {
        super(message);
...
export async function aiGemini(options) {
    ...
    try {
        ...
        success = true;
        actualTokens = result?.usageMetadata?.totalTokenCount || maxTokens;
        return result;
    }
    catch (err) {
        if (err instanceof AICallError)
...
const ACCOUNT_BUDGET_OVERRIDES = {
...
export function getWeeklyTokenBudget(accountId) {
    if (accountId)
        return getAccountBudget(accountId);
    return WEEKLY_TOKEN_BUDGET;
}
export { WEEKLY_TOKEN_BUDGET, ACCOUNT_BUDGET_OVERRIDES };
export async function getWeeklyTokenUsage(accountId) {
    try {
        const { db } = await import("./db");
        const { sql } = await import("drizzle-orm");
        const result = await db.execute(sql `
