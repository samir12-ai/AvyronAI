...
const PAIN_CATEGORIES = ["pain_signal"];
...
const DESIRE_CATEGORIES = ["desire_signal", "transformation_statement"];
const OBJECTION_CATEGORIES = ["audience_objection", "competitor_weakness"];
function generateSignalId(source, category, competitorId, index) {
    return `bridge_${source}_${category}_${competitorId}_${index}`;
}
function canonicalizePainSnippet(snippet) {
    const lower = snippet.toLowerCase();
    if (/frustrat|fed up|sick of|tired of/.test(lower))
        return "frustration with lack of results";
    if (/confus|overwhelm|too much|don't understand/.test(lower))
        return "confusion and overwhelm";
    if (/no time|too busy|takes too long|busy schedule/.test(lower))
        return "time pressure";
    if (/expensive|can't afford|cost|pric|budget/.test(lower))
        return "cost and affordability concerns";
    if (/scam|fake|legit|trust|too good/.test(lower))
        return "trust and credibility doubts";
    if (/afraid|scared|fear|worried|risk|fail/.test(lower))
        return "fear of failure";
    if (/stuck|no results|not working|plateau|waste/.test(lower))
        return "frustration with lack of results";
    if (/motivat|consisten|discipline|give up|quit/.test(lower))
        return "motivation and consistency issues";
    return snippet.slice(0, 80).trim();
}
function canonicalizeDesireSnippet(snippet) {
    const lower = snippet.toLowerCase();
    if (/muscle|strong|lean|bulk|physique/.test(lower))
        return "build muscle / get stronger";
    if (/confiden|self.?esteem|believe/.test(lower))
        return "increase confidence";
    if (/money|income|financ|wealth|revenue/.test(lower))
        return "financial improvement";
    if (/time|freedom|automat|passive/.test(lower))
        return "save time / freedom";
    if (/recogni|status|social|influen/.test(lower))
        return "social recognition";
    if (/health|energy|wellness|vitality/.test(lower))
        return "health and wellness";
    if (/transform|life.?chang|success|achieve|breakthrough/.test(lower))
        return "transformational outcome";
    if (/scale|grow|10x|doubl|tripl/.test(lower))
        return "scale and growth";
    return snippet.slice(0, 80).trim();
}
function canonicalizeObjectionSnippet(snippet) {
    const lower = snippet.toLowerCase();
    if (/expensive|afford|cost|pric|budget/.test(lower))
        return "too expensive";
    if (/no time|busy|time/.test(lower))
        return "no time";
    if (/tried|before|didn't work|already/.test(lower))
        return "tried before and failed";
    if (/skeptic|doubt|too good|fake|scam|real/.test(lower))
        return "skeptical / trust issues";
    if (/complex|hard|difficult|complicated/.test(lower))
        return "complexity / too hard";
    if (/don't|fail|miss|lack|gap|inferior|limit/.test(lower))
        return "perceived limitations";
    if (/cheap|overpric|empty|overhype|cookie|generic/.test(lower))
        return "competitor credibility gap";
    return snippet.slice(0, 80).trim();
}
function extractSemanticBridgeSignals(signalData, qualityGateResults) {
    const bridged = [];
    ...
    for (const competitorResult of signalData) {
        ...
        const semanticSignals = competitorResult.semanticSignals || [];
        let idx = 0;
...
function extractContentDnaBridgeSignals(contentDnaData) {
    const bridged = [];
    if (!contentDnaData || !Array.isArray(contentDnaData))
        return bridged;
    for (const dna of contentDnaData) {
        if (dna.dnaConfidence < CLEAN_PIPE_CONFIDENCE_FLOOR)
            continue;
        const competitorId = dna.competitorId || "unknown";
        const competitorName = dna.competitorName || "unknown";
        if (!dna.evidence || !Array.isArray(dna.evidence))
            continue;
        let idx = 0;
...
function resolveConflicts(existingSignals, bridgedSignals) {
...
export function executeSemanticBridge(miSnapshot) {
    const emptyResult = {
        painSignals: [],
        desireSignals: [],
        objectionSignals: [],
...
export function mergeBridgedIntoAudienceMap(existingSignals, bridgedSignals, miSnapshotId) {
    if (bridgedSignals.length === 0)
        return { merged: existingSignals, conflictsResolved: 0 };
...
export function validateBridgeIntegrity(bridgeResult) {
    const issues = [];
