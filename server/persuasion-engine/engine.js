import { ENGINE_VERSION, STATUS, BOUNDARY_HARD_PATTERNS, BOUNDARY_SOFT_PATTERNS, LAYER_WEIGHTS, INFLUENCE_DRIVERS, TRUST_SEQUENCE_STAGES, HYPE_PATTERNS_HARD, HYPE_PATTERNS_CONTEXTUAL, GENERIC_PERSUASION_PHRASES, GENERIC_STRUCTURE_PATTERNS, OBJECTION_PROOF_MAP, LOW_READINESS_STAGES, HIGH_READINESS_STAGES, EDUCATION_FIRST_MODES, FUNNEL_PERSUASION_COMPATIBILITY, AWARENESS_PERSUASION_MAP, TRUST_PROOF_ESCALATION_ORDER, MESSAGE_ARCHITECTURE_ORDER, MESSAGE_STEP_CATEGORY_MAP, } from "./constants";
import { formatAELForPrompt } from "../analytical-enrichment-layer/engine";
import { enforceEngineDepthCompliance, applyDepthPenalty, isDepthBlocking, buildDepthGateResult, } from "../causal-enforcement-layer/engine";
...
import { extractQualifyingSignals, MIN_QUALIFYING_SIGNALS, } from "../shared/signal-lineage";
...
function isLowReadiness(stage) {
    return LOW_READINESS_STAGES.includes(stage);
}
function isHighReadiness(stage) {
    return HIGH_READINESS_STAGES.includes(stage);
}
const OBJECTION_TYPE_MAP = {
...
function classifyObjectionStage(text, category) {
    if (category && OBJECTION_STAGE_MAP[category])
        return OBJECTION_STAGE_MAP[category];
    const lower = text.toLowerCase();
    if (lower.includes("price") || lower.includes("cost") || lower.includes("commit") || lower.includes("buy") || lower.includes("pay"))
        return "decision";
    if (lower.includes("compare") || lower.includes("alternative") || lower.includes("better") || lower.includes("different"))
        return "consideration";
    return "awareness";
}
function buildPersuasionResponse(objType, statement) {
    switch (objType) {
...
function buildStructuredObjectionMap(audience, mi) {
    const structuredObjections = [];
    const seen = new Set();
    ...
    for (const item of narrativeObjections) {
        ...
        let objType = classifyObjectionType(item.objection);
        if (signalType === "trust_barrier")
            objType = "trust";
...
function assessDataReliability(mi, audience, positioning, differentiation, offer, awareness, structuredObjections) {
    const advisories = [];
    ...
    if (totalMISignals < 3)
        advisories.push(`Low MI signal density: only ${totalMISignals} signal(s) detected`);
    const sourceDiversity = new Set();
    if ((audience.audiencePains || []).length > 0)
        sourceDiversity.add("pains");
    if ((audience.emotionalDrivers || []).length > 0)
        sourceDiversity.add("drivers");
    if (Object.keys(audience.objectionMap || {}).length > 0)
        sourceDiversity.add("objections");
    if ((audience.audienceSegments || []).length > 0)
        sourceDiversity.add("segments");
    if (mi.marketDiagnosis)
        sourceDiversity.add("diagnosis");
    if (positioning.narrativeDirection)
        sourceDiversity.add("narrative");
    if ((differentiation.pillars || []).length > 0)
        sourceDiversity.add("pillars");
    if (offer.coreOutcome)
        sourceDiversity.add("offer_outcome");
    if (awareness.entryMechanismType)
        sourceDiversity.add("awareness_entry");
    const signalDiversity = clamp(sourceDiversity.size / 7, 0, 1);
    if (sourceDiversity.size < 3)
        advisories.push(`Low signal diversity: only ${sourceDiversity.size}/9 data sources populated`);
    ...
    if (narrativeStability < 0.5)
        advisories.push("Low narrative stability: positioning and differentiation data sparse");
    const competitorValidity = clamp(totalMISignals / 5, 0, 1);
    const marketMaturityConfidence = clamp((safeNumber(mi.overallConfidence, 0) + safeNumber(positioning.confidenceScore, 0) + safeNumber(differentiation.confidenceScore, 0)) / 3, 0, 1);
    ...
    if (structuredObjections && structuredObjections.length > 0) {
        ...
        objectionSpecificity = clamp(countFactor * 0.4 + fieldCompleteness * 0.4 + sourceDiversityBonus + 0.1, 0.6, 1.0);
    }
    ...
    if (objectionSpecificity < 0.3)
        advisories.push(`Weak objection specificity (${Math.round(objectionSpecificity * 100)}%) — persuasion objection handling may be imprecise`);
    ...
    const trustSpecificity = trustReq !== "unknown"
        ...
        : 0.2;
    if (trustSpecificity < 0.4)
        advisories.push(`Weak trust specificity (${Math.round(trustSpecificity * 100)}%) — trust barrier mapping may be generic`);
    const overallReliability = clamp(signalDensity * 0.15 + signalDiversity * 0.15 + narrativeStability * 0.15 +
        competitorValidity * 0.10 + marketMaturityConfidence * 0.15 +
        objectionSpecificity * 0.15 + trustSpecificity * 0.15, 0, 1);
    const isWeak = overallReliability < 0.4;
    if (isWeak)
        advisories.push("Overall data reliability is WEAK — confidence scores will be capped");
    return {
...
function normalizeConfidence(rawScore, reliability) {
...
function classifyTrustBarriers(audience, awareness, differentiation, offer, mi) {
    const barriers = [];
...
function buildObjectionProofLinks(audience, differentiation, offer, awareness, mi) {
    const links = [];
    const proofArch = differentiation.proofArchitecture || [];
    const proofTypes = new Set(proofArch.map((p) => safeString(p?.type || p?.proofType || p, "unknown").toLowerCase()));
    const objectionMap = audience.objectionMap || {};
...
function validateScarcityConditions(awareness, audience, differentiation, funnel, objectionProofLinks) {
...
function layer1_awarenessToPersuasionFit(awareness, audience, funnel) {
    const findings = [];
    const warnings = [];
    let score = 0.5;
...
function layer2_objectionDetection(audience, offer, differentiation, awareness, mi, funnel, integrity) {
    const findings = [];
    const warnings = [];
    let score = 0.5;
...
function layer3_trustBarrierMapping(audience, awareness, integrity, differentiation, offer, mi, trustBarriers) {
    const findings = [];
    const warnings = [];
    let score = 0.5;
    const trustReq = safeString(awareness.trustRequirement, "medium");
...
function layer4_influenceDriverSelection(audience, differentiation, awareness, trustBarriers) {
    const findings = [];
    const warnings = [];
    let score = 0.5;
    ...
    if (entry === "pain_entry") {
        if (!selectedDrivers.includes("contrast"))
    ...
    if (entry === "myth_breaker_entry") {
        if (!selectedDrivers.includes("contrast"))
            selectedDrivers.push("contrast");
        if (!selectedDrivers.includes("specificity"))
    ...
    if (objections.length > 0) {
        if (!selectedDrivers.includes("risk_reversal"))
            selectedDrivers.push("risk_reversal");
        findings.push("Objections present → risk_reversal driver selected");
    }
    const segments = audience.audienceSegments || [];
    if (segments.length > 1) {
        if (!selectedDrivers.includes("social_proof"))
            selectedDrivers.push("social_proof");
...
function buildTrustProofSequence(proofArch, objectionProofLinks) {
    const availableProofTypes = new Set();
    ...
    for (const escalationStep of TRUST_PROOF_ESCALATION_ORDER) {
        ...
        const matched = [...availableProofTypes].some(p => p.includes(stepLower.replace("_proof", "")) || stepLower.includes(p.replace("_proof", "")));
        if (matched) {
...
function layer5_proofPriorityMapping(differentiation, audience, awareness, objectionProofLinks) {
    const findings = [];
    const warnings = [];
    let score = 0.5;
    const proofArch = differentiation.proofArchitecture || [];
...
function layer6_messageOrderLogic(awareness, audience, offer, funnel, differentiation, trustBarriers, objectionProofLinks) {
    const findings = [];
    const warnings = [];
    let score = 0.5;
    const readiness = safeString(awareness.targetReadinessStage, "problem_aware");
    ...
    const proofCount = (differentiation.proofArchitecture || []).length;
    if (readiness === "unaware") {
        sequence.push("acknowledge_pain", "disrupt_assumption", "educate_on_problem", "introduce_mechanism", "reinforce_with_proof", "build_authority", "remove_risk", "soft_next_step");
        ...
        sequence.push("acknowledge_pain", "disrupt_assumption", "educate_on_problem", "demonstrate_understanding", "introduce_mechanism");
        if (proofCount > 0)
            sequence.push("reinforce_with_proof");
...
function validateMessageArchitecture(sequence) {
    const violations = [];
    ...
    for (const cat of knownCategories) {
        const archIdx = MESSAGE_ARCHITECTURE_ORDER.indexOf(cat);
        if (archIdx === -1)
            continue;
...
function layer7_antiHypeGuard(allText, objectionProofLinks, trustBarriers, readinessStage) {
    const findings = [];
    const warnings = [];
    let score = 0.85;
...
function layer8_persuasionStrengthScoring(layerResults, awareness, trustBarriers, objectionProofLinks, funnel, selectedMode) {
    const findings = [];
    const warnings = [];
    let weightedSum = 0;
    let totalWeight = 0;
    ...
    if (isLowReadiness(readiness)) {
        const isEducationMode = EDUCATION_FIRST_MODES.includes(selectedMode);
        if (!isEducationMode) {
...
function selectPersuasionMode(awareness, audience, differentiation, funnel) {
    const entry = safeString(awareness.entryMechanismType, "");
    const readiness = safeString(awareness.targetReadinessStage, "unknown");
    const proofCount = (differentiation.proofArchitecture || []).length;
    const authorityMode = safeString(differentiation.authorityMode, "");
    ...
    if (isLowReadiness(readiness)) {
        if (entry === "diagnostic_entry" || funnelType.includes("diagnostic"))
            return "diagnostic_led";
        if (entry === "pain_entry")
            return "empathy_led";
        if (readiness === "unaware")
            return "education_proof_hybrid";
        return awarenessDefault || "education_led";
    }
    if (readiness === "solution_aware") {
        if (entry === "myth_breaker_entry")
            return "contrast_led";
        if (entry === "diagnostic_entry")
            return "diagnostic_led";
        return awarenessDefault || "contrast_led";
    }
    if (readiness === "product_aware") {
        if (entry === "authority_entry" && authorityMode)
            return "authority_led";
        if (proofCount >= 2)
            return "proof_led";
        const compatibleModes = FUNNEL_PERSUASION_COMPATIBILITY[funnelType] || [];
        if (compatibleModes.length > 0 && compatibleModes.includes("proof_led"))
            return "proof_led";
    ...
    const compatibleModes = FUNNEL_PERSUASION_COMPATIBILITY[funnelType] || [];
    if (compatibleModes.length > 0) {
        if (proofCount >= 3 && compatibleModes.includes("proof_led"))
            return "proof_led";
        if (authorityMode && compatibleModes.includes("authority_led"))
            return "authority_led";
        return compatibleModes[0];
    }
    return awarenessDefault || "education_led";
}
const AWARENESS_COMPATIBLE_MODES = {
...
function applyAutoCorrection(selectedMode, readiness) {
...
function validatePersuasionRouteAgainstFunnel(mode, funnel, awareness, trustBarriers) {
    const funnelType = safeString(funnel.funnelType, "unknown").toLowerCase();
    const compatibleModes = FUNNEL_PERSUASION_COMPATIBILITY[funnelType] || [];
    const funWarnings = [];
    const readiness = safeString(awareness.targetReadinessStage, "unknown");
    ...
    if (proofPlacements.length > 0) {
        const earlyProofNeeded = proofPlacements.some((p) => {
    ...
    if (isLowReadiness(readiness) && !EDUCATION_FIRST_MODES.includes(mode)) {
...
function buildPersuasionRoutes(audience, awareness, differentiation, offer, funnel, mi, layerResults, trustBarriers, awarenessStageProperties, objectionProofLinks, autoCorrection, structuredObjections) {
    ...
    const routeWarnings = [...funnelValidation.warnings];
    ...
    const trustPath = (funnel.trustPath || []).map((tp) => typeof tp === "string" ? tp : tp?.stage || tp?.name || "trust_step");
    ...
    const mechanismRef = core && core.mechanismType !== "none" && core.mechanismName
        ...
        : "";
    const offerLockedDecisions = offer.lockedDecisions || [];
    const offerNonGenericAnchors = offer.nonGenericAnchors || [];
    let lockedRouteNameSuffix = "";
    const lockConstraintNotes = [];
    if (offerLockedDecisions.length > 0) {
        ...
        if (contrastAxisDecision) {
            ...
            if (axisCore)
                lockedRouteNameSuffix += ` [locked axis: ${axisCore.slice(0, 40)}]`;
        }
        for (const decision of offerLockedDecisions) {
            ...
            if (core)
                lockConstraintNotes.push(`LOCK CONSTRAINT — YOU MUST NOT reframe or contradict: "${core}"`);
        }
        if (enemyDecision) {
            const enemyCore = enemyDecision.replace(/^enemy:\s*/i, "").trim();
            if (enemyCore)
                lockConstraintNotes.push(`ENEMY ANCHOR: contrast against "${enemyCore}" — MUST be reflected in objection handling`);
        }
        if (mechanismDecision) {
            const mechCore = mechanismDecision.replace(/^mechanism:\s*/i, "").trim();
            if (mechCore)
                lockConstraintNotes.push(`MECHANISM ANCHOR: route MUST reflect "${mechCore}" as the solution vehicle`);
        }
    }
    ...
    if (altDrivers.length === 0)
        altDrivers.push("authority", "social_proof");
    const alternative = {
    ...
    const scarcityRejectionReasons = [];
    if (!scarcityValidation.allowed) {
        scarcityRejectionReasons.push(...scarcityValidation.blockedReasons);
    }
    else {
        scarcityRejectionReasons.push("Scarcity-based persuasion rejected: requires verified supply constraints — artificial urgency damages trust architecture");
    }
...
function applyPositioningLockConstraints(layers, lockedDecisions, nonGenericAnchors) {
    if (lockedDecisions.length === 0 && nonGenericAnchors.length === 0)
        return layers;
    const l4Index = layers.findIndex(l => l.layerName === "influence_driver_selection");
    if (l4Index === -1)
        return layers;
    const l4 = layers[l4Index];
    const lockConstraintLines = [
    ...
    const updatedL4 = { ...l4, findings: updatedFindings };
    const result = [...layers];
    result[l4Index] = updatedL4;
    return result;
}
export function analyzePersuasion(mi, audience, positioning, differentiation, offer, funnel, integrity, awareness, upstreamLineage = [], analyticalEnrichment) {
    ...
    const allText = [
        mi.marketDiagnosis || "",
        ...(mi.opportunitySignals || []).map((s) => typeof s === "string" ? s : JSON.stringify(s)),
        ...(audience.audiencePains || []).map((p) => typeof p === "string" ? p : JSON.stringify(p)),
        ...(audience.emotionalDrivers || []).map((d) => typeof d === "string" ? d : JSON.stringify(d)),
        positioning.narrativeDirection || "",
        positioning.enemyDefinition || "",
        ...(differentiation.pillars || []).map((p) => typeof p === "string" ? p : JSON.stringify(p)),
        ...(differentiation.claimStructures || []).map((c) => typeof c === "string" ? c : JSON.stringify(c)),
        offer.coreOutcome || "",
    ...
    const lockedDecisions = offer.lockedDecisions || [];
    const nonGenericAnchors = offer.nonGenericAnchors || [];
    ...
    if (lockedDecisions.length > 0) {
        const persuasionOutputText = [
            ...
            ...routes.primary.primaryInfluenceDrivers.map((d) => typeof d === "string" ? d : `${d.driver || ""} ${d.rationale || ""}`),
            ...routes.primary.objectionPriorities.map((o) => typeof o === "string" ? o : `${o.objection || ""} ${o.proofType || ""}`),
        ].join(" ");
        for (const decision of lockedDecisions) {
            ...
            if (similarity < 0.20) {
                crossEngineValidation.push(`POSITIONING LOCK DRIFT: Persuasion output has low semantic alignment (score=${similarity.toFixed(2)}, threshold=0.20) with locked decision "${decisionCore}" — output may have drifted from offer engine contracts`);
            }
        }
    }
    if (nonGenericAnchors.length > 0) {
        const routeText = [
            routes.primary.routeName || "",
            ...routes.primary.primaryInfluenceDrivers.map((d) => typeof d === "string" ? d : d.driver || ""),
        ].join(" ").toLowerCase();
        ...
        if (missingAnchors.length > nonGenericAnchors.length * 0.6) {
            crossEngineValidation.push(`GENERIC DRIFT WARNING: Persuasion route references ${nonGenericAnchors.length - missingAnchors.length}/${nonGenericAnchors.length} positioning anchors — risk of generic output (missing: ${missingAnchors.slice(0, 5).join(", ")})`);
        }
    }
    ...
    const celDepth = enforceEngineDepthCompliance("persuasion", [
        routes.primary.routeName || "",
        routes.primary.persuasionMode || "",
        ...routes.primary.primaryInfluenceDrivers.map((d) => typeof d === "string" ? d : `${d.driver || ""} ${d.rationale || ""}`),
        ...routes.primary.objectionPriorities.map((o) => typeof o === "string" ? o : `${o.objection || ""} ${o.proofType || ""}`),
        ...routes.primary.messageOrderLogic.map((m) => typeof m === "string" ? m : `${m.step || ""} ${m.rationale || ""}`),
    ...
    const acceptability = assessStrategyAcceptability(finalConfidence, finalLayersPassed, allLayers.length, boundaryCheck.passed, structuralWarnings);
    return {
...
export async function runPersuasionEngine(mi, audience, positioning, differentiation, offer, funnel, integrity, awareness, _accountId, upstreamLineage = [], analyticalEnrichment) {
