...
import { rootBundles, businessDataLayer, growthCampaigns, campaignSelections, miSnapshots, audienceSnapshots, positioningSnapshots, differentiationSnapshots, offerSnapshots, funnelSnapshots, awarenessSnapshots, persuasionSnapshots, channelSelectionSnapshots, iterationSnapshots, retentionSnapshots, budgetGovernorSnapshots, strategyValidationSnapshots, strategicPlans, calendarEntries, requiredWork, contentDna, } from "../shared/schema";
...
export async function buildRootBundle(campaignId, accountId) {
    ...
    const [miData, audData, posData, diffData, offerData, funnelData, awarenessData, persuasionData, channelData, iterData, retentionData, budgetData, statValData,] = await Promise.all([
    ...
    const businessRoots = {
        ...
        preferredChannels: channels?.selectedChannels?.map((c) => c.channel || c) || undefined,
    };
    const funnelRoots = {
    ...
    const contentRoots = {
        messagingAngles: territories.length > 0 ? territories.map((t) => t.name || t) : undefined,
        objections: pains.length > 0 ? pains.map((p) => p.pain || p.name || p).slice(0, 5) : undefined,
        contentPillars: pillars.length > 0 ? pillars.map((p) => p.name || p).slice(0, 5) : undefined,
...
export async function getActiveRootBundle(campaignId, accountId) {
    const [bundle] = await db.select().from(rootBundles)
        .where(and(eq(rootBundles.campaignId, campaignId), eq(rootBundles.accountId, accountId), eq(rootBundles.status, "active")))
        .orderBy(desc(rootBundles.createdAt))
        .limit(1);
    return bundle || null;
}
export async function getNextVersion(campaignId, accountId) {
...
export async function lockRootBundle(campaignId, accountId) {
...
export async function detectStaleness(campaignId, accountId) {
    const active = await getActiveRootBundle(campaignId, accountId);
    if (!active)
        return { isStale: false, reason: null, currentHash: null, activeHash: null };
...
export async function validateRootIntegrity(planId) {
    const issues = [];
    ...
    const calMismatchCount = await db.select({
        ...
        .where(and(eq(calendarEntries.planId, planId), sql `(${calendarEntries.rootBundleId} IS DISTINCT FROM ${plan.rootBundleId})`));
...
export function computeCalendarDeviation(planTargets, actualCounts) {
    const THRESHOLD = 5;
    const types = ["reels", "posts", "stories", "carousels", "videos"];
    const deviations = {};
    let maxDev = 0;
...
export function registerRootBundleRoutes(app) {
    app.get("/api/root-bundle/:campaignId", async (req, res) => {
        try {
    ...
    app.post("/api/root-bundle/lock", async (req, res) => {
        try {
    ...
    app.get("/api/root-bundle/:campaignId/integrity/:planId", async (req, res) => {
        try {
    ...
    app.get("/api/root-bundle/:campaignId/staleness", async (req, res) => {
        try {
            const campaignId = req.params.campaignId;
            const accountId = resolveAccountId(req);
            const staleness = await detectStaleness(campaignId, accountId);
