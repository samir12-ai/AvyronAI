...
import { db } from "./db";
import { auditLog, accountState, jobQueue, strategicPlans, featureFlags, aiUsageLog, strategyDecisions, strategyMemory, decisionOutcomes } from "@shared/schema";
...
import { DECISION_ACTION_POLICIES } from "./decision-policy";
...
const MODULE_EVENT_MAP: Record<string, string[]> = {
  ...
  enforcement: [
    "BLOCKED_DECISION",
    "DECISION_REJECTED_UNBOUND",
    "MEMORY_WRITE_BLOCKED",
    "MEMORY_FALLBACK_SOURCE_ACTIVE",
    "ENFORCEMENT_FAILURE_MEMORY_CONSTRAINTS",
    "SYNTHESIS_DEGRADED_AI_FAILED",
    "SYNTHESIS_DEGRADED_NO_DECISIONS",
    "PLAN_INTEGRITY_WARNING",
    "AUTOPILOT_BLOCKED_NO_PLAN",
  ],
};
...
export function registerAuditRoutes(app: Express) {
  ...
  // ─── 9) GET /api/audit/enforcement ─ Phase 3 Decision Enforcement Audit Trail ──
  app.get("/api/audit/enforcement", async (req: Request, res: Response) => {
    try {
      const accountId = resolveAccountId(req);
      const limit = Math.min(Math.max(parseInt(req.query.limit as string) || 50, 1), 200);
      const campaignId = req.query.campaign_id as string | undefined;
      ...
      const outcomes = await db
        ...
        .orderBy(desc(decisionOutcomes.executedAt))
        .limit(limit);

      const outcomeByDecisionId = new Map<string, any>();
      for (const o of outcomes) {
        outcomeByDecisionId.set(o.decisionId, o);
      }

      const memoryEntries = await db
        .select({
          id: strategyMemory.id,
          label: strategyMemory.label,
          confidenceScore: strategyMemory.confidenceScore,
          direction: strategyMemory.direction,
          engineName: strategyMemory.engineName,
          lastValidatedAt: strategyMemory.lastValidatedAt,
        })
        .from(strategyMemory)
      ...
      const highConfidenceMemory = memoryEntries.filter(
        (m) => (m.confidenceScore ?? 0) >= 0.65,
      );
      const lowConfidenceMemory = memoryEntries.filter(
        (m) => (m.confidenceScore ?? 0) < 0.65,
      );
      ...
      const decisionSummary = decisions.map((d) => {
        ...
        return {
          ...
          executedAt: (d as any).executedAt ?? null,
          outcome: outcome
            ? {
                preRoas: outcome.preMetricsRoas,
                postRoas: outcome.postMetricsRoas,
                preCtr: outcome.preMetricsCtr,
                postCtr: outcome.postMetricsCtr,
      ...
      res.json(
        successResponse({
          enforcement: {
            decisionPolicy: Object.values(DECISION_ACTION_POLICIES).map((p) => ({
              type: p.type,
              planEligible: p.planEligible,
              memoryEligible: p.memoryEligible,
              minConfidenceRequired: p.minConfidenceLevel,
              allowedActions: p.allowedActions,
              description: p.description,
            })),
            memoryIntegrity: {
              highConfidenceEntries: highConfidenceMemory.length,
              lowConfidenceEntries: lowConfidenceMemory.length,
              writePolicyMinConfidence: 0.65,
              provisionalPeriodsRequired: 2,
              fallbackPenaltyApplied: 0.15,
              flaggedEntries: lowConfidenceMemory.map((m) => ({
                label: m.label,
          ...
          outcomeTracking: {
            ...
            pendingEvaluation: outcomes.filter((o) => !o.outcome).length,
            evaluated: outcomes.filter((o) => !!o.outcome).length,
          },
        }),
      );
    } catch (err: any) {
      errorResponse("DECISION_AUDIT_ERROR", err.message || "Failed to fetch decision audit", 500, res);
    }
  });
