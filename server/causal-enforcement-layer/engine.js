...
import { classifyClaims, buildClaimBreakdown, extractFactualClaims, } from "./claim-classifier";
const LOG_PREFIX = "[CEL]";
const celReportCache = new Map();
const CEL_REPORT_TTL = 30 * 60 * 1000;
export function storeCELReport(campaignId, accountId, report) {
...
export function getCachedCELReport(campaignId, accountId) {
    const key = `${accountId}:${campaignId}`;
...
export function extractDominantCausalThemes(ael) {
...
export function enforcePositioningCompliance(territories, ael) {
    const result = {
...
export function enforceGenericEngineCompliance(engineId, outputTexts, ael) {
    const result = {
        engineId,
        passed: true,
        score: 1.0,
...
export function buildCELReport(campaignId, aelVersion, engineResults) {
...
export function buildCausalDirectiveForPrompt(ael) {
    if (!ael || !ael.root_causes || ael.root_causes.length === 0)
        return "";
    const { themes, primaryTheme, rootCauseTexts } = extractDominantCausalThemes(ael);
    if (!primaryTheme)
...
export function applyCompliancePenalties(territories, compliance) {
    if (compliance.passed && compliance.violations.length === 0)
        return;
    for (const violation of compliance.violations) {
        const territory = territories.find(t => violation.engineOutput.includes(t.name) ||
...
export function isDepthBlocking(depthResult) {
...
export function buildDepthRejectionDirective(depthResult, attempt, lockedDecisions) {
    const lines = [
    ...
    for (const v of depthResult.violations) {
        ...
        if (v.requiredDirection)
            lines.push(`     FIX: ${v.requiredDirection}`);
        if (v.rootCause)
            lines.push(`     USE THESE VERBATIM ROOT CAUSES IN YOUR OUTPUT: "${v.rootCause}"`);
    }
...
export function buildDepthGateResult(depthResult, attempt, maxAttempts, regenerationLog) {
...
export function enforceEngineDepthCompliance(engineId, outputTexts, ael, preClassifiedClaims) {
    ...
    for (const pattern of GENERIC_FALLBACK_PATTERNS) {
        ...
        if (matches)
            genericTermCount += matches.length;
    }
    ...
    result.enforcementLog.push(`DEPTH_CHECK: depthScore=${result.causalDepthScore} | rootCauseRefs=${rootCauseRefCount} | chainRefs=${chainRefCount} | barrierRefs=${barrierRefCount} | generic=${genericTermCount} | shallow=${shallowPatternCount}`);
    result.enforcementLog.push(`RESULT: passed=${result.passed} | score=${result.score.toFixed(2)} | violations=${result.violations.length} (blocking=${blockingViolations.length}, major=${majorViolations.length}, minor=${minorViolations.length})`);
    return result;
}
export function applyDepthPenalty(confidenceScore, depthResult) {
    if (depthResult.passed && depthResult.violations.length === 0)
