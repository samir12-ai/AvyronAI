...
(function (ClaimType) {
    ClaimType["FactualClaim"] = "factual_claim";
    ClaimType["InferredInsight"] = "inferred_insight";
    ClaimType["EmotionalSynthesis"] = "emotional_synthesis";
})(ClaimType || (ClaimType = {}));
const FACTUAL_RULE_PATTERNS = [
    /\b\d+\s*%/,
    /\b\d+x\b/i,
...
function applyRulePass(sentence) {
...
function applyEmbeddingPass(sentence) {
...
export function classifyClaims(text) {
...
export function buildClaimBreakdown(claims) {
    let factual = 0;
    let inferred = 0;
    let emotional = 0;
    for (const c of claims) {
        if (c.type === ClaimType.FactualClaim)
            factual++;
        else if (c.type === ClaimType.InferredInsight)
            inferred++;
        else
            emotional++;
    }
...
export function extractFactualClaims(claims) {
    return claims.filter(c => c.type === ClaimType.FactualClaim).map(c => c.text);
}
export function extractNonFactualClaims(claims) {
    return claims.filter(c => c.type !== ClaimType.FactualClaim).map(c => c.text);
}
