...
import { enforceGenericEngineCompliance, buildCELReport, storeCELReport, } from "../causal-enforcement-layer/engine";
import { initializeSignalGovernance, resolveSignalsForEngine, getGovernanceSummary, } from "../signal-governance/engine";
...
import { orchestratorJobs, growthCampaigns, businessDataLayer, integritySnapshots, budgetGovernorSnapshots, channelSelectionSnapshots, iterationSnapshots, retentionSnapshots, iterationGateInputs, manualCampaignMetrics, retentionGateInputs, manualRetentionMetrics, } from "@shared/schema";
import { eq, desc, and } from "drizzle-orm";
import { ENGINE_PRIORITY_ORDER, checkPriorityViolation, shouldBlockDownstream, } from "./priority-matrix";
import { synthesizePlan } from "./plan-synthesis";
import { buildMemoryContext, serializeMemoryContextForPrompt, } from "./memory-context";
...
async function getIterationGateData(accountId, campaignId) {
    ...
    const missing = [];
    const prefillable = {};
    if (!gateRow?.primaryKpi)
        missing.push("primaryKpi");
    else
        prefillable["primaryKpi"] = gateRow.primaryKpi;
    if (!gateRow?.dataWindowDays)
        missing.push("dataWindowDays");
    else
        prefillable["dataWindowDays"] = String(gateRow.dataWindowDays);
    const hasAnyMetric = metricsRow && ((metricsRow.spend || 0) > 0 ||
    ...
    if (!hasAnyMetric) {
        ...
        if (metricsRow?.spend)
            prefillable["spend"] = String(metricsRow.spend);
        if (metricsRow?.impressions)
            prefillable["impressions"] = String(metricsRow.impressions);
        if (metricsRow?.clicks)
            prefillable["clicks"] = String(metricsRow.clicks);
        if (metricsRow?.conversions)
            prefillable["conversions"] = String(metricsRow.conversions);
        if (metricsRow?.revenue)
            prefillable["revenue"] = String(metricsRow.revenue);
    }
    return {
...
async function getRetentionGateData(accountId, campaignId) {
    const [gateRow] = await db
        .select()
    ...
    const missing = [];
    const prefillable = {};
    if (!gateRow?.retentionGoal)
        missing.push("retentionGoal");
    else
        prefillable["retentionGoal"] = gateRow.retentionGoal;
    if (!gateRow?.businessModel)
        missing.push("businessModel");
    else
        prefillable["businessModel"] = gateRow.businessModel;
    if (!gateRow?.reachableAudience)
        missing.push("reachableAudience");
    else
        prefillable["reachableAudience"] = gateRow.reachableAudience;
    if (!metricsRow || (metricsRow.totalCustomers || 0) <= 0)
        missing.push("totalCustomers");
    else
        prefillable["totalCustomers"] = String(metricsRow.totalCustomers);
    if (!metricsRow || metricsRow.returningCustomers == null)
        missing.push("returningCustomers");
    else
        prefillable["returningCustomers"] = String(metricsRow.returningCustomers);
    if (!metricsRow || (metricsRow.totalPurchases || 0) <= 0)
        missing.push("totalPurchases");
    else
        prefillable["totalPurchases"] = String(metricsRow.totalPurchases);
    if (!metricsRow || ![30, 60, 90].includes(metricsRow.dataWindowDays || 0))
        missing.push("dataWindowDays");
    else
        prefillable["dataWindowDays"] = String(metricsRow.dataWindowDays);
    return {
        gateInputs: gateRow || null,
...
function extractMiInput(miResult) {
    if (!miResult?.output)
        return {};
...
function extractAudienceInput(audienceResult) {
    if (!audienceResult)
        return {};
    return {
...
function extractPositioningInput(positioningResult) {
    if (!positioningResult)
        return {};
    const out = positioningResult.output || positioningResult;
    const territories = out.territories || positioningResult.territories || [];
    return {
        territories,
        ...
        domainFailures: territories.map((t) => t.domainFailure).filter(Boolean),
        operationalProblems: territories.map((t) => t.operationalProblem).filter(Boolean),
        proofRequirements: territories.map((t) => t.proofRequirement).filter(Boolean),
    };
}
function extractDifferentiationInput(diffResult) {
    if (!diffResult)
        return {};
    return {
...
function buildUpstreamLineage(ctx) {
    const entries = [];
    let idx = 0;
...
function resolveSglOrBlock(engineId, ctx, startTime) {
    if (!ctx.sglState)
        return null;
...
async function executeEngine(engineId, ctx, config, results) {
    ...
    try {
        ...
        switch (engineId) {
            case "market_intelligence": {
                const result = await MarketIntelligenceV3.run("FULL", config.accountId, config.campaignId, config.forceRefresh || false, "STRATEGY_MODE");
                output = result;
            ...
            case "audience": {
                ...
                if (ctx.audience) {
                    try {
                        ...
                        const mappedObjections = rawObjections.map((o) => ({
                        ...
                        ctx.sglState = initializeSignalGovernance(ctx.audience.structuredSignals || { pain_clusters: [], desire_clusters: [], pattern_clusters: [], root_causes: [], psychological_drivers: [] }, mappedObjections);
            ...
            case "positioning": {
                ...
                {
                    const sglBlock = resolveSglOrBlock("positioning", ctx, startTime);
                    if (sglBlock)
                        return sglBlock;
                }
                const result = await runPositioningEngine(config.accountId, config.campaignId, ctx.miSnapshotId, ctx.audienceSnapshotId, ctx.analyticalEnrichment);
                output = result;
                snapshotId = result.snapshotId;
                ...
                if (ctx.analyticalEnrichment && result.territories) {
                    const posTexts = result.territories.map((t) => `${t.name} ${t.contrastAxis} ${t.narrativeDirection}`);
            ...
            case "differentiation": {
                {
                    const sglBlock = resolveSglOrBlock("differentiation", ctx, startTime);
                    if (sglBlock)
                        return sglBlock;
                }
                ...
                const result = await runDifferentiationEngine(miInput, audInput, posInput, config.accountId, undefined, ctx.analyticalEnrichment);
                output = result;
                snapshotId = result.snapshotId;
                ctx.differentiation = result;
                ctx.differentiationSnapshotId = result.snapshotId;
                if (ctx.analyticalEnrichment) {
                    const diffTexts = (result.claims || result.claimStructures || []).map((c) => typeof c === "string" ? c : c.claim || c.title || JSON.stringify(c));
            ...
            case "mechanism": {
                {
                    const sglBlock = resolveSglOrBlock("mechanism", ctx, startTime);
                    if (sglBlock)
                        return sglBlock;
                }
                const posInput = extractPositioningInput(ctx.positioning);
            ...
            case "offer": {
                {
                    const sglBlock = resolveSglOrBlock("offer", ctx, startTime);
                    if (sglBlock)
                        return sglBlock;
                }
                const miInput = extractMiInput(ctx.mi);
                ...
                const result = await runOfferEngine(miInput, audInput, posInput, diffInput, config.accountId, upstreamLineage, ctx.mechanism || undefined, undefined, ctx.analyticalEnrichment);
                output = result;
                snapshotId = result.snapshotId;
                ctx.offer = result;
                if (ctx.analyticalEnrichment) {
            ...
            case "awareness": {
                {
                    const sglBlock = resolveSglOrBlock("awareness", ctx, startTime);
                    if (sglBlock)
                        return sglBlock;
                }
                const miInput = extractMiInput(ctx.mi);
                ...
                const upstreamLineage = buildUpstreamLineage(ctx);
                const result = await runAwarenessEngine(miInput, audInput, posInput, diffInput, offerInput, config.accountId, upstreamLineage, undefined, ctx.analyticalEnrichment);
                output = result;
                snapshotId = result.snapshotId;
            ...
            case "funnel": {
                {
                    const sglBlock = resolveSglOrBlock("funnel", ctx, startTime);
                    if (sglBlock)
                        return sglBlock;
                }
                ...
                const result = await runFunnelEngine(miInput, audInput, offerInput, posInput, diffInput, config.accountId, awarenessInput, ctx.analyticalEnrichment);
                output = result;
                snapshotId = result.snapshotId;
                ctx.funnel = result;
                if (ctx.analyticalEnrichment) {
                    const funnelTexts = (result.stages || []).map((s) => `${s.name || ""} ${s.objective || ""} ${s.contentStrategy || ""}`);
            ...
            case "integrity": {
                ...
                const result = runIntegrityEngine(miInput, audInput, posInput, diffInput, offerInput, funnelInput);
                output = result;
            ...
            case "persuasion": {
                {
                    const sglBlock = resolveSglOrBlock("persuasion", ctx, startTime);
                    if (sglBlock)
                        return sglBlock;
                }
                const miInput = extractMiInput(ctx.mi);
                ...
                const result = await runPersuasionEngine(miInput, audInput, posInput, diffInput, offerInput, funnelInput, integrityInput, awarenessInput, config.accountId, persuasionLineage, ctx.analyticalEnrichment);
                output = result;
                snapshotId = result.snapshotId;
                ctx.persuasion = result;
                if (ctx.analyticalEnrichment) {
                    const persTexts = (result.sequences || result.persuasionSequence || []).map((s) => typeof s === "string" ? s : s.technique || s.name || JSON.stringify(s));
            ...
            case "statistical_validation": {
                ...
                const result = await runStatisticalValidationEngine(miInput, audInput, offerInput, funnelInput, awarenessInput, persuasionInput, config.accountId, []);
                output = result;
                snapshotId = result.snapshotId;
            ...
            case "channel_selection": {
                ...
                const result = runChannelSelectionEngine(audInput, awarenessInput, persuasionInput, offerInput, budgetInput, validationInput, "INTELLIGENT", ctx.memoryContext || undefined);
                output = result;
            ...
            case "iteration": {
                ...
                const result = await runIterationEngine(hasAnyPerf ? performance : null, ctx.funnel || null, null, ctx.persuasion || null, ctx.memoryContext || undefined);
                output = result;
            ...
            case "retention": {
                ...
                const purchaseMotivations = audiencePains.slice(0, 4).map((p) => ({
                    ...
                    category: "pain_alleviation",
                })).filter((m) => m.motivation !== "Unknown motivation");
...
async function writeStrategyMemoryEntries(config, results, planId, planOutput) {
    try {
        const entries = [];
        ...
        if (channel?.status === "SUCCESS" && channel.output) {
            ...
            const primaryName = out.primaryChannel?.channelName || out.primaryChannel?.name || null;
            if (primaryName) {
        ...
        if (iteration?.status === "SUCCESS" && iteration.output) {
            ...
            const targets = (out.optimizationTargets || []);
            const topTarget = targets[0]?.targetName || targets[0]?.target || null;
            if (topTarget) {
                entries.push({
        ...
        if (retention?.status === "SUCCESS" && retention.output) {
            ...
            const loops = (out.retentionLoops || []);
        ...
        console.log(`[Orchestrator] MEMORY_WRITTEN | planId=${planId} | entries=${entries.length} | written=${mutationResult.written} | updated=${mutationResult.updated} | decayed=${mutationResult.decayed}`);
    }
    catch (memErr) {
        console.warn(`[Orchestrator] MEMORY_WRITE_FAILED | error=${memErr.message}`);
    }
}
export async function runOrchestrator(config) {
    ...
    if (config.pausedJobId) {
        ...
        if (pausedJob?.sectionStatuses) {
            try {
                previousSectionStatuses = JSON.parse(pausedJob.sectionStatuses);
            }
            catch { }
        }
        ...
        if (!config.preassignedJobId) {
            await db.insert(orchestratorJobs).values({
                ...
                campaignId: config.campaignId,
                status: "RUNNING",
                sectionStatuses: JSON.stringify(ENGINE_PRIORITY_ORDER.map(e => ({ id: e.id, name: e.name, status: "PENDING" }))),
            });
        }
    }
    ...
    const startIndex = config.resumeFromEngine
        ...
        : 0;
    const ENGINE_TIMEOUT_MS = 120000; // 2-minute hard ceiling per engine
    ...
    for (let i = effectiveStartIndex; i < ENGINE_PRIORITY_ORDER.length; i++) {
        ...
        if (scopedEngineSet && !scopedEngineSet.has(engineDef.id)) {
            ...
            results.set(engineDef.id, { engineId: engineDef.id, status: "SKIPPED", output: null, durationMs: 0 });
            continue;
        }
    ...
    await db.update(orchestratorJobs)
        ...
        sectionStatuses: JSON.stringify(ENGINE_PRIORITY_ORDER.map(e => {
            const r = results.get(e.id);
            const status = r?.status || "PENDING";
            return {
                id: e.id,
...
export async function getOrchestratorStatus(jobId) {
    const [job] = await db
        .select()
        .from(orchestratorJobs)
        .where(eq(orchestratorJobs.id, jobId))
    ...
    if (job.status === "NEEDS_INPUT" && job.needsInputFields) {
        try {
            needsInput = JSON.parse(job.needsInputFields);
        }
        catch { }
    }
    return {
...
export async function getLatestOrchestratorRun(accountId, campaignId) {
    const [job] = await db
        .select()
        .from(orchestratorJobs)
