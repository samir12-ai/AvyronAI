import { db } from "../db";
import { growthCampaigns, businessDataLayer, strategicPlans, requiredWork, calendarEntries, studioItems, orchestratorJobs, manualCampaignMetrics, miSnapshots, audienceSnapshots, positioningSnapshots, differentiationSnapshots, mechanismSnapshots, offerSnapshots, funnelSnapshots, integritySnapshots, awarenessSnapshots, persuasionSnapshots, strategyValidationSnapshots, budgetGovernorSnapshots, channelSelectionSnapshots, iterationSnapshots, retentionSnapshots, contentDna, goalDecompositions, growthSimulations, executionTasks, planAssumptions, contentPerformanceSnapshots, } from "@shared/schema";
import { eq, and, desc, count, sql } from "drizzle-orm";
import { getActiveRootBundle, detectStaleness } from "../root-bundle";
export async function loadSystemContext(accountId, campaignId) {
    const warnings = [];
    ...
    const [bizData] = await db
        ...
        .limit(1);
    if (!campaign)
        warnings.push("No active campaign found");
    if (!bizData)
        warnings.push("Business profile not configured");
    ...
    if (activePlan) {
        ...
        const calendarStats = await db
            .select({
            total: count(),
            pending: sql `count(case when status = 'PENDING' or status = 'DRAFT' then 1 end)`,
            completed: sql `count(case when status = 'COMPLETED' or status = 'PUBLISHED' then 1 end)`,
        })
        ...
        const studioStats = await db
            ...
            ready: sql `count(case when status = 'READY' then 1 end)`,
    ...
    let contentPerformanceSummary = null;
    try {
...
export function buildSystemPrompt(context) {
    ...
    if (context.activePlan) {
        ...
        if (context.activePlan.summary)
            lines.push(`  Summary: ${context.activePlan.summary}`);
    }
    else {
    ...
    if (context.engineStatus) {
        lines.push(`ENGINES: Last run ${context.engineStatus.status} | ${context.engineStatus.sections?.filter((s) => s.status === "SUCCESS").length || 0} engines completed`);
    }
    ...
    if (context.contentDnaSnapshot) {
        ...
        if (d.executionRules) {
            if (d.executionRules.alwaysInclude?.length)
                lines.push(`  Always include: ${d.executionRules.alwaysInclude.join(", ")}`);
            if (d.executionRules.neverDo?.length)
                lines.push(`  Never do: ${d.executionRules.neverDo.join(", ")}`);
        }
    }
    else {
        lines.push("");
    ...
    if (context.simulation) {
        ...
        if (sim.conservativeCase)
            lines.push(`  Conservative: ${sim.conservativeCase.expectedCustomers || sim.conservativeCase.expectedLeads || "N/A"} (${sim.conservativeCase.achievementPct || 70}%)`);
        if (sim.baseCase)
            lines.push(`  Base: ${sim.baseCase.expectedCustomers || sim.baseCase.expectedLeads || "N/A"} (${sim.baseCase.achievementPct || 100}%)`);
        if (sim.upsideCase)
            lines.push(`  Upside: ${sim.upsideCase.expectedCustomers || sim.upsideCase.expectedLeads || "N/A"} (${sim.upsideCase.achievementPct || 115}%)`);
        if (sim.bottleneckAlerts?.length)
            lines.push(`  Bottlenecks: ${sim.bottleneckAlerts.join(", ")}`);
    }
    ...
    if (context.contentPerformanceSummary) {
        ...
        if (cp.dominantFormat)
            lines.push(`  Best performing: ${cp.dominantFormat.toUpperCase()}`);
        if (cp.underperformingFormats.length > 0)
            lines.push(`  Underperforming: ${cp.underperformingFormats.map(f => f.toUpperCase()).join(", ")}`);
        for (const [fmt, data] of Object.entries(cp.byFormat)) {
            if (!data) {
                lines.push(`  ${fmt.toUpperCase()}: No data yet`);
                continue;
            }
