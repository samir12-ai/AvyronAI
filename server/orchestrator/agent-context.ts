  ...
  try {
    const decisions = await db
      .select({
        ...
        autoGenerated: strategyDecisions.autoGenerated,
    ...
    rootBundle: await (async () => {
      try {
        ...
        return {
          ...
          // eslint-disable-next-line semantic/no-semantic-fallback -- D (H8): agent-context display: when not stale, surface active snapshot status as-is for operator visibility
...
export function buildSystemPrompt(context: SystemContext): string {
  ...
  if (context.recentDecisions && context.recentDecisions.length > 0) {
    lines.push("");
    lines.push(`AUTONOMOUS AGENT DECISIONS (last ${context.recentDecisions.length}, most recent first):`);
    for (const d of context.recentDecisions) {
      const age = d.createdAt ? Math.round((Date.now() - new Date(d.createdAt).getTime()) / 3600000) : null;
      const ageStr = age !== null ? (age < 1 ? "< 1h ago" : `${age}h ago`) : "";
      const autoTag = d.autoGenerated ? "[AUTO]" : "[MANUAL]";
      // eslint-disable-next-line semantic/no-semantic-fallback -- L (H8): agent-context decision-row log line — operator-facing template string, not a decision input
      lines.push(`  ${autoTag} [${(d.priority || "medium").toUpperCase()}] [${(d.riskLevel || "low").toUpperCase()} risk] [${d.status || "pending"}] ${ageStr}`);
      lines.push(`    Trigger: ${d.trigger}`);
      lines.push(`    Action: ${d.action}`);
      lines.push(`    Reason: ${d.reason}`);
    }
    lines.push("  These decisions were generated by the autonomous monitoring agent. Reference them when the user asks what the system has been doing or what insights have surfaced.");
  } else {
    lines.push("");
    lines.push("AUTONOMOUS AGENT DECISIONS: None yet. The agent will begin generating insights as the campaign runs.");
  }
