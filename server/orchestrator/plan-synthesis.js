...
function extractLockedDecisions(results) {
    const lines = [];
    const positioning = results.get("positioning");
    if (positioning?.status === "SUCCESS" && positioning.output) {
    ...
    if (mechanism?.status === "SUCCESS" && mechanism.output) {
        ...
        if (out.mechanismName)
            lines.push(`  Mechanism name: "${out.mechanismName}"`);
        if (out.mechanismType)
            lines.push(`  Mechanism type: "${out.mechanismType}"`);
    }
    const offer = results.get("offer");
    if (offer?.status === "SUCCESS" && offer.output) {
        const out = offer.output.output || offer.output;
        if (out.offerName)
            lines.push(`  Offer name: "${out.offerName}"`);
        if (out.coreOutcome)
            lines.push(`  Offer core outcome: "${out.coreOutcome}"`);
    }
    const diff = results.get("differentiation");
    if (diff?.status === "SUCCESS" && diff.output) {
        const out = diff.output.output || diff.output;
        if (out.pillars && out.pillars.length > 0) {
            const pillarNames = out.pillars.map((p) => p.name || p.pillarName).filter(Boolean);
            if (pillarNames.length > 0) {
                lines.push(`  Differentiation pillars: ${pillarNames.map((n) => `"${n}"`).join(", ")}`);
            }
        }
        const authorityMode = out.authorityMode?.mode || (typeof out.authorityMode === "string" ? out.authorityMode : null);
        if (authorityMode && authorityMode !== "unknown")
            lines.push(`  Authority mode: "${authorityMode}"`);
    }
    const persuasion = results.get("persuasion");
    if (persuasion?.status === "SUCCESS" && persuasion.output) {
        const out = persuasion.output.output || persuasion.output;
        if (out.persuasionMode)
            lines.push(`  Persuasion mode: "${out.persuasionMode}"`);
        const primaryRouteName = out.primaryRoute?.routeName || out.primaryRoute?.name;
        if (primaryRouteName)
            lines.push(`  Persuasion primary route: "${primaryRouteName}"`);
    }
    const channel = results.get("channel_selection");
    if (channel?.status === "SUCCESS" && channel.output) {
        const out = channel.output.output || channel.output;
        ...
        if (primaryName)
            lines.push(`  Primary channel: "${primaryName}"${primaryRole ? ` — role: "${primaryRole}"` : ""}`);
        ...
        if (secondaryName)
            lines.push(`  Secondary channel: "${secondaryName}"${secondaryRole ? ` — role: "${secondaryRole}"` : ""}`);
    }
    const funnel = results.get("funnel");
    if (funnel?.status === "SUCCESS" && funnel.output) {
        const out = funnel.output.output || funnel.output;
        if (out.trustPathScore !== undefined)
            lines.push(`  Trust path score: ${out.trustPathScore}`);
        const funnelLabel = out.funnelType || out.funnelName;
        if (funnelLabel)
            lines.push(`  Funnel type: "${funnelLabel}"`);
    }
    return lines.join("\n");
}
async function generatePlanWithAI(engineInsights, businessData, campaign, goalMathContext, lockedDecisions, accountId = "default", memoryContextBlock, campaignId = "", precomputedRhythm) {
...
async function buildDeterministicPlan(businessData, campaign, objective, campaignId, accountId) {
...
function deriveContentVolume(plan, periodDays) {
    ...
    return {
        ...
        totalContentPieces: plan.contentDistribution.reelsPerWeek * weeks +
...
function generateCalendarSlots(plan, periodDays, campaignId, accountId, planId, rootBundleId = null, rootBundleVersion = null) {
    const slots = [];
...
export async function synthesizePlan(config, ctx, results, memoryContextBlock) {
    const [bizData] = await db
        .select()
        .from(businessDataLayer)
        .where(and(eq(businessDataLayer.accountId, config.accountId), eq(businessDataLayer.campaignId, config.campaignId)))
        .limit(1);
    ...
    const calendarSlots = generateCalendarSlots(synthesized, periodDays, config.campaignId, config.accountId, plan.id, rootBundle?.id || null, rootBundle?.version || null);
    ...
    if (goalMathContext) {
        try {
            await generateSimulation(config.campaignId, config.accountId, plan.id, goalMathContext.goal, goalMathContext.funnel, goalMathContext.feasibility, bizData, rootBundle?.id || null);
    ...
    if (readiness && readiness.assumptions.length > 0) {
        try {
            const assumptions = readiness.assumptions.map((a) => ({
                assumption: a.item,
                confidence: a.confidence,
                impactSeverity: a.impact,
                source: "plan_gate",
