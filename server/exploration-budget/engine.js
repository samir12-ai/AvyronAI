...
function explorationRateFromConfidence(avgConfidence) {
...
export async function computeExplorationBudget(campaignId, accountId, memoryBlock, totalWeeklySlots) {
    ...
    const perfSnapshots = await db
        ...
        .where(and(eq(contentPerformanceSnapshots.accountId, accountId), eq(contentPerformanceSnapshots.campaignId, campaignId)))
...
async function persistExplorationBudget(campaignId, accountId, budget) {
    ...
    const existing = await db
        .select()
        .from(strategyMemory)
        .where(and(eq(strategyMemory.accountId, accountId), eq(strategyMemory.campaignId, campaignId), eq(strategyMemory.memoryType, "exploration_budget"), eq(strategyMemory.engineName, "exploration-budget")))
        .limit(1);
