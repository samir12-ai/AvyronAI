...
function generateTasksFromPlan(planData, periodDays) {
    const tasks = [];
...
export async function composeTasks(planId, campaignId, accountId, planData, periodDays, rootBundleId = null) {
    await db.delete(executionTasks).where(and(eq(executionTasks.planId, planId), eq(executionTasks.campaignId, campaignId)));
...
export function registerTaskComposerRoutes(app) {
    app.get("/api/execution-tasks/:campaignId", async (req, res) => {
        try {
            ...
            if (!plan)
                return res.json({ hasTasks: false, tasks: [] });
    ...
    app.post("/api/execution-tasks/:taskId/status", async (req, res) => {
        try {
    ...
    app.post("/api/execution-tasks/generate", async (req, res) => {
        try {
            const { campaignId, planId } = req.body;
            if (!campaignId || !planId)
                return res.status(400).json({ error: "campaignId and planId required" });
            const accountId = resolveAccountId(req);
