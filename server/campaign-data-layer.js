import { db } from "./db";
import { performanceSnapshots, revenueEntries, adSpendEntries, leads, conversionEvents, publishedPosts, ctaVariants, accountState, strategicPlans, calendarEntries, manualCampaignMetrics, } from "@shared/schema";
import { eq, desc, and, isNotNull, min, max, gte, inArray } from "drizzle-orm";
...
export async function resolveDataMode(accountId) {
    try {
...
export async function getManualMetrics(campaignId, accountId = "default") {
    try {
...
export async function getDashboardMetrics(campaignId, accountId = "default") {
    ...
    if (mode === "REAL" && hasMetaData) {
        return {
            ...
            metrics: {
                ...
                contentCount: planMetrics.hasPlan ? planMetrics.generatedPieces : realMetrics.contentCount,
                queuedCount: 0,
    ...
    if (hasManualData) {
        ...
        return {
            ...
            metrics: {
                ...
                contentCount: planMetrics.hasPlan ? planMetrics.generatedPieces : (realMetrics.contentCount || 0),
                queuedCount: 0,
    ...
    const dataSource = planMetrics.hasPlan ? "PLAN" : "NONE";
    return {
...
function snapshotFilter(campaignId, accountId) {
    return and(eq(performanceSnapshots.accountId, accountId), eq(performanceSnapshots.campaignId, campaignId), isNotNull(performanceSnapshots.campaignId));
}
function revenueFilter(campaignId, accountId) {
    return and(eq(revenueEntries.accountId, accountId), eq(revenueEntries.campaignId, campaignId), isNotNull(revenueEntries.campaignId));
}
function spendFilter(campaignId, accountId) {
    return and(eq(adSpendEntries.accountId, accountId), eq(adSpendEntries.campaignId, campaignId), isNotNull(adSpendEntries.campaignId));
}
function conversionFilter(campaignId, accountId) {
    return and(eq(conversionEvents.accountId, accountId), eq(conversionEvents.campaignId, campaignId), isNotNull(conversionEvents.campaignId));
}
function leadsFilter(campaignId, accountId) {
    return and(eq(leads.accountId, accountId), eq(leads.campaignId, campaignId), isNotNull(leads.campaignId));
}
function postsFilter(campaignId, accountId) {
    return and(eq(publishedPosts.accountId, accountId), eq(publishedPosts.campaignId, campaignId), isNotNull(publishedPosts.campaignId));
}
export async function getCampaignMetrics(campaignId, accountId = "default") {
    const now = new Date();
...
export async function getRevenueSummary(campaignId, accountId = "default") {
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const allRevenue = await db.select().from(revenueEntries)
        .where(revenueFilter(campaignId, accountId));
    ...
    for (const r of allRevenue) {
        if (r.postId)
            byPost[r.postId] = (byPost[r.postId] || 0) + (r.amount || 0);
        if (r.campaignId)
            byCampaign[r.campaignId] = (byCampaign[r.campaignId] || 0) + (r.amount || 0);
        if (r.funnelStage)
            byFunnelStage[r.funnelStage] = (byFunnelStage[r.funnelStage] || 0) + (r.amount || 0);
    }
...
function computeRolling7DayBaseline(snapshots, metricExtractor) {
    const now = new Date();
...
function validateSignal(metricName, currentValue, baselineValue, rolling, rawSignalType) {
...
function getExpansionSuggestion(metric, ratio) {
    const budgetIncrease = Math.min(Math.round((ratio - 1) * 50), 100);
    let audienceExpansion;
...
export async function detectPerformanceSignals(campaignId, accountId = "default") {
    const signals = [];
    ...
    for (const post of recentWindow) {
        ...
        for (const m of metricChecks) {
            ...
            if (!rolling || rolling.current7d <= 0)
                continue;
