...
import { ENGINE_VERSION, DIFFERENTIATION_SCORE_WEIGHTS, COLLISION_RISK_WEIGHTS, STATUS, MIN_TERRITORIES, MAX_PILLARS, MIN_PILLAR_SCORE, COLLISION_THRESHOLD, STABILITY_MIN_PROOFABILITY, STABILITY_MIN_TRUST_ALIGNMENT, BOUNDARY_HARD_PATTERNS, BOUNDARY_SOFT_PATTERNS, SIGNAL_WEIGHTS, SIGNAL_GROUNDING_MAX_RETRIES, } from "./constants";
import { enforceBoundaryWithSanitization, applySoftSanitization, assessDataReliability, normalizeConfidence, detectGenericOutput, } from "../engine-hardening";
function clamp(v, min = 0, max = 1) {
    return Math.max(min, Math.min(max, v));
}
const OBJECTION_DERIVATION_PATTERNS = [
...
function deriveObjectionsFromAudienceData(audience) {
    const derived = {};
...
function computeProfileGroundingScore(territoryName, profileSignals) {
    if (!profileSignals.hasProfile)
        return 0;
    ...
    for (const token of profileSignals.offerTokens) {
        if (tLower.includes(token) && token.length > 3) {
            semanticBoost += 0.1;
            break;
        }
    }
...
function buildEmptyResult(status, message, executionTimeMs) {
    return {
        status,
...
export function layer1_positioningIntakeLock(positioning) {
    const failures = [];
...
export function layer2_competitorClaimSurfaceMapping(mi) {
    const claims = [];
    ...
    if (multiSource && typeof multiSource === "object") {
        for (const [compId, compData] of Object.entries(multiSource)) {
            if (compData?.website) {
...
export function layer3_claimCollisionDetection(candidateClaims, competitorClaims) {
    const collisions = [];
    for (const candidate of candidateClaims) {
        for (const comp of competitorClaims) {
            ...
            const collisionRisk = clamp(similarity * COLLISION_RISK_WEIGHTS.similarityToCompetitorClaims +
...
export function layer4_proofDemandMapping(audience, territories) {
    const demands = [];
...
export function layer5_trustGapPrioritization(audience, territories) {
    const gaps = [];
    const objections = Object.entries(audience.objectionMap || {});
    ...
    for (const [objection, data] of objections) {
        ...
        const matchedTerritories = territories.filter((t, i) => jaccardSimilarity(objection, territoryNames[i]) > 0.15 ||
            jaccardSimilarity(objection, territoryDescriptions[i]) > 0.08).map(t => t.name);
...
export function layer6_authorityModeSelection(positioning, audience, mi) {
    const maturity = audience.maturityIndex ?? 0.5;
    const awareness = audience.awarenessLevel || "unaware";
    ...
    const hasStrongCompetitors = dominanceData && Array.isArray(dominanceData) && dominanceData.some((d) => d.dominanceScore > 0.7);
...
export function layer7_proofAssetArchitecture(proofDemands, trustGaps, territories) {
    const assets = [];
    const topGaps = trustGaps.slice(0, 5);
    for (const demand of proofDemands) {
        const gapAlignment = topGaps.some(g => demand.requiredFor.some(r => jaccardSimilarity(g.objection, r) > 0.10)) ? 0.3 : 0;
        const territoryAlignment = territories.some(t => demand.requiredFor.includes(t.name)) ? 0.2 : 0;
...
export function layer8_mechanismConstruction(positioning, proofDemands, claims, pillars) {
    ...
    const hasMethodAxis = axes.some((a) => typeof a === "string" ? a.includes("method") || a.includes("process") || a.includes("framework") :
...
export function validateMechanismIsTransformation(core) {
    const failures = [];
...
export function refineMechanismFromTheme(core, pillars, trustGaps) {
...
export function buildMechanismCore(mechanism, pillars, trustGaps, positioning) {
    const MECHANISM_TYPE_MAP = {
        framework: "framework",
...
export function layer9_differentiationPillarScoring(territories, competitorClaims, trustGaps, proofDemands, audience, mi, profileSignals) {
    const pillars = [];
    ...
    const competitorAuthorities = Array.isArray(dominanceData)
        ? dominanceData.map((d) => {
    ...
    for (const territory of territories) {
        ...
        const relevantGaps = trustGaps.filter(g => (g.relatedTerritories && g.relatedTerritories.includes(tName)) ||
        ...
        const positioningPressure = clamp((avgCompetitorAuthority * 0.40) + (narrativeOverlapPressure * 0.35) + (territorySaturation * 0.25));
        const positioningAlignment = clamp(1 - positioningPressure);
        const objectionCoverage = clamp(objections.filter(o => jaccardSimilarity(o, tLower) > 0.10).length / Math.max(objections.length, 1));
        ...
        const overallScore = clamp(uniqueness * DIFFERENTIATION_SCORE_WEIGHTS.uniqueness +
...
export function inferBusinessCategory(profile) {
    if (!profile)
        return "general";
    const combined = `${profile.businessType || ""} ${profile.coreOffer || ""} ${profile.productCategory || ""}`.toLowerCase();
    if (SERVICE_BUSINESS_TYPES.some(t => combined.includes(t)))
        return "service";
    if (PRODUCT_BUSINESS_TYPES.some(t => combined.includes(t)))
        return "product";
    return "general";
}
export function compressDifferentiationPillars(sorted, profile) {
    if (sorted.length <= 1)
        return sorted;
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (similarity >= overlapThreshold) {
            ...
            if (candidate.uniqueness > primary.uniqueness)
                primary.uniqueness = candidate.uniqueness;
            if (candidate.proofability > primary.proofability)
                primary.proofability = candidate.proofability;
...
export function layer10_claimStrengthScoring(pillars, collisions, territories) {
    const claims = [];
    for (const pillar of pillars) {
        const pillarCollisions = collisions.filter(c => jaccardSimilarity(c.candidateClaim, pillar.name) > 0.15);
        ...
        const overallScore = clamp(distinctiveness * 0.30 + believability * 0.25 + defensibility * 0.25 + relevance * 0.20);
        claims.push({
...
export function buildStructuredAELBlock(ael) {
    if (!ael)
        return "";
    const sections = [];
...
export async function layer11_aiRefinement(pillars, claims, mechanism, authorityMode, accountId, analyticalEnrichment, depthRejectionContext, domainContext, mechanismCore, profile) {
    ...
    if (aelBlock)
        console.log(`[DifferentiationEngine-V3] AEL_INJECTED | enrichmentSize=${aelBlock.length}chars | causalDirective=${causalDirective.length}chars | structuredBlock=${aelStructuredBlock.length}chars`);
...
Return JSON:
{
  ...
  "mechanismDescription": "refined mechanism description grounded in root causes"
    ...
    try {
        ...
        const aelRefs = (parsed.pillars || []).map((p, i) => ({
            pillar: i + 1,
            rc: p.rootCauseUsed || "MISSING",
            bb: p.barrierResolved || "MISSING",
        }));
        const claimRefs = (parsed.claims || []).map((c, i) => ({
            claim: i + 1,
            rc: c.rootCauseUsed || "MISSING",
            bb: c.barrierResolved || "MISSING",
        }));
        const rcHits = aelRefs.filter((r) => r.rc !== "MISSING").length;
        const bbHits = aelRefs.filter((r) => r.bb !== "MISSING").length;
        ...
        const refinedPillars = topPillars.map((p, i) => {
            ...
            if (!refined)
                return p;
            if (sanitizeGuardrail(refined.name) || sanitizeGuardrail(refined.description))
                return p;
        ...
        const refinedClaims = topClaims.map((c, i) => {
            ...
            if (!refined)
                return c;
            if (sanitizeGuardrail(refined.claim))
                return c;
...
export function sanitizeGuardrail(text) {
    if (!text)
        return false;
...
export function validateTerritoryEvidenceDensity(pillars, proofDemands, trustGaps, competitorClaims, territories) {
    let downgradedCount = 0;
    const territoryDescMap = new Map();
    if (territories) {
    ...
    const validatedPillars = pillars.map(pillar => {
        ...
        const proofClusters = new Set(proofDemands
            .filter(d => d.requiredFor.includes(tName))
            .map(d => d.category));
        const trustClusters = trustGaps.filter(g => (g.relatedTerritories && g.relatedTerritories.includes(tName)) ||
            jaccardSimilarity(g.objection, tLower) > 0.10 ||
        ...
        let sourceCount = 0;
        if (proofClusters.size > 0)
            sourceCount++;
        if (trustClusters.length > 0)
            sourceCount++;
...
export function layer12_stabilityGuard(pillars, claims, collisions, territories, hasProfileSignals = false) {
    const failures = [];
    const warnings = [];
...
export async function runDifferentiationEngine(mi, audience, positioning, accountId, profile, analyticalEnrichment) {
    ...
    const dataReliability = assessDataReliability(competitorClaimCount, audienceSignalCount, !!positioning.narrativeDirection, !!(l1.territories.length > 0), !!(audience.audiencePains && audience.audiencePains.length > 0), 0);
    ...
    for (let depthAttempt = 1; depthAttempt <= depthGateMaxAttempts; depthAttempt++) {
        ...
        try {
            const domainCtx = {
                domainFailures: positioning.domainFailures || [],
        ...
        celDepth = enforceEngineDepthCompliance("differentiation", [
            ...finalPillars.map(p => `${p.name} ${p.description}`),
            ...finalClaims.map(c => c.claim),
            finalMechanism.description,
        ...
        if (depthAttempt >= depthGateMaxAttempts) {
            ...
            failedResult.celDepthCompliance = celDepth;
            failedResult.depthGateResult = depthGateResult;
            return failedResult;
        }
    ...
    const signalGroundingLog = [];
    let groundingRejectionContext = "";
    let l12Stability;
    let genericOutputCheck;
