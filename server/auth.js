...
function generateToken(userId, email, accountId) {
    return jwt.sign({ userId, email, accountId }, JWT_SECRET, { expiresIn: "14d" });
}
...
export function registerAuthRoutes(app) {
    app.post("/api/auth/register", async (req, res) => {
        try {
            ...
            await featureFlagService.seedDefaultFlags(userAccountId).catch(err => console.error("[Auth] Failed to seed default flags for new account:", err));
    ...
    app.post("/api/auth/login", async (req, res) => {
        try {
            ...
            await featureFlagService.seedDefaultFlags(userAccountId).catch(err => console.error("[Auth] Failed to seed default flags on login:", err));
    ...
    app.post("/api/auth/seen-intro", async (req, res) => {
        const authHeader = req.headers.authorization;
        if (!authHeader?.startsWith("Bearer ")) {
            return res.status(401).json({ error: "Not authenticated" });
        }
    ...
    app.post("/api/onboarding/track", async (req, res) => {
        const authHeader = req.headers.authorization;
        if (!authHeader?.startsWith("Bearer ")) {
            return res.status(401).json({ error: "Not authenticated" });
        }
