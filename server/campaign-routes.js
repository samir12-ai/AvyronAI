import { db } from "./db";
import { campaignSelections, manualCampaignMetrics, manualRetentionMetrics, iterationGateInputs, retentionGateInputs, strategyMemory, userPublicProfiles } from "@shared/schema";
...
const VALID_GOAL_TYPES = ["LEADS", "AWARENESS", "RETARGETING", "SALES", "TESTING"];
export function registerCampaignRoutes(app) {
    ...
    app.post("/api/campaigns/select", async (req, res) => {
        try {
            ...
            const existing = await db
                .select()
                .from(campaignSelections)
                .where(and(eq(campaignSelections.accountId, accountId), eq(campaignSelections.selectedCampaignId, campaignId)))
                .limit(1);
            let selection;
    ...
    app.delete("/api/campaigns/:campaignId", async (req, res) => {
        try {
            ...
            await db.delete(manualRetentionMetrics)
                .where(and(eq(manualRetentionMetrics.campaignId, campaignId), eq(manualRetentionMetrics.accountId, accountId)));
            await db.delete(iterationGateInputs)
                .where(and(eq(iterationGateInputs.campaignId, campaignId), eq(iterationGateInputs.accountId, accountId)));
            await db.delete(retentionGateInputs)
                .where(and(eq(retentionGateInputs.campaignId, campaignId), eq(retentionGateInputs.accountId, accountId)));
    ...
    app.get("/api/campaigns/:campaignId/manual-metrics", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
    ...
    app.put("/api/campaigns/:campaignId/manual-metrics", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
    ...
    app.get("/api/campaigns/:campaignId/retention-metrics", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
    ...
    app.put("/api/campaigns/:campaignId/retention-metrics", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
            ...
            if (tc !== undefined)
                data.totalCustomers = tc;
            if (tp !== undefined)
                data.totalPurchases = tp;
            if (rc !== undefined)
                data.returningCustomers = rc;
            if (aov !== undefined)
                data.averageOrderValue = aov;
            if (rfc !== undefined)
                data.refundCount = rfc;
            if (mc !== undefined)
                data.monthlyCustomers = mc;
            if (dw !== undefined)
                data.dataWindowDays = dw;
            const rawDerived = computeDerivedFromRaw(tc || 0, tp || 0, rc || 0, aov || 0, rfc || 0, dw || 30);
            if (rawDerived.repeatPurchaseRate !== null)
                data.repeatPurchaseRate = rawDerived.repeatPurchaseRate;
            if (rawDerived.refundRate !== null)
                data.refundRate = rawDerived.refundRate;
            if (rawDerived.purchaseFrequency !== null)
                data.purchaseFrequency = rawDerived.purchaseFrequency;
    ...
    app.get("/api/campaigns/:campaignId/iteration-gate", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
            ...
            const [campaignData] = await db.select().from(manualCampaignMetrics)
                ...
                .limit(1);
            const missing = [];
            if (!existing || !existing.hasExistingAsset)
                missing.push("Existing campaign, asset, or funnel to iterate on");
            if (!existing || !existing.primaryKpi)
                missing.push("Primary KPI for optimization");
            if (!existing || !existing.dataWindowDays)
                missing.push("Data window (7, 14, 30, or 60 days)");
            const hasMetric = campaignData && ((campaignData.spend > 0) ||
            ...
            if (!hasMetric)
                missing.push("At least one real performance metric in Campaign Metrics (spend, impressions, clicks, leads, or revenue)");
            res.json({
                success: true,
    ...
    app.put("/api/campaigns/:campaignId/iteration-gate", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
            const data = {
            ...
            const [campaignData] = await db.select().from(manualCampaignMetrics)
                ...
                .limit(1);
            const missing = [];
            if (!result.hasExistingAsset)
                missing.push("Existing campaign, asset, or funnel to iterate on");
            if (!result.primaryKpi)
                missing.push("Primary KPI for optimization");
            if (!result.dataWindowDays)
                missing.push("Data window (7, 14, 30, or 60 days)");
            const hasMetric = campaignData && ((campaignData.spend > 0) ||
                (campaignData.impressions > 0) ||
                (campaignData.clicks > 0) ||
    ...
    app.get("/api/campaigns/:campaignId/retention-gate", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
            ...
            const [retentionData] = await db.select().from(manualRetentionMetrics)
                .where(and(eq(manualRetentionMetrics.campaignId, campaignId), eq(manualRetentionMetrics.accountId, accountId)))
                .limit(1);
            const hasRawData = retentionData && ((retentionData.totalCustomers != null && retentionData.totalCustomers > 0) ||
            ...
            if (!existing || !existing.retentionGoal)
                missing.push("A retention goal (repeat purchase, renewal, churn reduction, or win-back)");
            if (!existing || !existing.businessModel)
                missing.push("Business model (one-time purchase, recurring subscription, retainer, or repeat purchase)");
            if (!existing || !existing.reachableAudience)
                missing.push("A reachable audience after purchase (customer list, email, WhatsApp, phone, or community)");
    ...
    app.put("/api/campaigns/:campaignId/retention-gate", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
            const data = {
            ...
            if (!result.retentionGoal)
                missing.push("A retention goal (repeat purchase, renewal, churn reduction, or win-back)");
            if (!result.businessModel)
                missing.push("Business model (one-time purchase, recurring subscription, retainer, or repeat purchase)");
            if (!result.reachableAudience)
                missing.push("A reachable audience after purchase (customer list, email, WhatsApp, phone, or community)");
            const derived = retentionData ? computeDerivedRetentionMetrics(retentionData) : null;
    ...
    app.get("/api/campaigns/:campaignId/strategy-memory", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
            const entries = await db
                .select()
                .from(strategyMemory)
                .where(and(eq(strategyMemory.campaignId, campaignId), eq(strategyMemory.accountId, accountId)))
    ...
    app.get("/api/campaigns/:campaignId/user-channels", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
            const profiles = await db
                ...
                .where(and(eq(userPublicProfiles.campaignId, campaignId), eq(userPublicProfiles.accountId, accountId)));
    ...
    app.put("/api/campaigns/:campaignId/user-channels", async (req, res) => {
        try {
            const { campaignId } = req.params;
            const accountId = resolveAccountId(req);
            ...
            for (const ch of channelsToUpsert) {
                const existing = await db
                    .select()
                    .from(userPublicProfiles)
                    .where(and(eq(userPublicProfiles.accountId, accountId), eq(userPublicProfiles.campaignId, campaignId), eq(userPublicProfiles.platform, ch.platform)))
                    .limit(1);
                if (existing.length > 0) {
                    const updated = await db
...
function computeDerivedFromRaw(totalCustomers, totalPurchases, returningCustomers, averageOrderValue, refundCount, dataWindowDays) {
...
function computeDerivedRetentionMetrics(metrics) {
    ...
    const retentionStrengthScore = clamp((repeatPurchaseRate * 0.3) +
        ((1 - refundRate) * 0.2) +
        (Math.min(purchaseFrequency / 4, 1) * 0.25) +
        (Math.min(mc / 100, 1) * 0.25), 0, 1);
    return {
...
export async function requireCampaign(req, res, next) {
    try {
        const accountId = resolveAccountId(req);
        ...
        if (requestedCampaignId) {
            selections = await db
                .select()
                .from(campaignSelections)
                .where(and(eq(campaignSelections.accountId, accountId), eq(campaignSelections.selectedCampaignId, requestedCampaignId)))
                .limit(1);
        }
