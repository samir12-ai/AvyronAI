import { ENGINE_VERSION, STATUS, BOUNDARY_HARD_PATTERNS, BOUNDARY_SOFT_PATTERNS, LAYER_WEIGHTS, } from "./constants";
import { assessDataReliability, normalizeConfidence, detectGenericOutput, enforceBoundaryWithSanitization, } from "../engine-hardening";
...
export function layer1_strategicConsistency(positioning, differentiation, offer, funnel, audience) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    ...
    if (core && core.mechanismType !== "none" && core.mechanismName) {
        ...
        const mechanismWords = mechanism.toLowerCase().split(/\s+/).filter((w) => w.length > 4);
        const offerWords = offerMechanism.toLowerCase().split(/\s+/).filter((w) => w.length > 4);
        const overlap = mechanismWords.filter((w) => offerWords.includes(w)).length;
    ...
    if (outcome && pains.length > 0) {
        const painTexts = pains.map((p) => (typeof p === "string" ? p : p?.pain || p?.name || "").toLowerCase());
        const outcomeWords = outcome.toLowerCase().split(/\s+/).filter((w) => w.length > 3);
        const anyPainMatch = painTexts.some((pt) => outcomeWords.some((w) => pt.includes(w)));
...
export function layer2_audienceOfferAlignment(audience, offer, funnel) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    const pains = audience.audiencePains || [];
    ...
    if (pains.length > 0 && outcome) {
        const painTexts = pains.slice(0, 5).map((p) => (typeof p === "string" ? p : p?.pain || p?.name || "").toLowerCase());
        const anyMatch = painTexts.some((pt) => {
            const words = pt.split(/\s+/).filter((w) => w.length > 3);
            return words.some((w) => outcome.includes(w));
        });
        if (anyMatch) {
    ...
    if (objections.length > 0) {
        ...
        const frictionMitigations = (Array.isArray(funnel.frictionMap) ? funnel.frictionMap : []).map((f) => (f.mitigation || "").toLowerCase());
        const trustActions = (funnel.trustPath || []).map((t) => (t.action || "").toLowerCase());
        const allFunnelText = [...frictionMitigations, ...trustActions, ...funnelStages.map((s) => (s.purpose || "").toLowerCase())].join(" ");
        let addressedCount = 0;
        for (const obj of objections.slice(0, 5)) {
            const objWords = obj.toLowerCase().split(/\s+/).filter((w) => w.length > 3);
            if (objWords.some((w) => allFunnelText.includes(w))) {
                addressedCount++;
            }
        }
...
export function layer3_positioningDifferentiationCompatibility(positioning, differentiation) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    ...
    if (narrative.includes("strategic") && pillars.every((p) => {
    ...
    if (enemy && mechanism) {
        const enemyWords = enemy.split(/\s+/).filter((w) => w.length > 3);
        const mechLower = mechanism.toLowerCase();
        const enemyInMechanism = enemyWords.some((w) => mechLower.includes(w));
        if (enemyInMechanism) {
...
export function layer4_offerFunnelCompatibility(offer, funnel) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    ...
    const commitmentMap = { micro: 0.1, low: 0.3, medium: 0.5, high: 0.7, very_high: 0.9 };
...
export function layer5_trustPathContinuity(funnel, differentiation) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    const trustPath = funnel.trustPath || [];
    ...
    const hasProofBeforeCommitment = trustPath.some((t, i) => i < trustPath.length - 1 && t.proofType && t.proofType !== "transparency_proof");
    ...
    const conversionStageIndex = stages.findIndex((s) => s.name === "conversion" || s.conversionGoal === "purchase");
    const proofStageIndex = stages.findIndex((s) => s.name === "consideration" || s.contentType === "proof_content");
    ...
    const proofTypes = new Set(proofPlacements.map((p) => p.proofType));
    const diffProof = differentiation.proofArchitecture || [];
    const diffProofTypes = new Set(diffProof.map((p) => p.category));
...
export function layer6_proofSufficiency(differentiation, offer, funnel) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    ...
    const proofPlacements = funnel.proofPlacements || [];
    const offerStrength = safeNumber(offer.offerStrengthScore, 0.5);
    const proofCategories = new Set(proofArch.map((p) => p.category));
...
export function layer7_conversionFeasibility(funnel, audience, offer) {
    const findings = [];
    const warnings = [];
    let score = 1.0;
    const stages = funnel.stageMap || [];
    ...
    const criticalFriction = frictionMap.filter((f) => f.severity > 0.7).length;
...
const AWARENESS_PERSUASION_MAP = {
...
function validateCrossEngineAlignment(audience, differentiation, offer) {
    const findings = [];
    const warnings = [];
    const awareness = audience.awarenessLevel || "problem_aware";
    ...
    const core = differentiation.mechanismCore;
    if (core && core.mechanismType !== "none" && core.mechanismName) {
        const hasActionVerbs = core.mechanismSteps.some(step => MECHANISM_ACTION_VERBS.some(verb => step.toLowerCase().includes(verb)));
        if (!hasActionVerbs) {
...
export function layer8_systemCoherence(layerResults, mi, positioning, differentiation, offer, funnel, audience) {
    const findings = [];
    const warnings = [];
...
export function runIntegrityEngine(mi, audience, positioning, differentiation, offer, funnel) {
    ...
    const dataReliability = assessDataReliability(competitorCount, signalCount, !!positioning.narrativeDirection, !!(differentiation.pillars && differentiation.pillars.length > 0), !!(audience.audiencePains && audience.audiencePains.length > 0), safeNumber(mi.overallConfidence, 0));
    ...
    let overallIntegrityScore = clamp(allLayers.reduce((sum, l) => sum + l.score * (LAYER_WEIGHTS[l.layerName] || 0.1), 0));
    ...
    const flaggedInconsistencies = allLayers.filter(l => !l.passed).flatMap(l => l.warnings.map(w => `[${l.layerName}] ${w}`));
