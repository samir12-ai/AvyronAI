import { ENGINE_SIGNAL_REQUIREMENTS, } from "./types";
import { SGL_VERSION, MIN_SIGNALS_PER_CATEGORY, MIN_TOTAL_SIGNALS, SIGNAL_CONFIDENCE_FLOOR, LEAKAGE_PATTERNS, RAW_COMMENT_PATTERNS, } from "./constants";
function generateTraceToken() {
    return `SGL_${Date.now()}_${Math.random().toString(36).substr(2, 8)}`;
}
export function isRawComment(text) {
...
export function purifyEvidence(rawEvidence) {
    const purified = [];
    let discarded = 0;
...
function clusterToGovernedSignals(clusters, category, sourceEngine, traceId) {
    return clusters
...
function buildCoverageReport(signals) {
    const counts = {
...
export function initializeSignalGovernance(structuredSignals, objectionMap) {
    const traceToken = generateTraceToken();
    const sourceEngine = "audience";
    const painSignals = clusterToGovernedSignals(structuredSignals.pain_clusters, "pain", sourceEngine, traceToken);
    const desireSignals = clusterToGovernedSignals(structuredSignals.desire_clusters, "desire", sourceEngine, traceToken);
    const patternSignals = clusterToGovernedSignals(structuredSignals.pattern_clusters, "pattern", sourceEngine, traceToken);
    const rootCauseSignals = clusterToGovernedSignals(structuredSignals.root_causes, "root_cause", sourceEngine, traceToken);
    const psychDriverSignals = clusterToGovernedSignals(structuredSignals.psychological_drivers, "psychological_driver", sourceEngine, traceToken);
    const objectionSignals = objectionMap
    ...
    const totalInputClusters = structuredSignals.pain_clusters.length +
    ...
    console.log(`[SGL] INITIALIZED | v=${SGL_VERSION} | trace=${traceToken} | ` +
...
function buildEngineCoverageReport(signals, requiredCategories) {
    const counts = {
        pain: 0, desire: 0, objection: 0,
        pattern: 0, root_cause: 0, psychological_driver: 0,
    };
...
export function resolveSignalsForEngine(state, engineId) {
    const requiredCategories = ENGINE_SIGNAL_REQUIREMENTS[engineId] || [];
    const signals = state.governedSignals.filter(s => requiredCategories.includes(s.category));
    const cleanSignals = [];
    ...
    if (!coverage.coverageSufficient) {
        console.error(`[SGL] COVERAGE_BLOCKED | engine=${engineId} | ` +
    ...
    console.log(`[SGL] RESOLVE | engine=${engineId} | raw=${signals.length} | clean=${cleanSignals.length} | ` +
...
export function checkSignalLeakage(text) {
    const leaks = [];
    for (const pattern of LEAKAGE_PATTERNS) {
        if (pattern.test(text)) {
...
export function getGovernanceSummary(state) {
    return {
        version: SGL_VERSION,
        traceToken: state.traceToken,
