...
const AGENT_TOOLS = [
    {
        ...
        function: {
            ...
            description: "Triggers a full orchestrator re-run for the current campaign. Use when the user wants to regenerate their strategy, refresh their plan, or when significant business changes require a full plan update. This is an intensive operation that runs all 8+ strategy engines sequentially.",
            parameters: {
                type: "object",
                properties: {
                    justification: {
    ...
    },
    {
        ...
        function: {
            ...
            description: "Recomputes the adaptive content rhythm (reels/week, carousels/week, stories/day, posts/week) based on current performance data, competitor benchmarks, and business constraints. Use when the user reports a content format is underperforming, wants to adjust their posting cadence, or after significant performance changes.",
            parameters: {
                type: "object",
                properties: {
                    justification: {
    ...
    },
    {
        ...
        function: {
            ...
            description: "Returns the current engine health, last orchestrator run timestamps, content rhythm state, memory summary, and overall system status for the active campaign. Use when the user asks about system health, what's running, when things last ran, or wants a status overview.",
            parameters: {
                type: "object",
                properties: {},
                required: [],
...
async function handleToolCall(name, args, accountId, campaignId) {
    try {
        switch (name) {
            case "trigger_plan_rerun": {
                ...
                runOrchestrator({ accountId, campaignId, forceRefresh: true }).catch((err) => {
            ...
            case "get_system_status": {
                ...
                const rhythmMemory = await db
                    .select()
                    .from(strategyMemory)
                    .where(and(eq(strategyMemory.accountId, accountId), eq(strategyMemory.campaignId, campaignId), eq(strategyMemory.memoryType, "content_rhythm")))
            ...
            case "explain_forecast_model": {
                ...
                if (funnelMath) {
                    ...
                    if (funnelMath.requiredReach)
                        assumptions.push(`Required reach: ${funnelMath.requiredReach.toLocaleString()}`);
                    if (funnelMath.requiredLeads)
                        assumptions.push(`Required leads: ${funnelMath.requiredLeads.toLocaleString()}`);
                }
...
async function writeAgentActionMemory(accountId, campaignId, toolName, justification, result) {
    try {
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                if (finishReason === "tool_calls" && pendingToolName) {
                    let toolArgs = {};
                    try {
                        toolArgs = JSON.parse(pendingToolArgs);
                    }
                    catch { }
