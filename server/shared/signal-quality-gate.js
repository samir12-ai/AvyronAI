...
function computeSourceReliability(sr) {
    ...
    return Math.min(1, (sampleFactor * 0.25 +
        coverageFactor * 0.25 +
        lifecycleFactor * 0.2 +
        authorityFactor * 0.2 +
        (1 - (sr.varianceScore > 0.5 ? 0.1 : 0)) * 0.1) * lowSamplePenalty);
}
function computeFreshnessFactor(timeWindowDays) {
    if (timeWindowDays <= 7)
        return 1.0;
    if (timeWindowDays <= 14)
        return 0.95;
    if (timeWindowDays <= 30)
        return 0.85;
    if (timeWindowDays <= 60)
        return 0.7;
    return Math.max(0.3, 1 - (timeWindowDays / 180));
}
function computeSemanticStrengthBonus(signal) {
    return Math.min(0.15, signal.strength * 0.15);
}
export function scoreSignalQuality(signalResults) {
    const scored = [];
    let globalIdx = 0;
    for (const sr of signalResults) {
        ...
        for (const sem of sr.semanticSignals) {
            ...
            const rawQuality = (sourceReliability * 0.35 +
                freshness * 0.25 +
                sem.strength * 0.25 +
                strengthBonus +
...
export function crossValidateSignals(signals, signalClusters) {
    const clusterCategories = new Set(signalClusters.map(c => c.category));
    const reinforcedCategories = new Map();
...
export function applyQualityGate(signalResults, signalClusters) {
...
export function filterClustersByQuality(clusters, qualityGate) {
    ...
    for (const cluster of clusters) {
        const qualifiedSignals = cluster.signals.filter(sig => {
            ...
            for (const passed of passedSnippets) {
                if (jaccardSimilarity(norm, passed) >= SNIPPET_SIMILARITY_THRESHOLD)
                    return true;
            }
            return false;
        });
...
export function checkForOrphanClaims(claims, qualityGate) {
