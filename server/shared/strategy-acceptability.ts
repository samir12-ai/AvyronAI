export type StrategyAcceptabilityGrade = "green" | "yellow" | "orange" | "red";

export interface StrategyAcceptability {
  grade: StrategyAcceptabilityGrade;
  confidence: number;
  label: string;
  adaptiveFallback: string;
  reasons: string[];
}

export function assessStrategyAcceptability(
  confidence: number,
  layersPassed: number,
  totalLayers: number,
  ...
  if (confidence > 0.7 && passRate >= 0.8 && integrityPassed && warningCount <= 2) {
    ...
    if (warningCount > 0) reasons.push(`${warningCount} minor warning(s)`);
    return {
      grade: "green",
      confidence,
  ...
  if (confidence >= 0.5 && confidence <= 0.7 && passRate >= 0.5) {
    ...
    reasons.push(`${layersPassed}/${totalLayers} layers passed`);
    if (!integrityPassed) reasons.push("Minor integrity validation failures detected");
    if (warningCount > 2) reasons.push(`${warningCount} structural warning(s)`);
    return {
      grade: "yellow",
      confidence,
  ...
  if (confidence >= 0.3 && confidence < 0.5) {
    ...
    reasons.push(`${layersPassed}/${totalLayers} layers passed`);
    if (!integrityPassed) reasons.push("Significant integrity failures");
    if (warningCount > 0) reasons.push(`${warningCount} structural warning(s)`);
    return {
      grade: "orange",
      confidence,
  ...
  if (totalLayers > 0) reasons.push(`Only ${layersPassed}/${totalLayers} layers passed`);
  if (!integrityPassed) reasons.push("Major integrity failures");
  if (warningCount > 0) reasons.push(`${warningCount} structural warning(s)`);
  return {
    grade: "red",
    confidence,
