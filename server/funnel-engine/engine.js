...
import { ENGINE_VERSION, FUNNEL_STRENGTH_WEIGHTS, GENERIC_FUNNEL_PATTERNS, GENERIC_PENALTY, BOUNDARY_HARD_PATTERNS, BOUNDARY_SOFT_PATTERNS, AWARENESS_TO_FUNNEL_MAP, STATUS, MAX_FUNNEL_STAGES, ENTRY_MECHANISM_TYPES, ENTRY_ROUTE_TO_FUNNEL_FAMILIES, TRUST_LIGHT_FUNNELS, TRIGGER_CLASS_TO_FUNNEL_FAMILY, LOW_READINESS_STAGES, HIGH_READINESS_STAGES, COMMITMENT_TOLERANCE_MAP, FUNNEL_TYPES, AWARENESS_ROUTE_TO_ENTRY_MECHANISMS, TRIGGER_CLASS_TO_ENTRY_MECHANISMS, TRUST_STATE_ENTRY_OVERRIDES, AWARENESS_BLOCKED_FUNNELS, } from "./constants";
...
export function detectGenericFunnel(text) {
    if (!text)
        return false;
...
export function layer1_funnelEligibilityDetection(audience, offer) {
    const reasons = [];
    let score = 0;
...
export function layer2_offerToFunnelFit(offer, audience) {
...
export function layer3_audienceFrictionModeling(audience, funnelType) {
...
export function layer4_trustPathConstruction(audience, differentiation, funnelType) {
    const trustPath = [];
    const awareness = audience.awarenessLevel || "problem_aware";
    ...
    if (proofArch.some((p) => p.category === "case_proof")) {
        trustPath.push({
            step: stepNum++,
    ...
    if (proofArch.some((p) => p.category === "outcome_proof")) {
        trustPath.push({
            step: stepNum++,
    ...
    const hasCase = proofArch.some((p) => p.category === "case_proof");
    const hasOutcome = proofArch.some((p) => p.category === "outcome_proof");
    const hasProcess = proofArch.some((p) => p.category === "process_proof");
...
export function layer5_proofPlacementLogic(differentiation, stages) {
    const proofPlacements = [];
    const proofArch = differentiation.proofArchitecture || [];
    const availableProofTypes = new Set();
    for (const asset of proofArch) {
        if (asset.category)
            availableProofTypes.add(asset.category);
    }
...
export function layer6_commitmentLevelMatching(audience, offer, funnelType) {
    const awareness = audience.awarenessLevel || "problem_aware";
    const maturity = typeof audience.maturityIndex === "number" ? audience.maturityIndex : (Number(audience.maturityIndex) || 0.5);
    ...
    const funnelCommitmentRequirement = {
        direct: "high",
        webinar: "medium",
        challenge: "medium",
        vsl: "medium",
    ...
    if (awareness === "most_aware")
        audienceReadiness = 0.9;
    else if (awareness === "product_aware")
        audienceReadiness = 0.7;
    else if (awareness === "solution_aware")
        audienceReadiness = 0.55;
    else if (awareness === "problem_aware")
        audienceReadiness = 0.35;
    else
        audienceReadiness = 0.2;
    ...
    let commitmentLevel;
    if (audienceReadiness >= 0.7)
        commitmentLevel = "high";
    else if (audienceReadiness >= 0.5)
        commitmentLevel = "medium";
    else if (audienceReadiness >= 0.3)
        commitmentLevel = "low";
    else
...
export function layer7_funnelIntegrityGuard(candidate, audience, offer, positioning, differentiation) {
    const failures = [];
    if (candidate.trustPath.length < 2) {
        failures.push("Trust-before-ask violation: trust path has fewer than 2 steps");
    }
    const trustBeforeCommit = candidate.trustPath.some((s) => s.step < candidate.trustPath.length && s.proofType !== "transparency_proof");
...
export function layer8_funnelStrengthScoring(candidate) {
    const raw = candidate.eligibilityScore * FUNNEL_STRENGTH_WEIGHTS.eligibility +
...
export function layerEntryTriggerDetection(_audience, _offer, _funnelType, awarenessInput) {
    const PURPOSE_MAP = {
    ...
    const allAllowed = new Set(ENTRY_MECHANISM_TYPES);
    const enforcementChain = [];
...
export function compressFunnelStages(stages) {
    ...
    if (overflow.length > 0 && kept.length < MAX_FUNNEL_STAGES) {
        for (const extra of overflow) {
            if (kept.length >= MAX_FUNNEL_STAGES)
                break;
            kept.push(extra);
        }
    }
...
function buildStageMap(funnelType, awareness) {
    const stages = [];
    if (awareness === "unaware" || awareness === "problem_aware") {
        stages.push({
            name: "entry",
...
export function applyAwarenessPriorityMatrix(proposedType, awareness, audience, offer) {
    const allTypes = [...FUNNEL_TYPES];
    let eligible = new Set(allTypes);
    const blocked = [];
    const layers = [];
...
function buildFunnelCandidate(name, funnelType, audience, offer, positioning, differentiation, awarenessInput) {
...
export async function aiFunnelGeneration(audience, offer, positioning, differentiation, accountId, mi, awarenessCtx, analyticalEnrichment, depthRejectionContext) {
    const pains = audience.audiencePains || [];
    const desires = Object.entries(audience.desireMap || {});
    const pillars = differentiation.pillars || [];
...
CRITICAL: The funnel type MUST be compatible with the detected awareness route. A ${awarenessCtx.entryMechanism} entry mechanism requires a funnel that supports ${awarenessCtx.trustState} trust building.`
    ...
    if (aelBlock)
        console.log(`[FunnelEngine-V3] AEL_INJECTED | enrichmentSize=${aelBlock.length}chars | causalDirective=${causalDirective.length}chars`);
...
- Differentiation Pillars: ${JSON.stringify(pillars.slice(0, 3).map((p) => p.name))}
...
${(() => {
        ...
        const ctaPatterns = [];
        const navPatterns = [];
        for (const compData of Object.values(ms)) {
            if (compData?.website) {
                for (const cta of (compData.website.funnelCTAs || compData.website.ctaPatterns || []).slice(0, 3)) {
                    if (typeof cta === "string" && cta.trim())
                        ctaPatterns.push(cta);
                }
                for (const nav of (compData.website.featureHierarchy || compData.website.navigationFlow || []).slice(0, 3)) {
                    if (typeof nav === "string" && nav.trim())
                        navPatterns.push(nav);
                }
            }
        }
        if (ctaPatterns.length === 0 && navPatterns.length === 0)
            return "";
...
export async function runFunnelEngine(mi, audience, offer, positioning, differentiation, accountId, awareness, analyticalEnrichment) {
    ...
    const primaryFunnel = buildFunnelCandidate(aiFunnels.primary.name, aiFunnels.primary.type, audience, offer, positioning, differentiation, awareness);
    ...
    const alternativeFunnel = buildFunnelCandidate(aiFunnels.alternative.name, aiFunnels.alternative.type, audience, offer, positioning, differentiation, awareness);
    ...
    const rejectedFunnel = buildFunnelCandidate(aiFunnels.rejected.name, aiFunnels.rejected.type, audience, offer, positioning, differentiation, awareness);
    diagnostics.layer7_rejected = rejectedFunnel.integrityResult;
    const collectFunnelText = (f) => [
    ...
    const commitmentLevelOrder = {
        micro: 1, low: 2, medium: 3, high: 4, very_high: 5,
    };
    const awarenessReadinessMap = {
    ...
    let celDepth = enforceEngineDepthCompliance("funnel", [
        primaryFunnel.funnelType || "",
        ...(primaryFunnel.frictionMap || []).map((f) => `${f.stage || ""} ${f.frictionType || ""} ${f.mitigation || ""}`),
        ...(primaryFunnel.trustPath || []).map((t) => `${t.stage || ""} ${t.proofType || ""} ${t.rationale || ""}`),
    ...
    if (analyticalEnrichment && isDepthBlocking(celDepth)) {
        for (let funnelDepthAttempt = 2; funnelDepthAttempt <= funnelDepthGateMaxAttempts; funnelDepthAttempt++) {
            ...
            celDepth = enforceEngineDepthCompliance("funnel", [
    ...
    const rawConfidence = clamp(primaryFunnel.funnelStrengthScore *
    ...
    const acceptability = assessStrategyAcceptability(confidenceScore, layersPassed, 8, primaryFunnel.integrityResult.passed && boundaryCheck.passed, structuralWarnings);
