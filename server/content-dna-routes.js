...
import { contentDna, miSnapshots, audienceSnapshots, positioningSnapshots, differentiationSnapshots, offerSnapshots, funnelSnapshots, awarenessSnapshots, persuasionSnapshots, channelSelectionSnapshots, iterationSnapshots, businessDataLayer, } from "../shared/schema";
...
const truncJson = (obj, maxLen = 1200) => {
    const s = typeof obj === "string" ? obj : JSON.stringify(obj);
    return s?.substring(0, maxLen) || "";
};
async function gatherEngineContext(campaignId, accountId) {
    const [miData, audData, posData, diffData, offerData, funnelData, awarenessData, persuasionData, channelData, iterData, businessData,] = await Promise.all([
        db.select({
            ...
            if (multiSource && typeof multiSource === "object") {
                ...
                for (const compData of Object.values(multiSource)) {
                    if (compData?.website) {
                        ...
                        if (ws.headlineExtractions?.length)
                            websiteSignals.push(`Headlines: ${ws.headlineExtractions.slice(0, 3).join(", ")}`);
                        if (ws.ctaPatterns?.length)
                            websiteSignals.push(`CTAs: ${ws.ctaPatterns.slice(0, 3).join(", ")}`);
                    }
                    if (compData?.blog) {
                        const blog = compData.blog;
                        if (blog.topicClusters?.length)
                            blogSignals.push(`Topics: ${blog.topicClusters.slice(0, 3).join(", ")}`);
                    }
                }
...
export async function generateContentDna(campaignId, accountId, planId, rootBundleId, rootBundleVersion) {
...
export async function getLatestContentDna(campaignId, accountId) {
    const rows = await db.select().from(contentDna)
        .where(and(eq(contentDna.campaignId, campaignId), eq(contentDna.accountId, accountId), eq(contentDna.status, "active")))
...
export function registerContentDnaRoutes(app) {
    app.post("/api/content-dna/generate", requireCampaign, async (req, res) => {
        try {
    ...
    app.get("/api/content-dna/:campaignId", async (req, res) => {
        try {
