    ...
    function walk(obj, depth) {
        ...
        if (typeof obj === "string" && obj.length > 10) {
            ...
            for (const key of Object.keys(obj).slice(0, 30)) {
                if (key === "snapshotId" || key === "createdAt" || key === "updatedAt")
                    continue;
...
function checkOutputTraceability(output, sglTraceToken, engineId) {
    if (!output)
        return false;
...
function findOrphanOutputs(output, sglSignalTexts) {
    if (!output || sglSignalTexts.size === 0)
        return [];
    const orphans = [];
    const claims = [];
...
function checkAlignmentPair(sourceOutput, targetOutput, sourceId, targetId) {
...
export function runSystemIntegrityValidation(engineOutputs, sglState) {
    const reportId = `SIV_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
    const engineChecks = [];
    const crossAlignments = [];
    const failureReasons = [];
    const sglSignalTexts = new Set();
    if (sglState) {
    ...
    const summary = `SystemIntegrityValidator | status=${overallStatus} | ` +
