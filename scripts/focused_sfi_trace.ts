...
async function main() {
  ...
  console.log(`=== STARTING FOCUSED SFI TRACE FOR ${campaignId} ===`);
  const t0 = Date.now();
  ...
  try {
    ...
    console.log(`\n=== ORCHESTRATOR EXECUTION FINISHED in ${(Date.now() - t0) / 1000}s ===`);
    console.log(`Status: ${result.status}`);
    console.log(`Job ID: ${result.jobId}`);
    console.log(`System Verdict: ${result.systemVerdict}`);
    console.log(`Execution Mode: ${result.recommendedExecutionMode}`);
    ...
    console.error("Focused SFI trace failed:", err);
  }
