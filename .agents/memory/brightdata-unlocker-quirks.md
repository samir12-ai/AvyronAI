...
## Instagram internal-API endpoints are intercepted at the product level (since ~2026-07-19)
- Unlocker refuses IG internal API URL patterns (`/api/v1/users/web_profile_info/`, `?__a=1&__d=dis`): raw format → HTTP 200 + `x-brd-err-code: http_status` + 0 bytes; `format:"json"` reveals a **BD-synthesized** 400 (carries `x-brd-error*` headers, not relayed from IG); `__a=1` returns a canned "Forbidden. This is the obsolete endpoint…" body. Geo (`country`) and custom `headers` (x-ig-app-id) change nothing.
- IG profile page HTML still fetches (700–790 KB) but is a logged-out shell (`PolarisLoggedOut`, no `edge_owner_to_timeline_media`, no shortcodes) — HTML parsing truthfully yields 0 posts.
- **How to apply:** a fleet-wide simultaneous IG failure (owned AND control handles identical) = provider product change, not account blocks; don't tune backoff/gates. Diagnose with `format:"json"` to see the true target status. Viable fallback proven live: Apify `apify~instagram-profile-scraper` run-sync (~80s) returns profile + latestPosts with shortCode/timestamp/likes/comments on the existing `APIFY_API_KEY`.
