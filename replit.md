...
- **System Control Layer (Phase 2 — Active Enforcement)**: A unified final authority layer in `server/system-control/` that runs after all 15 engines complete and before plan synthesis. Evaluates full system state: structural checks (conversion path, signal grounding, integrity status, CEL compliance, upstream engine health, funnel completeness), cross-engine contradiction detection (budget vs channel, budget vs integrity, funnel vs structure), and produces a single `SystemControlVerdict` with verdict (`PASS|DOWNGRADE|REPAIR|BLOCK`), execution mode (`FULL_EXECUTION|RESTRICTED_EXECUTION|TEST_ONLY|REVIEW_REQUIRED|HALTED`), block reasons, downgrades, and contradiction reports. **Phase 2 active**: BLOCK verdicts override `overallStatus` to BLOCKED (sets `failedEngine="system_control"`), DOWNGRADE verdicts mutate the budget governor's decision action (e.g. `scale→test`) with `originalAction`, `downgradedBy`, and `downgradeReasons` attribution fields. `controlVerdict` is returned in `OrchestratorRunResult`. `CONFLICT_PRIORITY` in `server/conflict-resolver.ts` adds `"system_control"` tier above `"hard_constraints"`. Shadow mode is still available via `evaluateSystemControl(input, { shadowMode: true })`. Does NOT modify memory system, adaptive rhythm, iteration engine, exploration budget, or autonomous worker — reads their outputs only. 75/75 validation tests in `server/tests/system-control-proof.ts`.
