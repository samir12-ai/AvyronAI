...
- **System Control Layer (Phase 1 — Shadow Mode)**: A unified final authority layer in `server/system-control/` that runs after all 15 engines complete and before plan synthesis. Evaluates full system state: structural checks (conversion path, signal grounding, integrity status, CEL compliance, upstream engine health, funnel completeness), cross-engine contradiction detection (budget vs channel, budget vs integrity, funnel vs structure), and produces a single `SystemControlVerdict` with verdict (`PASS|DOWNGRADE|REPAIR|BLOCK`), execution mode (`FULL_EXECUTION|RESTRICTED_EXECUTION|TEST_ONLY|REVIEW_REQUIRED|HALTED`), block reasons, downgrades, and contradiction reports. Currently in shadow mode (observation only — logs verdict but does not affect execution). Integrated at `server/orchestrator/index.ts` between integrity validation and plan synthesis gate. Does NOT modify memory system, adaptive rhythm, iteration engine, exploration budget, or autonomous worker — reads their outputs only. 49/49 validation tests in `server/tests/system-control-proof.ts`.
