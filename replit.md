...
- **Causal Enforcement Layer (CEL)**: A post-generation compliance layer (`server/causal-enforcement-layer/`) that programmatically enforces alignment between AEL root causes and all downstream engine outputs. 7 constraint rules: TRUST_OPACITY, VALUE_PERCEPTION, MECHANISM_COMPREHENSION, FEAR_RISK, IDENTITY_STATUS, KNOWLEDGE_GAP, OVERWHELM_COMPLEXITY. Pre-generation: `buildCausalDirectiveForPrompt()` injects hard constraints into engine prompts (positioning, differentiation, offer, funnel). Post-generation: `enforcePositioningCompliance()` applies scoring penalties (blocking=0, major=-0.30, minor=-0.10) and flags violations. Orchestrator tracks `celResults` across all 5 checked engines and builds a final CEL report. API: `GET /api/cel/report/:campaignId`, `GET /api/cel/rules`. Frontend: `CELCompliancePanel` in AELDebugPanel showing theme detection, active constraints, per-engine pass/fail, and violation details.
