...
- **Studio**: Media library with AI Auto-Fill metadata (Goal, Audience, CTA, Series, Offer generated by AI from title — user reviews/edits before save), AI Video Editor with FFmpeg rendering, and AI Video Analysis Assist for content extraction. Full integration with Google Veo 3.1 (via `@google/genai` SDK, model `veo-3.1-generate-preview`) for text-to-video and image-to-video generation. Backend routes in `server/veo-routes.ts` using `GOOGLE_GEMINI_API_KEY`. `POST /api/studio/ai-metadata` endpoint generates publishing metadata from title/mediaType/platform.
