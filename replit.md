...
- **Input Validation Gates**: Mandatory input gates for Iteration and Retention Engines to ensure foundational data is provided before analysis. Gate UIs display connected campaign/retention metrics as read-only data (no manual re-entry), with auto-detection of existing customer data from retention metrics.
