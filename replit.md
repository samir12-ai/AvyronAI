...
- **Structured Signal Flow (Audience → Positioning)**: The Audience Engine outputs `structuredSignals` with 5 categories: `pain_clusters`, `desire_clusters`, `pattern_clusters`, `root_causes`, `psychological_drivers`. Each cluster has `id`, `label`, `frequency`, `confidence`, `evidence[]`, and `sourceLayer` (surface/pattern/interpretation). **Graceful degradation**: `MIN_EVIDENCE_PER_SIGNAL` lowered to 1 (was 3) — weak signals preserved with capped confidence (≤0.35) instead of being discarded. `applyEvidenceIntegrityFilter` now keeps low-evidence signals at reduced confidence. When all signal clusters are empty, fallback inference from raw content generates minimum viable signals. The Positioning Engine infers structured signals from audience pains/desires when signals are absent or malformed (via `inferStructuredSignalsFromAudience`) instead of returning `SIGNAL_REQUIRED`. Signal enforcement exhaustion produces `LOW_CONFIDENCE` status with best-effort output instead of `SIGNAL_DRIFT`. **Orchestrator cascade**: `SIGNAL_CASCADE_MAP` only blocks downstream on `SIGNAL_REQUIRED` (hard missing data). `LOW_CONFIDENCE` and `SIGNAL_DRIFT` pass through without blocking. Positioning gate status: `SIGNAL_PASSED` for all non-SIGNAL_REQUIRED outcomes. Frontend: `SignalFlowPanel`, `DepthGateStatusPanel` show LOW_CONFIDENCE as amber/informational.
- **Differentiation Signal Grounding Enforcement**: L12 stability guard uses hard `groundingFailures`. Signal grounding enforcement loop (max 3 attempts) wraps L11 AI refinement: validates proofability ≥ 0.15 (lowered from 0.30), trust alignment ≥ 0.10 (lowered from 0.15), no all-weak claims, AND generic output detection — rejects and re-prompts with specific grounding rejection context. **On exhaustion**: produces `LOW_CONFIDENCE` status with 50% confidence penalty and best-effort output (was: `SIGNAL_GROUNDING_FAILED` with confidence=0 and empty result). Orchestrator cascade: differentiation `LOW_CONFIDENCE` → `DEPTH_PASSED` (not blocking). Mechanism and Offer engines always execute when differentiation produces output. Frontend: `DifferentiationEngine.tsx` shows `LOW_CONFIDENCE` as amber informational badge. `DepthGateStatusPanel` shows `LOW CONFIDENCE` label.
