...
- **Authority Hierarchy Enforcement**: Strict Awareness → Funnel → Persuasion authority hierarchy. Awareness decisions are immutable downstream. Funnel cannot contradict awareness stage (AWARENESS_BLOCKED_FUNNELS blocks direct/tripwire/application/product-launch for unaware). Persuasion cannot override awareness conclusions (hard CTA blocked for unaware/problem_aware). Entry trigger enforcement has 3 valid derivation paths only (no implicit fallback). Cross-engine validation detects awareness↔funnel↔persuasion contradictions. Orchestrator gates funnel execution behind completed awareness output (MISSING_DEPENDENCY).
- **Persuasion Engine V4 (Structural Fix)**: Replaced `proof_led_entry` with `education_proof_hybrid` mode for unaware audiences. Proof role = `support_education` (assists understanding + trust building, NOT conversion). Controlled persuasion layer with `micro_tension`, `progressive_curiosity`, `contrast_framing` at calibrated pressure (`low_but_present` for unaware). Message order: `acknowledge_pain → disrupt_assumption → educate_on_problem → introduce_mechanism → reinforce_with_proof → build_authority → remove_risk → soft_next_step`. Anti-hype guard is contextual: hard hype patterns (deceptive claims) always penalized, contextual power words ("game changer", "effortless") evaluated by stage alignment — false positives eliminated. Pressure calibration: indirect for unaware (curiosity + contrast), moderate_indirect for problem_aware, direct for high readiness. For unaware + high trust: light authority injection + structured proof within education flow → Awareness-Persuasion Fit ≥ 75%, Persuasion Strength ≥ 80%.
...
- **Execution Activation Layer**: Auto-triggers content production pipeline when a plan is approved — eliminates the "approved but idle" state. Enforces strict content queue minimums (≥3 Reels, ≥2 Carousels, ≥2 Stories), Reels ratio ≥50%, scheduling completeness validation, and funnel feeding checks. Uses a strict state machine (IDLE → ACTIVATING → ACTIVE → RUNNING → COMPLETED / FAILED / ACTIVATION_FAILED / STARVED). Both approval endpoints auto-trigger activation as fire-and-forget. Content queue violations hard-block activation (ACTIVATION_FAILED). API: `/api/execution-activation/activate/:planId`, `/retry/:planId`, `/status/:planId`, `/content-queue/:planId`, `/validate-funnel/:planId`. Module: `server/execution-activation/`.
...
- **Channel Selection Engine V4 (Awareness-Driven Hard Blocking)**: Channels are hard-blocked (not soft-penalized) based on audience awareness stage via a `CHANNEL_ROLE_REGISTRY` defining per-channel roles, allowed/blocked awareness stages, and restriction rules. `unaware` audiences ONLY receive `social_organic`, `content_platform`, and `community` channels — all paid, search, email, referral, direct, and partnership channels are hard-blocked. `enforceAwarenessConstraint()` performs 3-layer enforcement: registry block list → type block list → role compatibility check. `scoreAwarenessMapping()` returns score=0 for blocked channels, causing immediate candidate rejection. `runDecisionGate()` hard-fails (outcome=exploratory) on awareness score=0. `injectConversionChannel()` filters injection candidates by awareness stage with full constraint enforcement. Awareness layer weight increased to 0.20 (highest). `AWARENESS_INJECTION_PRIORITY` provides stage-specific channel injection order lists. `awarenessConstraintViolation` field tracks blocked channels in results.
...
- **Budget Governor Confidence Reconciliation**: The Budget Governor now reconciles `validationConfidence` (from Statistical Validation Engine, which measures strategic claim quality) with actual campaign performance metrics (conversions, spend, statistical validity). Three tiers: (1) PERFORMANCE OVERRIDE — when ≥100 conversions + ≥$500 spend + statistically valid, confidence is elevated to at least 80% regardless of claim-based score; (2) BLENDED — when statistically valid and campaign confidence exceeds base, a 40/60 weighted blend is applied; (3) PARTIAL BOOST — for early campaigns with some data, a small confidence boost is added. The secondary data-source-confidence gate in routes.ts is also bypassed when performance override criteria are met. Full diagnostics (base vs reconciled confidence, override reason, campaign metrics) are logged in `layerDiagnostics.confidenceReconciliation`. Type: `CampaignPerformanceMetrics` in `budget-governor/types.ts`.
