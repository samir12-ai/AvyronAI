...
- **Snapshot Hardening**: ENGINE_VERSION (v8) in constants.ts, `validateSnapshotCompleteness` guard, version-aware cache invalidation, anti-stale guard (CACHE_INVALID_INCOMPLETE_SNAPSHOT). No COMPLETE snapshot without validated analytical fields. `analysisVersion` column in mi_snapshots.
