...
- **MarketMind Agent (Agent Control Box)**: Replaced old AI Actions panel. Self-contained component (`components/MarketMindAgent.tsx`) backed by `/api/dashboard/agent-brief` (aggregates all 8+ engine outputs, plan data, metrics into AI-generated insight + priority action) and `/api/dashboard/agent-explain` (answers user questions using full engine context). Shows: pulsing status, campaign overview, AI insight, priority action, content progress bar, expandable engine intelligence details, plan section tags, and an interactive "Ask the Agent" chat.
