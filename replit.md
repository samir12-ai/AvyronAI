...
- **Cross-Engine Isolation**: Enforced via `server/strategy/dependency-validation.ts`. Each engine has prohibited write targets preventing cross-domain interference: Channel→no iteration/retention writes, Budget→no channel/iteration writes, Iteration→no budget/channel/retention writes, Retention→no budget/channel/iteration writes. Runtime validation available via POST `/api/system/validate-isolation`. All engines validate dependencies before execution (503 on version mismatch).
- **Channel Selection Engine V2**: Enforces persuasion compatibility threshold (< 50% = channel downgrade to exploratory). Features a mandatory Decision Gate layer that validates persuasion, guard, budget, and awareness conditions before final recommendation. Channels receive Objective Fit Scores (Awareness Fit, Nurture Fit, Conversion Fit) and Channel Differentiation Profiles (audience discovery dynamics, content velocity, algorithm amplification, conversion likelihood). Organic channels (Instagram Organic, TikTok Organic, etc.) receive zero budget allocation (informational only). Budget allocation is normalized to exactly 100% across paid channels with post-rounding reconciliation. Engine dependency validation (`server/strategy/dependency-validation.ts`) enforces version compatibility and Master Plan isolation rules at the route level.
