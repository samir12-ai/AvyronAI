...
- **Competitive Intelligence (MIv3)**: An 8-engine sequential pipeline for real-data competitor analysis with 5-source intelligence (Instagram, Website, Blog, TikTok, Google Reviews). Includes TikTok Success Qualification Layer (per-post relative scoring with HIGH/MID/LOW tiers vs creator baseline, plus BaselineReliability guard: RELIABLE/MODERATE/WEAK/INSUFFICIENT bands with lowDataPenalty and qualificationNotes), Reviews Intelligence Layer (regex-based pain/objection/trust-barrier extraction from Google Reviews with frequency scoring, 7 cluster categories, ReviewsReliabilityGuard with volume bands/lowVolumePenalty/recency weighting, and per-cluster dominanceScore/frequencyBand), 5-source weighted signal normalizer, and Hardened Cross-Signal Decision Layer with multi-type agreement detection (DIRECT_AGREEMENT via phrase overlap/Jaccard, INDIRECT_AGREEMENT via keyword clusters/category proximity, WEAK_OVERLAP, CONFLICT, SINGLE_SOURCE), per-decision confidenceFactors array, decisionReason string, matchedEvidence[] trail, and reliability-weighted confidence that integrates TikTok/Reviews reliability guards. Typed decisions: VALIDATED_PAIN, VALIDATED_HOOK, CONFIRMED_OBJECTION, WEAK_SIGNAL, CONFLICTED_SIGNAL. No HIGH confidence without strong multi-source evidence. Decision objects are first-class output (persisted in snapshots, returned via API, consumed by Dual Analysis, Plan Synthesis, and Memory Mutation). Auto-scraping for TikTok (via Apify) and Reviews runs alongside Instagram in the autonomous worker on 24-48h randomized intervals.
