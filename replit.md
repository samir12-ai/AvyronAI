...
- **Structured Signal Flow (Audience → Positioning)**: The Audience Engine outputs `structuredSignals` with 5 categories: `pain_clusters`, `desire_clusters`, `pattern_clusters`, `root_causes`, `psychological_drivers`. Each cluster has `id`, `label`, `frequency`, `confidence`, `evidence[]`, and `sourceLayer` (surface/pattern/interpretation). The Positioning Engine operates as a **hard-gated selection engine** — it REQUIRES structured signals to run (returns `SIGNAL_REQUIRED` status if absent/malformed, persisted to snapshots). Layer 7 injects root_causes and psychological_drivers as signal-sourced territory candidates (with `signalSource` field on `OpportunityGap`). Layer 11 is constrained to only select from structured signals (no generative fallbacks). Post-generation signal enforcement loop (max 3 attempts): validates signal traceability AND generic output detection — if either fails, L11 is re-prompted with rejection context. On exhaustion returns `SIGNAL_DRIFT` status with zero confidence (persisted to snapshots). All enforcement failure states (SIGNAL_REQUIRED, SIGNAL_DRIFT) are persisted via `buildAndPersistEmptyResult()` so `/api/positioning-engine/latest` always reflects true enforcement state. **Orchestrator cascade**: `SIGNAL_CASCADE_MAP` blocks Differentiation, Mechanism, and Offer when positioning has `SIGNAL_REQUIRED` or `SIGNAL_DRIFT` (returns `SIGNAL_BLOCKED` status). `EngineStepResult` union includes `SIGNAL_BLOCKED`. `shouldBlockDownstream()` recognizes signal failures. Positioning gate status tracked in `depthGateStatus` (`SIGNAL_PASSED`/`SIGNAL_REQUIRED`/`SIGNAL_DRIFT`). Frontend: `SignalFlowPanel` shows enforcement status banner (ENFORCED/SIGNAL_REQUIRED/SIGNAL_DRIFT/PENDING). `DepthGateStatusPanel` (in AELDebugPanel) includes positioning with signal-specific status labels (SIGNAL OK/SIGNALS MISSING/SIGNAL DRIFT/SIG BLOCKED) alongside depth gate statuses for all 7 engines.
- **Differentiation Signal Grounding Enforcement**: L12 stability guard now uses hard `groundingFailures` (not soft `lowConfidence` warnings). Signal grounding enforcement loop (max 3 attempts) wraps L11 AI refinement: validates proofability ≥ 0.30, trust alignment ≥ 0.25, no all-weak claims, AND generic output detection — rejects and re-prompts with specific grounding rejection context. On exhaustion returns `SIGNAL_GROUNDING_FAILED` status with confidence=0. Orchestrator cascade: `SIGNAL_CASCADE_MAP` extended — differentiation `SIGNAL_GROUNDING_FAILED` → blocks Mechanism and Offer via `SIGNAL_BLOCKED`. `shouldBlockDownstream()` recognizes `SIGNAL_GROUNDING_FAILED`. Frontend: `DifferentiationEngine.tsx` shows `SIGNAL_GROUNDING_FAILED` as hard red failure, stability guard shows `groundingFailures` as red items. `DepthGateStatusPanel` includes `GROUNDING FAILED` label.
