...
- ✅ Rule extended with `checkVariableDeclarator` (alias-variable detection: `const status = a || b`) and `checkObjectPattern` (destructured-default detection: `const { status = "x" } = obj`). Two new messageIds (`semanticFallbackAlias`, `semanticFallbackDestructured`) added alongside the prior H6/H8 baseline. Pass-3 final: alias / destructured detectors use the full audit-spec regex `(status|verdict|outcome|state|action)$/i` to catch D1 substitutions on suffix-style canonical contract fields (validationState, decisionAction, budgetAction, decisionGateOutcome, marketState, ...). 24 engine-internal first-canonical-write authoring sites carry documented `// eslint-disable-next-line semantic/no-semantic-fallback` comments — same pattern as the pass-2 F1 status-authoring exemptions in iteration-engine and retention-engine.
