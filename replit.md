...
- **Cross-Engine Isolation**: Enforced via `server/strategy/dependency-validation.ts`. Each engine has prohibited write targets preventing cross-domain interference: Channel→no iteration/retention writes, Budget→no channel/iteration/funnel writes, Iteration→no budget/channel/retention/funnel writes, Retention→no budget/channel/iteration/funnel writes. `funnel_stage_assignment` and `retention_metrics` are protected from all external engines (41 total isolation rules). Runtime validation available via POST `/api/system/validate-isolation`. All engines validate dependencies before execution (503 on version mismatch).
...
- **Governance Rules (Strategy Correction Transparency)**: 6 governance rules enforced across strategy corrections: (1) Auto-Injection Transparency — injected conversion channels tagged with `autoInjectedConversion`, `injectionReason`, `injectionStage` flags; (2) Persuasion Correction Boundaries — corrections only affect persuasion scoring, never channel capabilities or other engine outputs, flagged with `persuasionCorrectionApplied`; (3) Correction vs Strategy Distinction — all corrections tagged as original strategy decisions vs system corrections via `wasReconstructed`, `autoInjectedConversion`, `persuasionCorrectionApplied` fields; (4) Decision Gate Awareness — gate scoring applies confidence penalty (3% per correction, max 12%) and marks strategy as `structurallyRepaired` when corrections exist, preventing false confidence inflation; (5) Cross-Engine Conflict Prevention — all corrections confined to channel-selection engine, no cross-domain mutations; (6) Audit Trail — `correctionAuditTrail` array in output records correction type, timestamp, engine responsible, affected channel, and component for every structural correction. Frontend shows "Structurally Repaired" panel with color-coded audit trail entries.
- **Input Validation Gates (Iteration + Retention Engines)**: Both the Iteration Engine and Retention Engine enforce mandatory input gates that block analysis until foundational data is provided. DB tables: `iteration_gate_inputs` (hasExistingAsset, assetDescription, primaryKpi, dataWindowDays, spend/impressions/clicks/leads/purchases/revenue) and `retention_gate_inputs` (hasExistingCustomers, retentionGoal, businessModel, reachableAudience). API endpoints: GET/PUT `/api/campaigns/:campaignId/iteration-gate` and `/api/campaigns/:campaignId/retention-gate`. Engine routes return 422 `INPUT_GATE_LOCKED` if gate requirements are not met. Frontend shows gate input forms with toggle switches, chip selectors, and numeric inputs. Analyze buttons are disabled when gate is locked. Campaign deletion cleans up gate input records.
