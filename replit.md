...
- **Positioning Engine V3**: A 12-layer strategic positioning engine for generating strategic insights and positioning statements. Dominant competitor detection produces strategic advisories (not stability failures). Stability Guard only fails on: market saturation exceeding threshold, insufficient demand signals, or narrative overlap (pseudo-differentiation). Sufficient market data (≥8 competitors, ≥6 signals) can override single-check failures. StabilityResult includes `advisories` array for strategic warnings. UI shows advisories with amber info icons separate from stability checks. **Two-level Category Resolution**: `layer1_categoryDetection` returns `CategoryResult { macro, subcategory }`. When ≥8 competitors + ≥6 signals, engine resolves a Strategic Subcategory (e.g., "AI Marketing System", "Growth Marketing Platform") instead of defaulting to "general". UI displays "macro / subcategory" in the Category meta item. **Expanded Signal Extraction**: 15 strategic signal patterns across semantic clusters (automation, anti_agency, cost_efficiency, speed, systematization, authority, strategy, execution, technology, analytics, growth, customization, community, transparency). `extractStrategicSignals()` scans MI data; `extractAudienceStrategicSignals()` scans audience data. No new data sources — extraction from existing datasets only. **Territory Evidence Density Validation**: `validateTerritoryEvidenceDensity()` checks unique semantic cluster count, cross-source confirmation (≥2 sources among MI/audience/competitor), and non-redundant evidence. Low density lowers territory confidence score without triggering stability failures.
- **Differentiation Engine V3**: A 12-layer proof-backed differentiation engine for identifying and articulating unique selling propositions. ENGINE_VERSION=6. **Uniqueness Score Calibration**: Uniqueness computed from narrative distance (0.40), signal rarity (0.35), territory saturation (0.25); clamped below 95%. **Positioning Pressure**: Positioning alignment derived as inverse of competitor authority weight + narrative overlap + territory saturation. **Territory Evidence Density Protection**: `validateTerritoryEvidenceDensity()` requires ≥2 semantic signal clusters and ≥2 distinct sources (MI/audience/competitor); pillars failing validation are downgraded to exploratory with 60% score reduction. **Mechanism Framing Guard**: Named Process / Framework mechanism types require ≥2 validated claim structures with overallScore ≥ MIN_PILLAR_SCORE and average proofability ≥ 0.50; otherwise falls back to generic differentiation. **Engine Boundary Enforcement**: Engine strictly limited to differentiation territories, claim strength, proof architecture, and mechanism framing. Guardrail blocks strategy/offer/funnel/execution/financial outputs. Downstream engines consume differentiation as advisory inputs only. **Objection Derivation**: `deriveObjectionsFromAudienceData()` ensures engine never blocks on empty objectionMap. Confidence × 0.85 when `lowObjectionDensity`.
