...
- **Deterministic Signal Mapping (Positioning Engine)**: Post-LLM `deterministicSignalMapping()` replaces LLM-dependent `mappedSignalIds` with system-calculated keyword/substring matching against structured signal labels. Matches territory claim text (name, enemyDefinition, contrastAxis, narrativeDirection, painAlignment, desireAlignment) to signal clusters using multi-method scoring (substring=3.0, word overlap=2.0, reverse overlap=0.5) with threshold≥0.5 and top-6 per territory. Orphan audit skips system-mapped territories. Result: signal coverage jumps from 0% to 32.5%, orphaned claims from 8/8 to 0/8, engineConfidence from 0.20 to 0.40.
