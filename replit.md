...
- **FAST_PASS / DEEP_PASS Architecture**: Hardened two-stage inventory system. FAST_PASS collects baseline posts (TARGET_POSTS_FAST=12, no comments, no enrichment). DEEP_PASS runs TWO stages: (1) additional post collection up to TARGET_POSTS_DEEP=30, (2) comment enrichment. Promotion to DEEP_PASS requires ALL FOUR: datasetGrew=true AND postsMet (>=14) AND commentsMet (>=50) AND enrichmentSucceeded. ENRICHMENT_NO_CHANGE status blocks promotion when dataset didn't grow. POST_EXPANSION_FAILED logged when post count doesn't increase. Per-competitor DEEP_PASS_DIAGNOSTICS log emitted with baseline/final counts, real/synthetic comment breakdown, and promotion decision. Orchestrator owns all promotion logic — enrichCompetitorWithComments never sets analysisLevel. Recovery path enforces same quad-gate (dataset growth + dual threshold + enrichment success). validateInventoryConsistency() runs after every DEEP_PASS cycle, demoting inconsistent competitors. LOW_SAMPLE_COMPETITOR flag (posts < MIN_POSTS_PER_COMPETITOR) reduces authority weight by LOW_SAMPLE_WEIGHT_FACTOR=0.5; market activity calculation excludes LOW_SAMPLE when 2+ ACTIVE competitors available. Competitor inventory tracked via `enrichmentStatus` (PENDING/ENRICHING/ENRICHED/SKIPPED/FAILED/ENRICHMENT_NO_CHANGE), `fetchMethod`, `postsCollected`, `commentsCollected`, `dataFreshnessDays`. DATA_DEGRADATION_GUARD prevents overwrites. Duplicate post protection via postId/shortcode. Cooldown enforcement (5-day synthetic enrichment cooldown). Pipeline order: FAST_PASS → baseline snapshot → DEEP_PASS (post collection + comment enrichment) → enriched snapshot. 254 orchestrator tests (FP-1 through FP-56), 981 total tests.
