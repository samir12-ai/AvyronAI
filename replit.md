...
- **Fortress Completion Engines (V3 Strategy Layer)**:
    - **Statistical Validation Engine (V2)**: Evaluates evidence density, validates claims against real MI signals AND cross-engine structural signals (persuasion drivers, awareness triggers, offer proof, funnel trust paths). Classifies claims as `signal`, `structured_inference`, `narrative`, `assumption`, or `inferred`. Confidence calibration treats structured engine inferences as supported evidence. Guard blocks/downgrades weak claims.
