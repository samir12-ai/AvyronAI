...
- **FAST_PASS / DEEP_PASS Architecture**: Hardened two-stage inventory system. FAST_PASS collects baseline posts (TARGET_POSTS_FAST=12, no comments, no enrichment). DEEP_PASS runs TWO stages: (1) additional post collection up to TARGET_POSTS_DEEP=30, (2) comment enrichment. Promotion to DEEP_PASS requires BOTH post threshold (MIN_POSTS_TARGET=14) AND comment threshold (MIN_COMMENTS_TARGET=50). Orchestrator owns all promotion logic — enrichCompetitorWithComments never sets analysisLevel. Recovery path enforces same dual-threshold gate. Competitor inventory tracked via `enrichmentStatus` (PENDING/ENRICHING/ENRICHED/SKIPPED/FAILED), `fetchMethod`, `postsCollected`, `commentsCollected`, `dataFreshnessDays`. DATA_DEGRADATION_GUARD prevents overwrites. Duplicate post protection via postId/shortcode. Cooldown enforcement (5-day synthetic enrichment cooldown). Pipeline order: FAST_PASS → baseline snapshot → DEEP_PASS (post collection + comment enrichment) → enriched snapshot.
