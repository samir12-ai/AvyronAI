...
- TikTok: Dual-source scraping with automatic fallback — Bright Data proxy first (currently blocked by policy_20050 compliance), then Apify (`clockworks~free-tiktok-scraper`) as fallback; `tiktok-apify-scraper.ts` handles Apify actor runs, polling, and dataset retrieval; posts tagged with `source: "apify"` vs `"brightdata"`; comments tagged `tiktok_apify` vs `tiktok_scraped`; data-acquisition queries include both sources; full validation engine: transcript-first hook detection with caption-proxy fallback; audience reaction extraction from comments; hookReliability and transcriptCoverage metadata; all gated behind qualification filter (HIGH+MID posts only)
