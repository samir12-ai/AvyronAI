...
- **Fulfillment Engine**: `server/fulfillment-engine.ts` — single function `computeFulfillment(campaignId, accountId)` computes live progress from `studio_items` only (source of truth, no double-counting with `published_posts`). Returns `{total: {required, fulfilled, remaining}, byBranch: {VIDEO, DESIGNER, WRITER}, byStatus, progressPercent}`. Wired to all 4 execution endpoints (dashboard, progress, required-work, calendar-entries) — all return identical fulfillment numbers from the same function. Branch mapping via `getBranchForMediaType()` in `lib/media-types.ts`: REEL/VIDEO→VIDEO, IMAGE/CAROUSEL→DESIGNER, POST/STORY→WRITER. Remaining = max(0, required - fulfilled), never negative. Frontend Pipeline shows per-branch progress bars with "X / Y completed" and remaining counts.
