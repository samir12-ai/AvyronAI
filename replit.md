...
- **Fortress Completion Engines (V3 Strategy Layer)**:
    - **Statistical Validation Engine (V3)**: Signal-grounded claim architecture. Builds signal clusters from MI (opportunity/threat), Audience (pains, desires, objections, emotional drivers, narrative objections) before validating claims. Claims matched to signal clusters receive `SignalProvenance` metadata (signal_id, signal_source, signal_origin_engine, signal_strength, evidence_reference). Claims without signal chain are classified as `hypothesis` and excluded from scoring. Confidence uses MIN-constrained formula: MIN(claim_support, signal_density, proof_strength, cross_engine_consistency) — never exceeds actual evidence support. Validation guard enforces: if signal-backed claims < 40%, strategy forced to PROVISIONAL and execution engines blocked.
