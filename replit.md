...
- **Plan Documents**: Generation and storage of strategic plans in `plan_documents` table with versioning (`blueprintId`, `version`, `contentJson`, `contentMarkdown`, `isFallback`). Auto-saved when Phase 5 completes. API: `GET /api/plans/:planId/document` and `GET /api/strategic/blueprint/:id/document`. Frontend: `PlanDocumentView` component renders 6 section cards, accessible from BuildThePlan Phase 5 and permanently embedded in StrategicPipeline (no toggle button — always visible when a plan exists). DB-backed so it persists across app restarts. Never returns 500 for expected missing states (404 PLAN_NOT_FOUND / DOCUMENT_NOT_FOUND).
- **Plan Document Design**: "The Plan" section uses gradient header (purple tones), status pill, version/date display, and 6 collapsible sections with gradient-colored icons (Content Distribution, Creative Testing, Budget Allocation, KPI Monitoring, Competitive Watch, Risk Triggers). Detail cards use info chips, platform dots, severity badges, and budget banners. All values rendered via `safeStr()` and `safeArray()` to prevent `[object Object]`.
...
- **Fulfillment Engine**: `server/fulfillment-engine.ts` — single function `computeFulfillment(campaignId, accountId)` computes live progress from `studio_items` only (sole source of truth). `published_posts` is for publishing output only, never counted for fulfillment. Returns `{total: {required, fulfilled, remaining}, byBranch: {VIDEO, DESIGNER, WRITER}, byStatus, progressPercent}`. Wired to all 4 execution endpoints (dashboard, progress, required-work, calendar-entries) + dashboard plan-status — all return identical dynamically-computed fulfillment numbers. No stored counters (legacy columns `generatedCount`, `readyCount`, etc. on `required_work` and `totalStudioItems`, `totalFailed`, `totalCanceled` on `strategic_plans` are no longer read or written). Branch mapping via `getBranchForMediaType()` in `lib/media-types.ts`: REEL/VIDEO→VIDEO, IMAGE/CAROUSEL→DESIGNER, POST/STORY→WRITER. Remaining = max(0, required - fulfilled), never negative. Frontend Pipeline shows per-branch progress bars with "X / Y completed" and remaining counts.
- **Fulfillment Write Paths**: All 3 content creation paths write to `studio_items`: (1) Studio tab `handleSaveMedia` → `POST /api/studio/case` creates both `published_posts` + `studio_items` with `sourcePostId` link, (2) Create tab `handleSave` → `POST /api/studio/items` for text content, (3) Create tab `handleSavePoster` → `POST /api/studio/items` for designs. All paths enforce mandatory `campaignId` (422 if missing). Ad-hoc content (no calendar entry) counts toward fulfillment. Idempotency: `sourcePostId` check on studio/case, `calendarEntryId` unique constraint on studio/items.
- **Fulfillment Delete**: `DELETE /api/studio/items/:id` removes studio item, causing fulfillment remaining to increase on next computation. Studio tab `handleDeleteMedia` calls server delete if `studioItemId` exists on the item.
- **Campaign Scoping Enforcement**: `POST /api/studio/case` and `POST /api/studio/items` both reject requests without `campaignId` with 422 `CAMPAIGN_ID_REQUIRED`. No NULL campaign_id rows are ever created in `studio_items`.
- **Quality Gate Tests**: `server/tests/media-types.test.ts` — 15 tests validating `normalizeMediaType` never returns undefined/null, handles whitespace/plural/edge cases, and `createRouteForContentType` returns valid routes for all content types. `server/tests/plan-document.test.ts` — 5 tests validating plan document API endpoints return proper 404s (never 500s) for expected missing states, correct error codes, and proper response shape. `server/tests/fulfillment.test.ts` — 9 tests validating branch mapping for all canonical types, null/undefined handling, never-negative remaining, and progress percent bounds.
