...
- **FAST_PASS / DEEP_PASS Architecture**: Hardened two-stage inventory system. FAST_PASS collects baseline posts (no comments, no enrichment). DEEP_PASS enriches after FAST_PASS completes (strict gate: lastCheckedAt required). Competitor inventory tracked via `enrichmentStatus` (PENDING/ENRICHING/ENRICHED/SKIPPED/FAILED), `fetchMethod`, `postsCollected`, `commentsCollected`, `dataFreshnessDays`. DATA_DEGRADATION_GUARD prevents overwrites. Duplicate post protection via postId/shortcode. Cooldown enforcement (5-day synthetic enrichment cooldown). Pipeline order: FAST_PASS → baseline snapshot → DEEP_PASS → enriched snapshot.
