...
- **Plan-First Dashboard**: Dashboard hierarchy: Performance → Active Plan Status → Execution Pipeline → Required Work → MarketMind Agent → Meta Strip → Advanced Insights. Floating agent FAB opens full-screen chat modal. Plan Document View (`components/PlanDocumentView.tsx`) surfaces: full 6-stage funnel pipeline (Reach→Clicks→Conversations→Leads→Qualified→Closed with conversion rates), lever-based simulation scenarios with highestLeverageDriver, diagnostics section with merged data sources (docContent + plan.sections), competitive strategy feed, risk triggers with expandable optimization playbooks, early warning system, and Blueprint↔DNA link section.
