...
- **Competitive Intelligence (MIv3)**: An 8-engine sequential pipeline for real-data competitor analysis with 5-source intelligence (Instagram, Website, Blog, TikTok, Google Reviews). Includes TikTok Success Qualification Layer (per-post relative scoring with HIGH/MID/LOW tiers vs creator baseline), Reviews Intelligence Layer (regex-based pain/objection/trust-barrier extraction from Google Reviews with frequency scoring and 7 cluster categories), 5-source weighted signal normalizer, and Cross-Signal Decision Layer (Jaccard similarity grouping with typed decisions: VALIDATED_PAIN, VALIDATED_HOOK, CONFIRMED_OBJECTION, WEAK_SIGNAL, CONFLICTED_SIGNAL). Auto-scraping for TikTok (via Apify) and Reviews runs alongside Instagram in the autonomous worker on 24-48h randomized intervals.
