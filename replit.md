...
- **Authority Hierarchy Enforcement**: Strict Awareness → Funnel → Persuasion authority hierarchy. Awareness decisions are immutable downstream. Funnel cannot contradict awareness stage (AWARENESS_BLOCKED_FUNNELS blocks direct/tripwire/application/product-launch for unaware). Persuasion cannot override awareness conclusions (hard CTA blocked for unaware/problem_aware, education-only sequences with soft_next_step). Entry trigger enforcement has 3 valid derivation paths only (no implicit fallback). Anti-hype enforcement applies double penalty (0.2/hit) for low-readiness audiences. Cross-engine validation detects awareness↔funnel↔persuasion contradictions. Orchestrator gates funnel execution behind completed awareness output (MISSING_DEPENDENCY).
