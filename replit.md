...
- **Positioning Engine V3**: 12-layer strategic positioning engine (80% deterministic, 20% AI synthesis). Layers: Category Detection, Market Narrative Map, Narrative Saturation Detection, Trust Gap Detection, Segment Priority Resolution, Market Power Analysis, Opportunity Gap Detection, Differentiation Axis Construction, Narrative Distance Scoring, Strategic Territory Selection, Positioning Statement Generation (AI only), Stability Guard. Mandatory dependency chain: MI Snapshot → Audience Snapshot → Positioning. Outputs Strategy Cards, territories with opportunity/confidence scores, flanking mode detection, stability validation. Status system: COMPLETE/MISSING_DEPENDENCY/INSUFFICIENT_SIGNALS/UNSTABLE. Frontend: Positioning tab (split from old Strategy tab) with Strategy Cards, Opportunity Gaps, Market Power bars, Stability Guard, Differentiation Axes. 56 unit tests. Hardening: Generic Territory Penalty (`computeSpecificityScore` with dilution for multi-word territories, GENERIC_TERRITORY_PATTERNS list), Narrative Validation Filter (`validateNarrativeOutput` rejects ad copy/CTAs/superlatives on Layer 11 AI outputs), Semantic Saturation Detection (`computeSemanticSaturation` with token overlap + hook archetype matching + min floor per competitor count), Narrative Distance Calibration (keyword overlap >25% caps distance at 0.75, substring matching), Multi-Signal Flanking Mode (engagement ≥0.50 + narrativeOwnership ≥0.60 + contentVolume ≥0.50 triggers flanking), Cross-Campaign Territory Diversity (`checkCrossCampaignDiversity` with Jaccard similarity, penalty applied before Layer 11).
- **Differentiation Engine V3**: 12-layer proof-backed differentiation engine. Pipeline: MI v3 → Audience v3 → Positioning v3 → Differentiation V3 → Build The Plan. Layers: L1 Positioning Intake Lock, L2 Competitor Claim Surface Mapping, L3 Claim Collision Detection, L4 Proof Demand Mapping, L5 Trust Gap Prioritization, L6 Authority Mode Selection (expert/operator/transparent_guide/contrarian_specialist/niche_authority), L7 Proof Asset Architecture (7 proof categories), L8 Mechanism Construction, L9 Differentiation Pillar Scoring (uniqueness*0.30 + proofability*0.25 + trustAlignment*0.20 + positioningAlignment*0.15 + objectionCoverage*0.10), L10 Claim Strength Scoring, L11 AI Refinement (wording only, guardrails enforced), L12 Stability Guard. Guardrails: no new territories, no offer logic, no channel logic, no audience recreation, no copywriting. Output sanitization on all final text. Cross-campaign enforcement on MI/Audience/Positioning snapshots. Status system: COMPLETE/MISSING_DEPENDENCY/INSUFFICIENT_SIGNALS/UNSTABLE/COLLISION_RISK. Frontend: Differentiation tab (separate from Positioning) showing pillars, proof architecture, authority mode, mechanism framing, trust priority map, claim structures, collision diagnostics, stability guard. 40 unit tests. Files: `server/differentiation-engine/` (constants.ts, types.ts, engine.ts, routes.ts), `components/DifferentiationEngine.tsx`.
- **Competitive Intelligence (MIv3)**: Provides real-data competitor analysis through a 6-layer pipeline (Signals → ContentDNA → Intent → Trajectory → Confidence → Dominance) for data processing and signal generation, with robust data integrity checks, cooldown mechanisms, and dynamic time window adjustments. It includes a two-speed data collection (Fast Pass=12 posts, Deep Pass=30 posts target) with Deep Pass recovery — `enrichCompetitorWithComments()` generates comments from stored posts without re-scraping (up to 8 highest-engagement posts, engagement+recency sorting), `recoverStuckDeepPass()` auto-detects stuck FAST_PASS competitors on server startup and enriches them. Deep Pass skips additional post collection when competitor already has ≥30 stored posts. Features market activity vs. market demand analysis. **Synthetic Comment Tagging**: All generated comments tagged with `isSynthetic=true` + `source=synthetic_enrichment` in DB and `[synthetic]` text prefix. Audience Engine filters synthetic comments at both DB query level (`isSynthetic=false`) and text level (`SYNTHETIC_FILTERS`). **Freshness Single Source of Truth**: `computeDataFreshnessDays()` is the sole function for freshness calculation across MI persistence, engine state, Strategic Gate, Positioning gate, and confidence decay. **Synthetic Comment Lifecycle**: `cleanupExpiredSyntheticComments()` runs 30s after startup — deletes synthetic comments older than `SYNTHETIC_RETENTION_DAYS` (30d), re-enriches affected competitors below threshold, demotes to FAST_PASS when no posts exist, removes orphaned comment references. **Churn Prevention**: `SYNTHETIC_ENRICHMENT_COOLDOWN_DAYS` (5d) per-competitor cooldown via `lastSyntheticEnrichmentAt`, real data preference (`REAL_DATA_SUFFICIENT` check before synthetic), `HIGH_SYNTHETIC_CHURN` flag when >2 enrichments in `SYNTHETIC_CHURN_WINDOW_DAYS` (14d), `SyntheticLifecycleDiagnostics` with 8 fields (generated/expired/regenerated counts, cooldownBlocked, realSufficient, highChurn competitors, averageDaysBetweenSyntheticRegeneration). Schema: `lastSyntheticEnrichmentAt`, `syntheticEnrichmentCount`, `syntheticChurnFlag` on `ciCompetitors`. 49 lifecycle validation tests.
...
- **Strategic Core Architecture ("Build The Plan")**: A 6-phase sequential engine (Gate → Blueprint → Confirm → Analyze → Validate → Execute) for comprehensive plan generation, generating an AI Creative Blueprint from market intelligence, competitor signals, business data, and campaign context. Strategic Gate (Phase 0) enforces MI v3 readiness via `getEngineReadinessState()` — gate blocks with `MI_NOT_READY` if no valid MI snapshot exists. Frontend shows MI status indicator (green/red) and disables "Unlock Analysis" button when MI is not READY.
...
- **Fetch System Hardening (6 Safeguards)**: Fair request allocation with per-competitor request caps (`Math.ceil(dynamicRequestBudget / competitorCount)`), explicit per-competitor fetch status (FETCH_SUCCESS/FETCH_FAILED/RATE_LIMITED/NO_POSTS_FOUND/SKIPPED_DUE_TO_BUDGET/SKIPPED_DUE_TO_RUNTIME/SKIPPED_DUE_TO_PAGES_CEILING/COOLDOWN_ACTIVE/BLOCKED_BY_PLATFORM), partial completion awareness with coverage ratio (`competitorsProcessed/competitorsTotal`) and scaled confidence penalty (`(1 - coverageRatio) * 0.5`), dynamic budget scaling (`computeDynamicBudgets()`: requests = 10 × competitorCount, runtime = 100s × competitorCount with 10min floor / 25min ceiling), structured diagnostics logging (`FetchJobDiagnostics` JSON per job with all coverage/budget/status fields), and 16 validation tests. Coverage data included in `getFetchJobStatus` response and audit logs.
