...
- **System Control Layer (Phase 3 — Repair Actions)**: A unified final authority layer in `server/system-control/` (engine.ts, types.ts, constants.ts, structural-checks.ts, contradiction-detector.ts, repair-actions.ts) that runs after all 15 engines complete and before plan synthesis. Evaluates full system state: structural checks, cross-engine contradiction detection, and produces a `SystemControlVerdict` with verdict (`PASS|DOWNGRADE|REPAIR|BLOCK`), execution mode, block reasons, downgrades, repair actions, and contradiction reports. **Phase 2 enforcement active**: BLOCK overrides `overallStatus` to BLOCKED; DOWNGRADE mutates budget action with attribution. **Phase 3 repair actions**: When all blocks are repairable, the engine attempts automated fixes before final verdict. Repairable: `NO_CONVERSION_PATH` → `INJECT_FALLBACK_CONVERSION` (injects safe default conversion channel with `systemControlRepair: true` flag), `SCALE_WITHOUT_REAL_DATA` → `DOWNGRADE_SCALE_TO_TEST` (converts block to test-mode), `INTEGRITY_FAILURE` → `REVALIDATE_INTEGRITY` (confirms failure, repair fails as expected). Non-repairable (always hard block): `COMPLIANCE_FAILURE`, `BUDGET_KILL`, `BUDGET_HALT`. Mixed repairable + non-repairable → repairs skipped, BLOCK stands. Shadow mode skips repairs entirely. Repair metadata (`repairActions[]`, `repairAttempted`) included in verdict. `controlVerdict` returned in `OrchestratorRunResult`. `CONFLICT_PRIORITY` has `"system_control"` tier above `"hard_constraints"`. Does NOT modify memory system, adaptive rhythm, iteration engine, exploration budget, or autonomous worker. 107/107 validation tests in `server/tests/system-control-proof.ts`.
