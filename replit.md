...
- **Fortress Completion Engines (V3 Strategy Layer)**:
    - **Statistical Validation Engine (V4)**: Signal-grounded claim architecture with equivalence mapping and traceability. **Signal Equivalence Mapping Layer** translates engine-specific source tags (offer_outcome, offer_mechanism, offer_proof, persuasion_driver, awareness_trigger, awareness_entry) to canonical signal clusters (audience_pain, market_opportunity, emotional_driver, audience_desire, audience_objection) with a softer 1-word match threshold for priority clusters (vs 2-word default). Only normalizes existing signals — never creates artificial ones. **Signal Trace System** assigns each claim a `signalTraceId` (e.g., OFFER_OUTCOME_001) and `signalPath` array showing the engine chain (e.g., [audience, offer, statistical_validation]). All unmapped signals are logged in diagnostics and displayed in the frontend. Builds signal clusters from MI (opportunity/threat), Audience (pains, desires, objections, emotional drivers, narrative objections) before validating claims. Claims matched to signal clusters receive `SignalProvenance` metadata. Claims without signal chain are classified as `hypothesis` and excluded from scoring. Confidence uses MIN-constrained formula: MIN(claim_support, signal_density, proof_strength, cross_engine_consistency). Validation guard enforces: if signal-backed claims < 40%, strategy forced to PROVISIONAL and execution engines blocked. **Multi-Hop Signal Lineage (V4.1)**: True multi-hop lineage persisted across all 5 snapshot tables via `signal_lineage` text column. Source engines (MI, Audience) generate `SignalLineageEntry` records at insert time with `hopDepth=0`. Intermediate engines (Offer, Awareness, Persuasion) read upstream lineage, match outputs to parent signals via word overlap, and create derived entries with incremented `hopDepth` and extended `signalPath`. Validation engine reads merged lineage from all upstream snapshots and resolves each claim to its real parent signal ID, origin engine, and full multi-hop path. Frontend displays `parentSignalId`, `originEngine`, and hop count on claim cards. Shared module: `server/shared/signal-lineage.ts`.
