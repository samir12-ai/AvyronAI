...
**Founding rule: every orchestrator decomposition step must be replay-verifiable BEFORE it lands.** A `ReplayCassette` is a content-addressed, deterministic recording of one `runOrchestrator(...)` invocation. The player consumes a cassette and runs a candidate orchestrator with all LLM calls STRICTLY mocked from the recorded outputs (NO re-rolls). Divergences are classified into a 7-class taxonomy so downstream Phase 4-B/4-C decomposition steps can prove behavioral equivalence against the live orchestrator.

| # | Rule | Enforcement |
|---|---|---|
| **REPLAY-HOF** | Every recorder boundary in `server/orchestrator/**` MUST funnel through the variable returned by `withReplayRecorder(...)` (convention: name it `__recorder`). Direct `recorder.record*(...)` calls outside the HOF bypass the sample-rate / off-switch gating. | ESLint rule `orchestrator-replay/no-bare-llm-call-in-replay` (`.local/eslint-rules/no-bare-llm-call-in-replay.js`) detects bare receivers. Allowed receiver name: literal `__recorder`. |
| **REPLAY-LLM-STRICT** | Direct imports of `aiChat` / `aiGemini` / `getOpenAI` / `getGemini` are FORBIDDEN inside `server/orchestrator/replay/**`. Replay code MUST route LLM calls through `StrictLlmMock`. A real LLM call from replay code would defeat hermeticity. | Same ESLint rule, mode (A). Exception list: `recorder.ts`, `llm-strict-mock.ts` (the adapter itself). |
| **REPLAY-GATE-OFF-DEFAULT** | Production recording is OFF by default. `ORCH_REPLAY_RECORD` env knob: unset / `0` / `false` → disabled (no-op recorder, zero overhead); `1` / `true` → every invocation; `sample:N` → 1-in-N. Target: ≤2% wall-clock overhead at `sample:50` (rolling p50 surfaced via `cv13_replay_recorder_overhead_ratio`). | `parseRecorderGate()` in `server/orchestrator/replay/recorder.ts`. `withReplayRecorder()` returns the singleton no-op recorder when gated off. |
| **REPLAY-SCHEMA-VERSION** | Cassette body carries `schemaVersion`. Player throws `ReplayCassetteVersionError` on unknown versions — never best-effort migrates. DB CHECK constraint `schema_version > 0` + UNIQUE on `cassette_hash`. | `REPLAY_CASSETTE_SCHEMA_VERSION = 1` in `server/orchestrator/replay/types.ts`. Migration `027_orchestrator_replay_cassettes.sql`. |
| **REPLAY-PII-REDACTED** | Cassettes are PII-redacted BEFORE persistence (email / @handle / FQDN). `RedactionMap` lives ONLY in memory during the request and is `.clear()`ed in `finalize()`. Same-input → same hash-token so diff equality is preserved across runs. | `server/orchestrator/replay/redaction.ts` + 6 redaction tests. |
| **REPLAY-HASH** | `cassetteHash` is SHA-256 of the INPUT envelope only — never the full body. Two captures of the same `(campaignId, accountId, forceRefresh, scopedEngines, …)` collide on the UNIQUE index so `ON CONFLICT DO NOTHING` dedupes the corpus, and a player can resolve a cassette by its input-fingerprint. Per-engine outputs / verdict / ledger are the EXPECTATIONS the player diffs against — not part of the cassette's identity. | `LiveRecorder.finalize()` computes `hashValue(this.input)`. |
| **REPLAY-LLM-FIFO** | When two recorded LLM calls share the same `(provider, model, promptHash)`, the `StrictLlmMock` MUST hand them back in original `callOrder` (FIFO queue per key). Overwriting on the second insert would silently drop the first recorded response when identical prompts repeat. Once the queue for a key is exhausted, further identical calls throw `LlmMockMissError`. | `StrictLlmMock` per-key array; insertion sorted by `callOrder` ascending; `resolve()` calls `.shift()`. |
| **REPLAY-DEEP-DIFF** | The classifier MUST do a true structural deep walk over the observation envelope after the boutique checks run, so any field added by a future engine (or any non-enumerated value drift like `controlVerdict.blockReasons`) surfaces as `STRUCTURAL` instead of silently passing. A `seen` set carries every path the boutique checks already classified so precedence (CANONICAL_FIELD wins over a co-located STRUCTURAL diff) holds and we never double-report. | `deepWalk()` in `server/orchestrator/replay/diff.ts` + tests 8–10 in `diff-fixtures.test.ts`. |
| **REPLAY-CAPTURE-PARITY** | The `cassetteHash` computed by `LiveRecorder.finalize()` MUST equal the `cassetteHash` computed by `scripts/replay-cli.ts capture-synthetic` for the same input envelope. Drift here means a synthetic cassette and a recorder-produced cassette of the same input cannot resolve via the same hash, breaking the player's lookup model. | Both call sites use `hashValue(body.input)`. |
| **REPLAY-DEDUPE-METRIC** | `cv13_replay_cassettes_total` MUST only increment on an actual `INSERT` — never on `ON CONFLICT DO NOTHING`. The recorder uses `RETURNING cassette_hash` and gates the metric increment on `rows.length > 0` so duplicate input captures of a hot campaign do not drift the corpus-size metric upward. | `LiveRecorder.finalize()` post-INSERT branch. |

Divergence taxonomy (precedence top-down): `STRUCTURAL` > `CANONICAL_FIELD` (D2-tracked) > `DEGRADATION_SURFACE` > `BUDGET_LEDGER` > `PROVENANCE` > `ORDER` > `TIMING_ONLY` (whitelist).

**Recorder integration in `runOrchestrator`** — all 9 boundaries wired in `server/orchestrator/index.ts`: (1) input at entry (line ~3576), (2) ctx-resolved + (8a) in-flight register after `config.jobId = jobId` (~3679), (3) per-engine-output after each `results.set(...)` (~4050), (6 site) system-control-verdict marker at `evaluateSystemControl` call (~4243) with capture after `handleTerminal()` once verdict is stable (~4626), (7) budget-ledger after each `budgetDecisionLedger.push(...)` (~4459), (4) synthesis-input + (5) plan-persist around `synthesizePlan(...)` (~4494/4504), (8b) in-flight settle after `handleTerminal()` (~4620), (9) final-result before each of the three returns (early BLOCKED ~3893, NEEDS_INPUT ~4164, main success ~4724). `finalize()` runs from a single site in the existing `try { ... } finally { ... }` (line ~4795) and is followed by a thrown-before-final guard that stamps `status: "ERROR"` so an aborted run still emits an `incomplete=skipped` audit line rather than a half-cassette.

**CLI candidates** — `replay:run --against current` invokes the real `runOrchestrator` via `currentOrchestratorCandidate()` and projects its `OrchestratorRunResult` into an `ActualReplayObservation` the deep-diff classifier walks. P4-A limitation: `runOrchestrator` does not yet accept an injected LLM adapter, so `--against current` is NON-HERMETIC (real LLM calls); the CLI prints a banner warning. `--against candidate` uses the deterministic identity candidate (cassette → observation echo) — that path is the green CI smoke target. Hermetic `--against current` lands in P4-B when the orchestrator's LLM clients accept a `StrictLlmMock`.

CV-13 metrics (mounted on `/metrics` via the existing `X-Admin-Token` gate): `cv13_replay_cassettes_total{source}`, `cv13_replay_age_max_hours`, `cv13_replay_recorder_overhead_ratio` (rolling 5-min p50), `cv13_replay_player_runs_total{outcome}`. Operator panel endpoints: `GET /api/admin/replay/cassettes` (corpus summary + last 50) and `GET /api/admin/replay/cassette/:hash` (one body, admin-token-gated download).

CLI: `npm run replay:list`, `npm run replay:run -- --cassette <hash> [--against current|candidate]` (exit non-zero on any non-TIMING_ONLY divergence), `npm run replay:capture-synthetic` (loads `server/tests/orchestrator-replay/cassettes/*.json` into DB). Test suite: `npm run replay:test` (34 tests across recorder/player/diff/redaction/synthetic-cassettes); flake check: `npm run replay:flake` (100-iteration mirror of `lifecycle-flake-check.sh`, Seal #18 conventions).

Six synthetic cassettes committed under `server/tests/orchestrator-replay/cassettes/` covering the seven declared path-shapes (clean, gate_retry, budget_downgrade, scoped_rerun, blocked_by_integrity, needs_input, error). Seven hand-crafted divergence fixtures prove the classifier returns the correct class for every taxonomy entry. Production sample:50 7d enablement is queued for operator handoff once the canary window is scheduled.
